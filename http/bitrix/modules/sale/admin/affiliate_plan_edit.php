...
?>
	...
	<?endif;?>
	<tr>
		<td width="40%"><span class="required">*</span><?echo GetMessage("SAPE1_SITE")?></td>
		<td width="60%">
			<?echo CSite::SelectBox("SITE_ID", $str_SITE_ID, "", "", "");?>
		</td>
	</tr>
	<tr>
		<td><span class="required">*</span><?echo GetMessage("SAPE1_NAME")?></td>
		<td>
			<input type="text" name="NAME" size="60" maxlength="250" value="<?= $str_NAME ?>">
		</td>
	</tr>
	<tr>
		<td valign="top"><?echo GetMessage("SAPE1_DESCR")?></td>
		<td valign="top">
	...
	<tr>
		<td><span class="required">*</span><?echo GetMessage("SAPE1_RATE")?></td>
		<td>
	...
	<tr>
		<td valign="top"><?= (($affiliatePlanType == "N") ? GetMessage("SAPE1_LIMIT") : GetMessage("SAPE1_LIMIT_HINT")) ?>:</td>
		<td valign="top">
		...
		<td colspan="2">
			...
			while ($arIBlock = $dbIBlockList->Fetch())
			{
				...
				foreach ($arSections as $sectionID => $arSubSection)
				{
					...
					for ($i = 0; $i < count($arSubSection); $i++)
					{
						...
						$str2 .= ", '".addslashes(htmlspecialcharsEx($arSubSection[$i]["NAME"]))."'";
					}
					$str1 .= ");\r\n";
					$str2 .= ");\r\n";
				}
			...
			function AffAddSectionRow(cnt, id, moduleID, sectionID, rate, rateCmn, ar)
			{
				...
				if ($simpleForm != "Y")
				{
					...
					oCell.vAlign = 'top';
					str = '';
					str += '<select name="MODULE_ID_' + cnt + '" id="ID_MODULE_ID_' + cnt + '" OnChange="ModuleChange(' + cnt + ')">';
					<?
				...
				str += '<select name="SECTION_IBLOCK_ID_' + cnt + '" onChange="ChlistIBlock(' + cnt + ')">';
				str += '<option value="0"> - </option>';
				<?
				foreach ($arIBlockCache as $key => $arIBlock)
				{
					?>str += '<option value="<?= $arIBlock["ID"] ?>"><?= str_replace("'", "\'", htmlspecialchars("[".$arIBlockTypeCache[$arIBlock["IBLOCK_TYPE_ID"]]."] ".$arIBlock["NAME"])) ?></option>';<?
				}
				?>
				str += '</select><br>';
				<?
				...
				{
					?>
					str += '<select name="SECTION_SELECTOR_LEVEL_' + cnt + '[<?= $i ?>]" onChange="Chlist(' + cnt + ', <?= $i ?>)">';
				...
				str += '<select name="RATE_TYPE_CMN_' + cnt + '" id="ID_RATE_TYPE_CMN_' + cnt + '">';
				str += '<option value="P">%</option>';
				<?
				foreach ($arCurrencies as $key => $value)
				{
					?>str += '<option value="<?= $key ?>"><?= AddSlashes($value) ?></option>';<?
				}
				?>
				str += '</select>';
			...
			<table cellpadding="3" cellspacing="1" border="0" width="100%" class="internal" id="SECTIONS_TABLE">
				<tr class="heading">
					...
					if ($simpleForm != "Y")
					{
						?><td><span class="required">*</span><?echo GetMessage("SAPE1_MODULE")?></td><?
					}
					?>
					<td><span class="required">*</span><?echo GetMessage("SAPE1_SECTION")?></td>
					<td><span class="required">*</span><?echo GetMessage("SAPE1_RATE1")?></td>
					<td>&nbsp;</td>
				</tr>
				<?
				$cnt = -1;
				if ($bVarsFromForm)
				{
					...
					if ($NUM_SECTIONS > 0)
					{
						for ($i = 0; $i <= $NUM_SECTIONS; $i++)
						{
							...
							<script language="JavaScript">
							<!--
							AffAddSectionRow(-1, <?= htmlspecialchars(${"ID_".$i}) ?>, '<?= htmlspecialchars(${"MODULE_ID_".$i}) ?>', '<?= htmlspecialchars(${"SECTION_ID_".$i}) ?>', '<?= htmlspecialchars(${"RATE_".$i}) ?>', '<?= htmlspecialchars(${"RATE_TYPE_CMN_".$i}) ?>', <?= ((StrLen($str) > 0) ? "new Array(0, ".$str.")" : "new Array()") ?>);
							//-->
							</script>
							<?
						}
				...
				else
				{
					...
					{
						$cnt++;
						$str_MODULE_ID = htmlspecialchars($arPlanSection["MODULE_ID"]);
						$str_SECTION_ID = htmlspecialchars($arPlanSection["SECTION_ID"]);
						$str_RATE = htmlspecialchars($arPlanSection["RATE"]);
						if ($arPlanSection["RATE_TYPE"] == "P")
							$str_RATE_TYPE_CMN = "P";
						else
							$str_RATE_TYPE_CMN = htmlspecialchars($arPlanSection["RATE_CURRENCY"]);
						...
						<script language="JavaScript">
						<!--
						AffAddSectionRow(-1, <?= $arPlanSection["ID"] ?>, '<?= $str_MODULE_ID ?>', '<?= $str_SECTION_ID ?>', '<?= $str_RATE ?>', '<?= $str_RATE_TYPE_CMN ?>', <?= ((StrLen($str) > 0) ? "new Array(0, ".$str.")" : "new Array()") ?>);
						//-->
						</script>
						<?
					}
