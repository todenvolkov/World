...
if (($arID = $lAdmin->GroupAction()) && $saleModulePermissions >= "W")
{
	...
	foreach ($arID as $ID)
	{
		...
		switch ($_REQUEST['action'])
		{
			...
			case "pay_affiliate":
				if (!CSaleAffiliate::PayAffiliate($ID, "P"))
				{
					if ($ex = $APPLICATION->GetException())
						$lAdmin->AddGroupError($ex->GetString(), $ID);
					else
			...
			case "pay_affiliate_account":
				if (!CSaleAffiliate::PayAffiliate($ID, "U"))
				{
					if ($ex = $APPLICATION->GetException())
						$lAdmin->AddGroupError($ex->GetString(), $ID);
					else
...
while ($arAffiliate = $dbResultList->NavNext(true, "f_"))
{
	...
	$fieldEdit = "<input type=\"text\" name=\"FIELDS[".$f_ID."][PAID_SUM]\" value=\"".htmlspecialchars($val)."\" size=\"7\">";
	...
	$fieldEdit = "<input type=\"text\" name=\"FIELDS[".$f_ID."][PENDING_SUM]\" value=\"".htmlspecialchars($val)."\" size=\"7\">";
