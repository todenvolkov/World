...
while ($arDeliveryService = $dbResultList->NavNext(true, "f_"))
{
	$row =& $lAdmin->AddRow($f_SID, $arDeliveryService, "sale_delivery_handler_edit.php?SID=".$f_SID."&lang=".LANG, GetMessage("SALE_EDIT_DESCR"));
	
	$row->AddField("SID", $f_SID);
	...
	if ($f_INSTALLED == 'Y')
	{
		$res = '';
		$bUseTable = true;
		foreach ($arDeliveryHandlersList[$f_SID] as $siteID => $arHandler)
		{
			if ($siteID == 'ALL')
			{
				$res = '<div class="lamp-'.($f_ACTIVE == "Y" ? "green" : "red").'">';
				$bUseTable = false;
				break;
			}
			else
	...
	$row->AddField("HANDLER", $f_HANDLER);
	...
	if ($saleModulePermissions >= "W")
	{
		...
		if ($f_INSTALLED == "Y")
		{
			$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("SALE_DH_EDIT_DESCR"), "ACTION"=>$lAdmin->ActionRedirect("sale_delivery_handler_edit.php?SID=".urlencode($f_SID)."&lang=".LANG), "DEFAULT"=>true);
			$arActions[] = array("SEPARATOR" => true);
			$arActions[] = array("ICON"=>"delete", "TEXT"=>GetMessage("SALE_DH_DELETE_DESCR"), "ACTION"=>"if(confirm('".GetMessage('SALE_DH_CONFIRM_UNINSTALL')."')) ".$lAdmin->ActionDoGroup($f_SID, "reset"));
		}
		else
		{
			$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("SALE_DH_EDIT_DESCR"), "ACTION"=>$lAdmin->ActionRedirect("sale_delivery_handler_edit.php?SID=".urlencode($f_SID)."&lang=".LANG), "DEFAULT"=>true);
...
if ($saleModulePermissions == "W")
{
	$arContext = array(
		array(
			...
			"LINK" => "sale_delivery.php?lang=".LANG,
