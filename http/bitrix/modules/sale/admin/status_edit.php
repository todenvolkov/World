...
?>
	...
	<tr>
		<td width="40%">
			<?if (strlen($ID)<=0):?><span class="required">*</span><?endif;?><?echo GetMessage("SALE_CODE")?><?if (strlen($ID)<=0):?> (1 <?echo GetMessage("SALE_CODE_LEN")?>)<?endif;?>:
		</td>
		<td width="60%">
			<?if (strlen($ID)>0):?>
				<b><?echo $ID ?></b>
	...
	for ($i = 0; $i<count($arSysLangs); $i++):
		...
		$str_NAME = htmlspecialchars($arStatusLang["NAME"]);
		$str_DESCRIPTION = htmlspecialchars($arStatusLang["DESCRIPTION"]);
		if ($bInitVars)
		{
		...
		<tr>
			<td>
				<span class="required">*</span><?echo GetMessage("SALE_NAME")?>:
			</td>
			<td>
	...
	<tr>
		<td colspan="2" align="center">
			...
			<tr class="heading">
				<td rowspan="2"><?= GetMessage("SSEN_USER_GROUP") ?></td>
				<td colspan="8"><?= GetMessage("SSEN_USER_PERMS") ?></td>
			</tr>
			<tr class="heading">
