...
?>
	...
	<tr>
		<td valign="top">
			<span class="required">*</span><?echo GetMessage("SALE_LOCATIONS")?>:<br><img src="/bitrix/images/sale/mouse.gif" width="44" height="21" border="0" alt="">
		</td>
		<td valign="top">
			<select name="LOCATION_ID[]" size="10" multiple>
				...
				<?while ($vars = $db_vars->Fetch()):?>
					<option value="<?echo $vars["ID"]?>"<?if (in_array(IntVal($vars["ID"]), $arCurLocs)) echo " selected"?>><?echo htmlspecialchars($vars["COUNTRY_NAME"]." - ".$vars["CITY_NAME"])?></option>
				<?endwhile;?>
			</select>
		</td>
	</tr>
	...
	for ($i = 0; $i<count($arSysLangs); $i++):
		...
		<tr>
			<td><span class="required">*</span><?echo GetMessage("SALE_NAME")?>:</td>
			<td>
