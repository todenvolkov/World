...
{
	$row =& $lAdmin->AddRow($f_ID, $arCCard, "sale_delivery_edit.php?ID=".$f_ID."&lang=".LANG.GetFilterParams("filter_"), GetMessage("SALE_EDIT_DESCR"));
	...
	$arActions = Array();
	$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("SALE_EDIT_DESCR"), "ACTION"=>$lAdmin->ActionRedirect("sale_delivery_edit.php?ID=".$f_ID."&lang=".LANG.GetFilterParams("filter_").""), "DEFAULT"=>true);
...
if ($saleModulePermissions == "W")
{
	$aContext = array(
		array(
			...
			"LINK" => "sale_delivery_edit.php?lang=".LANG,
...
?>
		...
		<td>
			<select name="filter_location">
				...
				<?while ($vars = $db_vars->Fetch()):?>
					<option value="<?echo $vars["ID"]?>"<?if (IntVal($vars["ID"])==IntVal($filter_location)) echo " selected"?>><?echo htmlspecialchars($vars["COUNTRY_NAME"]." - ".$vars["CITY_NAME"])?></option>
				<?endwhile;?>
			</select>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<input type="text" name="filter_weight_from" value="<?echo htmlspecialchars($filter_weight_from) ?>" size="5">
			<?echo GetMessage("SALE_F_TO")?>
			<input type="text" name="filter_weight_to" value="<?echo htmlspecialchars($filter_weight_to) ?>" size="5">
		</td>
	</tr>
	<tr>
		...
		<td>
			<?echo GetMessage("SALE_F_FROM")?>
			<input type="text" name="filter_order_price_from" value="<?echo htmlspecialchars($filter_order_price_from) ?>" size="10">
			<?echo GetMessage("SALE_F_TO")?>
			<input type="text" name="filter_order_price_to" value="<?echo htmlspecialchars($filter_order_price_to) ?>" size="10">
