...
{
	$row =& $lAdmin->AddRow($f_ID, $arLocation);

	$row->AddField("ID", $f_ID);
	$row->AddField("COUNTRY_NAME", $f_COUNTRY_NAME_ORIG." <small>[".$f_COUNTRY_NAME."]</small>");
	$row->AddField("CITY_NAME", $f_CITY_NAME_ORIG.((IntVal($f_CITY_ID)>0) ? " <small>[".$f_CITY_NAME."]</small>" : ""));
	$row->AddField("SORT", $f_SORT);

	$arActions = Array();
	$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("SALE_EDIT_DESCR"), "ACTION"=>$lAdmin->ActionRedirect("sale_location_edit.php?ID=".$f_ID."&lang=".LANG.GetFilterParams("filter_").""), "DEFAULT"=>true);
...
?>
	<tr>
		...
		<td>
			<select name="filter_country">
				...
				{
					?><option value="<?echo $arContList["ID"] ?>"<?if (IntVal($arContList["ID"])==IntVal($filter_country)) echo " selected";?>><?echo htmlspecialchars($arContList["NAME"]) ?> [<?echo htmlspecialchars($arContList["NAME_LANG"]) ?>]</option><?
				}
				?>
			</select>
		</td>
	...
	<tr>
		<td><?echo GetMessage("SALE_F_COUNTRY")?>:</td>
		<td>
			<input type="text" name="filter_country_name" value="<?echo htmlspecialchars($filter_country_name) ?>" size="30"><?=ShowFilterLogicHelp()?>
		</td>
	</tr>
	<tr>
		<td><?echo GetMessage("SALE_F_CITY")?>:</td>
		<td>
			<input type="text" name="filter_city_name" value="<?echo htmlspecialchars($filter_city_name) ?>" size="30"><?=ShowFilterLogicHelp()?>
		</td>
	</tr>
<?
$oFilter->Buttons(
