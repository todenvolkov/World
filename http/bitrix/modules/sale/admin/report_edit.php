...
<!--
	...
	while ($arOrderProps = $dbOrderProps->GetNext())
	{
		...
		if(!in_array($orderID, $arOrderSel))
		{
			...
			$i++;
			?>
			arPropFieldsList[<?= $i ?>] = '<?= str_replace("'", "\'", $orderID) ?>';
			arPropFieldsNameList[<?= $i ?>] = '<?= str_replace("'", "\'", "[".$orderID."] ".$arOrderProps["NAME"]) ?>';
			<?
			if ($arOrderProps["TYPE"] == "LOCATION")
			{
				$i++;
				?>
				arPropFieldsList[<?= $i ?>] = '<?= str_replace("'", "\'", $orderID."_COUNTRY") ?>';
				arPropFieldsNameList[<?= $i ?>] = '<?= str_replace("'", "\'", "[".$orderID."] ".$arOrderProps["NAME"]." (".GetMessage("SPS_JCOUNTRY").")") ?>';
				<?

				$i++;
				?>
				arPropFieldsList[<?= $i ?>] = '<?= str_replace("'", "\'", $orderID."_CITY") ?>';
				arPropFieldsNameList[<?= $i ?>] = '<?= str_replace("'", "\'", "[".$orderID."] ".$arOrderProps["NAME"]." (".GetMessage("SPS_JCITY").")") ?>';
				<?
			}
		}
	}
					...
					<tr>
						...
						<td>
							...
							<?if(strlen($val["VALUE"]) > 0 && strlen($val["TYPE"]) > 0)
							{
								?>
								<script>
									PropertyTypeChange('<?=str_replace("'", "\'", htmlspecialcharsEx($key))?>', '<?=str_replace("'", "\'", htmlspecialcharsEx($val["VALUE"]))?>');
								</script>
								<?
							}
							?>
					...
					<tr>
						...
						<td>
							...
							<?if(strlen($val["VALUE"]) > 0)
							{
								?>
								<script>
									PropertyTypeChange('<?=$key?>', '<?=$val["VALUE"]?>');
								</script>
								<?
							}
							?>
