	...
	if (StrLen($errorMessage) <= 0)
	{
		...
		if ($affiliates_calculated <= 0)
		{
			...
			if (StrLen($DATE_CALC_TO) <= 0)
				$DATE_CALC_TO = date($DB->DateFormatToPHP(CSite::GetDateFormat("FULL")), time());
			if (StrLen($DATE_CALC_TO) > 0)
				$arFilterTmp["<ORDER_DATE_ALLOW_DELIVERY"] = $DATE_CALC_TO;
			if (StrLen($DATE_PLAN_TO) <= 0)
				$DATE_PLAN_TO = date($DB->DateFormatToPHP(CSite::GetDateFormat("FULL")), time());
			...
			while ($arAffiliates = $dbAffiliates->Fetch())
			{
				...
				if (!CSaleAffiliate::PayAffiliate($arAffiliates["ID"], $SUM_TODO, $paySum))
				{
					...
					if ($ex = $APPLICATION->GetException())
						...
						$errorMessageTmp .= GetMessage("SAC_ERROR_PAY_AFFILIATE");
				}
		...
		if (!$bAllAffiliatesCalc || !$bAllAffiliatesPay)
		{
			...
			for ($i = 0; $i < count($arAffiliateID); $i++)
...
?>
	...
	<input<?= ($saleModulePermissions >= "W") ? "" : " disabled" ?> type="submit" name="apply" value="<?echo GetMessage("SAC_DO_CALC")?>" title="<?echo GetMessage("SAC_DO_CALC_DESCR")?>">
