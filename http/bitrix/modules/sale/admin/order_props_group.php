...
{
	$row =& $lAdmin->AddRow($f_ID, $arPropsGroup);
	...
	if (in_array("PERSON_TYPE_ID", $arVisibleColumns))
	{
		...
		$fieldValue = "[".$arPersType["ID"]."] ".htmlspecialcharsEx($arPersType["NAME"])." (".htmlspecialcharsEx($arPersType["LID"]).")";
	}
	...
	$arActions = Array();
	$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("SOPGAN_EDIT_PROMT"), "ACTION"=>$lAdmin->ActionRedirect("sale_order_props_group_edit.php?ID=".$f_ID."&lang=".LANG.GetFilterParams("filter_").""), "DEFAULT"=>true);
