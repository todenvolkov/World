...
?>
		...
		<td width="60%">
			<select name="TYPE">
				...
				foreach ($SALE_FIELD_TYPES as $key => $value):
					?><option value="<?echo $key?>"<?if ($str_TYPE==$key) echo " selected"?>>[<?echo htmlspecialchars($key) ?>] <?echo htmlspecialchars($value) ?></option><?
				endforeach;
				?>
			</select>
		</td>
	...
	<tr>
		...
		<td width="60%" valign="top">
			<input type="checkbox" name="IS_LOCATION" value="Y" <?if ($str_IS_LOCATION=="Y") echo "checked"?><?=$disMulti?>><br>
	...
	<tr>
		<td colspan="2" align="center">
			<table cellspacing="0" class="internal">
				<tr class="heading">
					<td align="center"><span class="required">*</span><?echo GetMessage("SALE_VARIANTS_CODE")?></td>
					<td align="center"><span class="required">*</span><?echo GetMessage("SALE_VARIANTS_NAME")?></td>
			...
			while ($db_propsVars->ExtractFields("f_"))
			{
				...
				<tr>
					<td>
						...
						<input type="text" name="VALUE_<?echo $ind;?>" value="<?echo htmlspecialchars($f_VALUE);?>" size="5">
					</td>
					<td>
						<input type="text" name="NAME_<?echo $ind;?>" value="<?echo htmlspecialchars($f_NAME);?>" size="30">
					</td>
					<td>
						<input type="text" name="SORT_<?echo $ind;?>" value="<?echo IntVal($f_SORT);?>" size="3">
					</td>
					<td>
						<input type="text" name="DESCRIPTION_<?echo $ind;?>" value="<?echo htmlspecialchars($f_DESCRIPTION);?>" size="30">
					</td>
					<td>
