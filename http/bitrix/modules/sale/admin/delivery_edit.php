...
?>
	...
	<tr>
		<td width="40%"><span class="required">*</span><?echo GetMessage("F_LANG") ?>:</td>
		...
		<td width="60%" valign="top">
			<select name="LOCATION1[]" size="5" multiple>
				...
				<?while ($vars = $db_vars->Fetch()):?>
					<option value="<?echo $vars["ID"]?>"<?if (is_array($arLOCATION1) && in_array(IntVal($vars["ID"]), $arLOCATION1)) echo " selected"?>><?echo htmlspecialchars($vars["COUNTRY_NAME"]." - ".$vars["CITY_NAME"])?></option>
				<?endwhile;?>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td width="60%" valign="top">
			<select name="LOCATION2[]" size="5" multiple>
				...
				?>
				<?while ($vars = $db_vars->Fetch()):?>
					<option value="<?echo $vars["ID"]?>"<?if (is_array($arLOCATION2) && in_array(IntVal($vars["ID"]), $arLOCATION2)) echo " selected"?>><?echo htmlspecialchars($vars["NAME"])?></option>
				<?endwhile;?>
			</select>
		</td>
	</tr>
