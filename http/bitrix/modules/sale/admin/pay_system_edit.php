...
if (($ID>0) && ($arPaySystem = CSalePaySystem::GetByID($ID)))
{
	$LID = $arPaySystem["LID"];
}
elseif (strlen($LID)>0)
			...
			else
			{
				...
				if (strlen($errorMessage1) <= 0)
				{
					...
					$arFields = array(
							...
							"ENCODING" => ${"ENCODING_".$arPersonType["ID"]},
						);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/bitrix/admin/sale_pay_system_edit.php?lang=".LANG."&LID=".UrlEncode($LID).GetFilterParams("filter_"),
			"ICON" => "btn_new"
		);
...
function ActionFileChange(ind, exist)
{
	...
	window.frames["hidden_action_frame_" + ind].location.replace('/bitrix/admin/sale_pay_system_get.php?lang=<?= htmlspecialchars($lang) ?>&file='+escape(psAction)+'&divInd='+ind+'&exist='+exist);
}
...
$dbPersonType = CSalePersonType::GetList(array("SORT" => "ASC"), array("LID" => $LID));
while ($arPersonType = $dbPersonType->Fetch())
{
	$i++;
	$aTabs[] = array("DIV" => "edit".($i + 2), "TAB" => htmlspecialcharsEx($arPersonType["NAME"]), "ICON" => "sale", "TITLE" => str_replace("#PLTYPE#", htmlspecialcharsEx($arPersonType["NAME"]), GetMessage("SPSN_TAB_PAYSYS_DESCR_1")));
...
?>
	...
	for ($tpInd = 0; $tpInd < count($arPersonTypeList); $tpInd++)
	{
		...
		<tr>
			<td width="40%"><?= GetMessage("SPS_ACTION_ID") ?>:</td>
			<td width="60%">
		...
		<?endif;?>
		<tr>
			<td width="40%"><span class="required">*</span><?= GetMessage("SPS_NAME") ?>:</td>
			<td width="60%">
				<input type="text" name="NAME_<?= $arPersonType["ID"] ?>" value="<?= $f_NAME ?>" size="60">
			</td>
		</tr>
		<tr>
			<td width="40%" valign="top"><span class="required">*</span><?= GetMessage("SPS_ACT_FILE") ?>:</td>
			<td width="60%" valign="top">
				<?
				...
				<select name="ACTION_FILE_<?= $arPersonType["ID"] ?>" OnChange="ActionFileChange(<?= $arPersonType["ID"] ?>, 'Y')">
					...
					for ($i = 0; $i < count($arUserPSActions); $i++)
					{
						?><option value="<?= htmlspecialchars($arUserPSActions[$i]["PATH"]) ?>"<?if ($f_ACTION_FILE == $arUserPSActions[$i]["PATH"]) echo " selected";?>><?= htmlspecialcharsEx($arUserPSActions[$i]["TITLE"]) ?></option><?
					}
					?>
					...
					for ($i = 0; $i < count($arSystemPSActions); $i++)
					{
						?><option value="<?= htmlspecialchars($arSystemPSActions[$i]["PATH"]) ?>"<?if ($f_ACTION_FILE == $arSystemPSActions[$i]["PATH"]) echo " selected";?>><?= htmlspecialcharsEx($arSystemPSActions[$i]["TITLE"]) ?></option><?
					}
					?>
				</select>
		...
		<tr>
			<td width="40%"><?= GetMessage("SPS_NEW_WINDOW") ?>:</td>
			<td width="60%">
				<input type="checkbox" name="NEW_WINDOW_<?= $arPersonType["ID"] ?>" value="Y"<?if ($f_NEW_WINDOW == "Y") echo " checked";?>>
			</td>
		</tr>
		<tr>
			<td width="40%"><?= GetMessage("SPS_ENCODING") ?>:</td>
			<td width="60%">
				<input type="text" name="ENCODING_<?= $arPersonType["ID"] ?>" value="<?= $f_ENCODING ?>" size="20">
			</td>
		</tr>
		<tr>
			<td valign="top" align="center" colspan="2">
				...
				if ($bInitVars)
				{
					...
					for ($i = 0; $i < count($arActFields); $i++)
					{
						...
						if (strlen($typeTmp) <= 0)
							$valueTmp = ${"VALUE2_".$arActFields[$i]."_".$arPersonType["ID"]};
						?>
						var param_<?= $arActFields[$i] ?>_type_<?= $arPersonType["ID"] ?> = '<?= ${"TYPE_".$arActFields[$i]."_".$arPersonType["ID"]} ?>';
						var param_<?= $arActFields[$i] ?>_value_<?= $arPersonType["ID"] ?> = '<?= str_replace("'", "\'", $valueTmp) ?>';
						<?
					}
				}
				else
				{
					...
					{
						?>
						var param_<?= $key ?>_type_<?= $arPersonType["ID"] ?> = '<?= $value["TYPE"] ?>';
						var param_<?= $key ?>_value_<?= $arPersonType["ID"] ?> = '<?= str_replace("'", "\'", $value["VALUE"]) ?>';
						<?
					}
				}
				?>
				...
				<div id="pay_sys_act_<?= $arPersonType["ID"] ?>" style="display: block; background-color: #E4EDF3;">
				</div>
				<a href="javascript:ShowHideStatus(<?= $arPersonType["ID"] ?>);" id="pay_sys_switch_<?= $arPersonType["ID"] ?>"><br><?= GetMessage("SPS_HIDE_PROPS") ?></a><iframe style="width:0px; height:0px; border: 0px" name="hidden_action_frame_<?= $arPersonType["ID"] ?>" src="" width="0" height="0"></iframe>
			</td>
		</tr>
