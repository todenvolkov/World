...
if ($REQUEST_METHOD == "POST" && strlen($Print)>0 && check_bitrix_sessid() && $bUserCanViewOrder)
{
	if(count($REPORT_ID) > 0)
	{
		...
		{
			?>
			window.open('sale_print.php?doc=<?echo htmlspecialchars($REPORT_ID[$i]) ?>&ORDER_ID=<?echo $ID ?>&BASKET_IDS=<?echo urlencode($sBasket) ?>&QUANTITIES=<?echo urlencode($sQuantity) ?>', '_blank');
			<?
		}
		?>
		</script>
					...
					while ($arBasket = $db_basket->GetNext())
					{
						...
						<tr>
							...
							<td valign="top" align="right">
								<?echo SaleFormatCurrency(($arBasket["PRICE"]), $str_CURRENCY);?>
							</td>
							<td valign="top" align="right">
								<?echo SaleFormatCurrency(IntVal($arBasket["QUANTITY"])*DoubleVal($arBasket["PRICE"]), $str_CURRENCY);?>
							</td>
						</tr>
						<?
					}
		...
		while ($ar_tax_list = $db_tax_list->Fetch())
		{
			...
			<tr>
				...
				<td align="left" width="50%">
					<?echo SaleFormatCurrency($ar_tax_list["VALUE_MONEY"], $str_CURRENCY) ?>
				</td>
			</tr>
			<?
		}
