...
if ($bDoPayAction)
{
	...
	if (!isset($INPUT_CARD_NUM) || strlen($INPUT_CARD_NUM) <= 0)
		$strPaySysError .= "Please enter credit card number".".<br />";

	$INPUT_CARD_NUM = preg_replace("/[\D]+/", "", $INPUT_CARD_NUM);
	if (strlen($INPUT_CARD_NUM) <= 0)
		$strPaySysError .= "Please enter valid credit card number".".<br />";

	$INPUT_CARD_EXP_MONTH = IntVal($_REQUEST["ccard_date1"]);
	if ($INPUT_CARD_EXP_MONTH < 1 || $INPUT_CARD_EXP_MONTH > 12)
		$strPaySysError .= "Please enter valid credit card expiration month".".<br />";
	...
	if ($INPUT_CARD_EXP_YEAR < 2005)
		$strPaySysError .= "Please enter valid credit card expiration year".".<br />";
	...
	<table border="0" cellpadding="3" cellspacing="0" width="100%">
		<tr>
			...
			<td class="tablebody" width="60%">
				<input type="text" class="inputtext" name="ccard_num" size="30" value="<?= htmlspecialchars($_REQUEST["ccard_num"]) ?>">
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="tablebody" width="60%">
				<input type="text" class="inputtext" name="ccard_code" size="5" value="<?= htmlspecialchars($_REQUEST["ccard_code"]) ?>">
			</td>
		</tr>
	</table>
	<?
