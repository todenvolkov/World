  ...
  <tr>
	<td class="but2" align="center">
		...
		if (strlen($_POST["GetPayRes"]) > 0)
		{
			...
			$cvv2 = htmlspecialchars(Trim($_POST["cvv2"]));
			if (strlen($cvv2) <= 0)
				$strErrorTmp.= "Please fill in \"CVV2\" field. ";

			$cardexp1 = IntVal(htmlspecialchars($_POST["cardexp1"]));
			$cardexp2 = IntVal(htmlspecialchars($_POST["cardexp2"]));
			...
			$noc = htmlspecialchars(trim($_POST["noc"]));
			if (strlen($noc) <= 0)
		...
		else
		{
			...
			$noc_def = htmlspecialchars(CSalePaySystemAction::GetParamValue("NOC"));
			$address1_def = htmlspecialchars(CSalePaySystemAction::GetParamValue("ADDRESS"));
			$zipcode_def = htmlspecialchars(CSalePaySystemAction::GetParamValue("ZIP"));
			?>
			...
			<form onsubmit="return checkFields(this)" method="post" action="">
				...
				<table>
					<tr>
						...
						<td><input class="inputtext" maxlength="16" type="text" name="cardnum" value="<?= htmlspecialchars($cardnum) ?>" size="35"></td>
					</tr>
					<tr>
						<th><font class="text">CVV2</font></th>
						<td><input type="text" class="inputtext" maxlength="4" name="cvv2" value="<?= htmlspecialchars($cvv2) ?>" size="5"></td>
					</tr>
					<tr>
