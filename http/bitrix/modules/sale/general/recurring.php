...
class CAllSaleRecurring
{
		...
		if ($arProduct["WITHOUT_ORDER"] == "Y" || $arRecur["SUCCESS_PAYMENT"] == "Y")
		{
			...
			if ($arProduct["WITHOUT_ORDER"] == "N")
			{
				...
				if ($bSuccess)
				{
					...
					$event->Send("SALE_NEW_ORDER_RECURRING", $arOrder["LID"], $arFields, "N");
				}
		...
		if ($bSuccess)
		{
			...
			else
			{
				$arFields = array(
						...
						"RECUR_SCHEME_TYPE" => $arProduct["RECUR_SCHEME_TYPE"],
						"WITHOUT_ORDER" => $arProduct["WITHOUT_ORDER"],
						"NEXT_DATE" => Date($GLOBALS["DB"]->DateFormatToPHP(CLang::GetDateFormat("FULL", SITE_ID)), time() + SALE_PROC_REC_TIME),
	...
	function CancelRecurring($ID, $val, $description = "")
	{
		...
		$event->Send("SALE_RECURRING_CANCEL", $arOrder["LID"], $arFields, "N");
