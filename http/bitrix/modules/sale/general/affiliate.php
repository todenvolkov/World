...
class CAllSaleAffiliate
{
	...
	function Delete($ID)
	{
		...
		return $DB->Query("DELETE FROM b_sale_affiliate WHERE ID = ".$ID." ", true);
	}
	...
	function Calculate($dateFrom = false, $dateTo = false, $datePlanFrom = false, $datePlanTo = false)
	{
		...
		if (!$dateFrom || StrLen($dateFrom) <= 0)
		{
			if (!$dateTo || StrLen($dateTo) <= 0)
				$dateTo = date($DB->DateFormatToPHP(CSite::GetDateFormat("FULL")), time());
		...
		while ($arOrder = $dbOrders->Fetch())
		{
			...
			else
			{
				if ($arOrder["BASKET_MODULE"] == "catalog")
				{
					...
					$arProductSections = CCatalogProduct::GetProductSections($arOrder["BASKET_PRODUCT_ID"]);
				}
				else
				{
					$events = GetModuleEvents("sale", "OnAffiliateGetSections");
					if ($arEvent = $events->Fetch())
						$arProductSections = ExecuteModuleEvent($arEvent, $arOrder["BASKET_MODULE"], $arOrder["BASKET_PRODUCT_ID"]);
				}
			...
			if ($realRateType == "P")
			{
				if ($arOrder["CURRENCY"] != $affiliateCurrency)
				{
					...
					$affiliateSum += roundEx(($arOrder["BASKET_PRICE"] - $arOrder["BASKET_DISCOUNT_PRICE"]) * $arOrder["BASKET_QUANTITY"] * $GLOBALS["SALE_CONVERT_CURRENCY_CACHE"][$arOrder["CURRENCY"]."-".$affiliateCurrency] * $realRate / 100, SALE_VALUE_PRECISION);
				}
				else
				{
					$affiliateSum += roundEx(($arOrder["BASKET_PRICE"] - $arOrder["BASKET_DISCOUNT_PRICE"]) * $arOrder["BASKET_QUANTITY"] * $realRate / 100, SALE_VALUE_PRECISION);
				}
			}
			else
			{
