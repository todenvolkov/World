<?if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();?><form id="pay" name="pay" method="POST" action="https://merchant.webmoney.ru/lmi/payment.asp">
	...
	<input type="hidden" name="LMI_PAYMENT_DESC" value="Заказ <?= CSalePaySystemAction::GetParamValue("ORDER_ID") ?> от <?= htmlspecialchars(CSalePaySystemAction::GetParamValue("DATE_INSERT")) ?>">
	<input type="hidden" name="LMI_PAYMENT_NO" value="<?= CSalePaySystemAction::GetParamValue("ORDER_ID") ?>">
	<input type="hidden" name="LMI_PAYEE_PURSE" value="<?= htmlspecialchars(CSalePaySystemAction::GetParamValue("SHOP_ACCT")) ?>">
	<input type="hidden" name="LMI_SIM_MODE" value="<?= htmlspecialchars(CSalePaySystemAction::GetParamValue("TEST_MODE")) ?>">
