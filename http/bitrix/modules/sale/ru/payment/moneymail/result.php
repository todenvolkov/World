	...
	if($aResult[0] == "OK")
	{
		...
		if(intval($aFields["issuer_id"]) == $ORDER_ID)
		{
			...
			foreach($aFields as $key=>$val)
				$str_PS_RESPONSE_FORMATTED .= '<tr valign="top" class="tablebody"><td><font class="tablebodytext">'.htmlspecialchars($key).'</font></td><td><font class="tablebodytext">'.htmlspecialchars($val).'</font></td></tr>'."\n";
