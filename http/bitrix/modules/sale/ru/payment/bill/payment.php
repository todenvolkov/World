...
if ($arBasket = $dbBasket->Fetch()):
	...
	<tr>
		...
		<td align="right" bgcolor="#ffffff" style="border: 1pt solid #000000; border-top:none;"><?= SaleFormatCurrency($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"], $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"], true) ?></td>
	</tr>
</table>
<?endif?>
<p><b>Итого к оплате:</b> 
	...
	{
		echo Number2Word_Rus($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"]);
	}
	else
	{
		echo SaleFormatCurrency($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"], $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"]);
	}
	?></p>
<p>
...
if (strlen($stamp) > 0)
{
	...
	{
		list($width, $height, $type, $attr) = getimagesize($_SERVER["DOCUMENT_ROOT"].$stamp); 
