...
<input type="HIDDEN" name="sum" value="<?= htmlspecialchars($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SHOULD_PAY"]) ?>">
<input type=hidden name="ShopID" value="<?= htmlspecialchars(CSalePaySystemAction::GetParamValue("SHOP_ACCOUNT")) ?>">
<input type=hidden name="wbp_InactivityPeriod" value="2">
<input type=hidden name="wbp_ShopAddress" value="195.239.63.41:8128">
<input type=hidden name="wbp_ShopKeyID" value="<?= htmlspecialchars(CSalePaySystemAction::GetParamValue("SHOP_KEY_ID")) ?>">
<input type=hidden name="wbp_ShopEncryptionKey" value="<?= htmlspecialchars(CSalePaySystemAction::GetParamValue("SHOP_KEY")) ?>">
...
Заказ No <?= $ORDER_ID." от ".htmlspecialchars($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DATE_INSERT"]) ?>
</textarea><br>
<br>
