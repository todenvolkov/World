		...
		<td colspan="3">
					...
					<td style="border-bottom : 1px solid Black;">
						...
						else
						{
							if(strlen($arParams["BUYER_COMPANY"]) > 0)
								$userName = $arParams["BUYER_COMPANY"];
							else
			...
			if (count($arBasketIDs)>0)
			{
				...
				<table width="585" border="1" bordercolor="#000000" style="padding-left:10px" align="center" cellpadding="0" cellspacing="0">
					...
					{
						$ORDER_PRICE += DoubleVal($ar_basket["PRICE"])*IntVal($ar_basket["QUANTITY"]);
					}
					...
					for ($i = $start_i; $i < $end_i; $i++)
					{
						...
						if ($arQuantities[$i] > IntVal($arBasket["QUANTITY"]))
						{
							$arQuantities[$i] = IntVal($arBasket["QUANTITY"]);
						}
						...
						if ($i>=$curNum && $i<$curNum+$NUMPERPAGE)
						{
							...
							<tr>
								<td><?echo $i+1;?></td>
								<td><?echo htmlspecialchars($arBasket["NAME"]);?></td>
							...
							foreach ($arTaxList as $key => $val):
								...
								<tr>
									<td align="right" colspan="4">
										...
										echo htmlspecialchars($val["TAX_NAME"]); 
										if ($val["IS_PERCENT"]=="Y")
										{
