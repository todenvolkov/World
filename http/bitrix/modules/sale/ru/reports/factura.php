...
<table x:str border=0 cellpadding=0 cellspacing=0 width=987 style='border-collapse:
 ...
 <tr class=xl50 height=18 style='mso-height-source:userset;height:13.5pt'>
  ...
  <td class=xl64 colspan=10 style='mso-ignore:colspan'>
	...
	{
		?>
		<?=$arParams["COMPANY_NAME"]?>, <?=$arParams["BUYER_COUNTRY"]?>, <?=$arParams["BUYER_INDEX"]?>, г. <?=$arParams["BUYER_CITY"]?>, <?=$arParams["BUYER_ADDRESS"]?>
		<?
	}
	?>
</td>
 ...
 <tr class=xl50 height=18 style='mso-height-source:userset;height:13.5pt'>
  ...
  <td colspan=10 class=xl68 width=79 style='width:59pt'>
	<?if(empty($arParams))
	{	
...
for ($mi = 0; $mi < count($arBasketIDs); $mi++):
	...
	if ($arQuantities[$mi] > IntVal($arBasket["QUANTITY"]))
	{
		$arQuantities[$mi] = IntVal($arBasket["QUANTITY"]);
	}

	$b_AMOUNT = DoubleVal($arBasket["PRICE"])-DoubleVal($arBasket["DISCOUNT_PRICE"]);
...
*/
 ...
 <tr class=xl39>
  ...
  width:233pt'><?echo htmlspecialcharsEx($arBasket["NAME"]) ?></td>
  ...
  width:59pt' x:num><?echo SaleFormatCurrency($nds_val*$arQuantities[$mi], $arOrder["CURRENCY"], true); $total_nds += $nds_val;?></td>
  <td class=xl45 align=right width=83 style='border-top:none;border-left:none;
  width:62pt' x:num><?echo SaleFormatCurrency($item_price*$arQuantities[$mi]+$nds_val*$arQuantities[$mi], $arOrder["CURRENCY"], true); 
