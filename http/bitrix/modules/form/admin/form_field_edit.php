...
<input type="hidden" name="from_edit" value="<?echo htmlspecialchars($from_edit)?>" />
...
function addAnswer()
{
	...
	if (jsUtils.IsIE())
	{
		var input1 = document.createElement('<input type="hidden" name="ANSWER[]" />');
		var input2 = document.createElement('<input type="hidden" name="ANSWER_ID_' + (rows_count+1) + '" />');
	}
	else
	{
		...
		input2.type = 'hidden';
		input2.name = 'ANSWER_ID_' + (rows_count+1);
	}
	
	input1.value = rows_count + 1;
	input2.value = '0';
