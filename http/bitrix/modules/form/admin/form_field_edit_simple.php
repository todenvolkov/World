...
<input type="hidden" name="from_edit" value=<?=htmlspecialchars($from_edit)?> />
...
foreach ($arDisplayTypes as $e)
	echo "document.getElementById('type_".$e."').style.display=\"none\"\n";
?>
	multi = false;
	if (a == 'textarea')
		var b = a;
...
function RowInsert()
{
	...
	oLastSort = document.getElementById(oLastRow.id + '_sort');
	...
	oCell.innerHTML = '<input type=hidden name="ANSWER[]" value=0><input name="MESSAGE[]" type=text>\n'; 

	var oCell = oRow.insertCell(-1);
	oCell.innerHTML = '<input name="SORT[]" id=' + oRow.id + '_sort value=' + Sort +' type=text size=5>\n'; 

	var oCell = oRow.insertCell(-1);
	oCell.innerHTML = '<input name="DEF[]" type=hidden value="" id='+oRow.id+'_def_hidden><input OnClick="ClearAll(\'' + oRow.id + '\')" type=checkbox id='+ oRow.id +'_def>';

	var oCell = oRow.insertCell(-1);
	oCell.innerHTML = '<div OnClick=RowDelete("'+oRow.id+'",0) title="<?=GetMessage("FORM_ANS_DEL")?>" id=btn_delete style="width:20;height:20;cursor:pointer"></div>\n';
	oCell.id = oRow.id + '_del';
			...
			<div id="type_list">
				...
				if (is_array($arRows))
				{
					foreach($arRows AS $arRow):
						...
						<tr id="row<?=$i?>">
							...
							<td><input name="DEF[]" type="hidden" value="<?=$arRow['DEF_HIDDEN']?>" id="row<?=$i?>_def_hidden" /><input OnClick="ClearAll('row<?=$i?>')" type="checkbox" <?=$arRow['DEF']?> id="row<?=$i?>_def" /></td>
							<td id="row<?=$i?>_del" <?if(count($arRows)<2) print 'style="visibility:hidden"'?>><div OnClick="RowDelete('row<?=$i?>',1)" title="<?=GetMessage("FORM_ANS_DEL")?>" id="btn_delete" style="width:20;height:20;cursor:pointer;"></div></td>
						</tr>
						<?
					endforeach;
				}
				...
				?>
					<tr id="row1">
						...
						<td><input name="DEF[]" type="hidden" id="row1_def_hidden" /><input OnClick="ClearAll('row1')" type="checkbox" id="row1_def" /></td>
						<td id="row1_del" style="visibility:hidden"><div OnClick="RowDelete('row1',0)" title="<?=GetMessage("FORM_ANS_DEL")?>" id="btn_delete" style="width:20;height:20;cursor:pointer;"></div></td>
					</tr>
				<?
				}
				?>	
