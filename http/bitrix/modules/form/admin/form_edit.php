...
if ($copy_id > 0 && check_bitrix_sessid() && $F_RIGHT >= 30)
{
	...
	{
		LocalRedirect("form_edit.php?ID=".$new_id."&lang=".LANGUAGE_ID);
	}
}
...
if ($reset_id > 0 && check_bitrix_sessid() && $F_RIGHT >= 30)
{
	...
	LocalRedirect("form_edit.php?ID=".$reset_id."&lang=".LANGUAGE_ID);
}
...
if ((strlen($_REQUEST['save'])>0 || strlen($_REQUEST['apply'])>0) && $_SERVER['REQUEST_METHOD']=="POST" && ($F_RIGHT>=30 || $ID<=0) && check_bitrix_sessid())
{
	...
	if ($res>0)
	{
		if ($bEditTemplate && $USE_DEFAULT_TEMPLATE == "N")
		{
			...
			{
				CFormOutput::__saveTemplate($res, $FORM_TEMPLATE);
		...
		if ($bSimple)
		{
			...
			if ($USE_MAIL_TEMPLATE && count($arr['reference_id']) == 0)
					CForm::SetMailTemplate($res, "Y");
			elseif (!$USE_MAIL_TEMPLATE && count($arr['reference_id']) > 0)
			{
...
//			echo "<pre>"; print_r($arFields_status); echo "</pre>"; die();
		...
		if (strlen($strError)<=0)
		{
			if (strlen($save)>0)
			{
				...
				else LocalRedirect("form_list.php?lang=".LANGUAGE_ID);

			}
			else LocalRedirect("form_edit.php?ID=".$ID."&lang=".LANGUAGE_ID."&".$tabControl->ActiveTabParam().(!empty($_REQUEST["back_url"]) ? "&back_url=".urlencode($_REQUEST["back_url"]) : ""));
...
//echo "<pre>"; print_r($rsForm); echo "</pre>";
...
?>
<form name="form1" method="POST" action="" enctype="multipart/form-data" <?if ($bEditTemplate):?> onsubmit="return formSubmit();"<?endif;?>>
<?=bitrix_sessid_post()?>
...
__arr_api_methods_params_captions = {
	CSS_STYLE:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_CSS_STYLE');?>',
	ALIGN:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_ALIGN');?>',
	MAX_HEIGHT:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_MAX_HEIGHT');?>',
	MAX_WIDTH:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_MAX_WIDTH');?>',
	ENLARGE_SHOW:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_ENLARGE_SHOW');?>',
	ENLARGE_TITLE:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_ENLARGE_TITLE');?>',
	HSPACE:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_HSPACE');?>',
	VSPACE:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_VSPACE');?>',
	BORDER:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_BORDER');?>',
	FIELD_SID:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_FIELD_SID');?>',
	CAPTION:'<?=GetMessage('FORM_API_PARAMS_CAPTIONS_CAPTION');?>',
	NOT_SHOW_CSS: '<?=GetMessage('FORM_API_PARAMS_CAPTIONS_NOT_SHOW_CSS')?>'
}

var __arr_api_methods_title = ['<?=GetMessage('F_API_SHOWFORMTITLE')?>', '<?=GetMessage('F_API_SHOWFORMDESCRIPTION')?>', '<?=GetMessage('F_API_SHOWFORMERRORS')?>', '<?=GetMessage('F_API_SHOWFORMNOTE')?>', '<?=GetMessage('F_API_SHOWFORMIMAGE')?>', '<?=GetMessage('F_API_SHOWINPUTCAPTION')?>', '<?=GetMessage('F_API_SHOWREQUIRED')?>', '<?=GetMessage('F_API_SHOWDATEFORMAT')?>', '<?=GetMessage('F_API_SHOWINPUTCAPTIONIMAGE')?>', '<?=GetMessage('F_API_SHOWCAPTCHA')?>', '<?=GetMessage('F_API_SHOWCAPTCHAFIELD')?>', '<?=GetMessage('F_API_SHOWCAPTCHAIMAGE')?>', '<?=GetMessage('F_API_SHOWSUBMITBUTTON')?>', '<?=GetMessage('F_API_SHOWAPPLYBUTTON')?>', '<?=GetMessage('F_API_SHOWRESETBUTTON')?>', '<?=GetMessage('F_API_SHOWRESULTSTATUS')?>', '<?=GetMessage('F_API_SHOWRESULTSTATUSFORM')?>'];

var __arr_field_titles = {FIELD_SID: '<?=GetMessage('FORM_TITLE_FIELD_SID');?>', CAPTION_UNFORM:'<?=GetMessage('FORM_TITLE_FIELD_CAPTION');?>', isHTMLCaption:'<?=GetMessage('FORM_TITLE_FIELD_CAPTION_HTML');?>', isRequired:'<?=GetMessage('FORM_TITLE_FIELD_REQUIRED');?>', type:'<?=GetMessage('FORM_TITLE_FIELD_TYPE');?>', structure:'<?=GetMessage('FORM_TITLE_FIELD_STRUCTURE');?>', inResultsTable:'<?=GetMessage('FORM_TITLE_FIELD_IN_RESULTS_TABLE');?>', inExcelTable:'<?=GetMessage('FORM_TITLE_FIELD_IN_EXCEL_TABLE');?>'};

var oForm = new CFormInfo(arrInputObjects);

var __arr_messages = {
	FORM_TASKBAR_CFORM: '<?=GetMessage('FORM_TASKBAR_CFORM')?>',
	FORM_TASKBAR_CFORMOUTPUT: '<?=GetMessage('FORM_TASKBAR_CFORMOUTPUT')?>',
	FORM_TASKBAR_API: '<?=GetMessage('FORM_TASKBAR_API')?>',
	FORM_METHOD_HAS_NO_PARAMS: '<?=GetMessage('FORM_METHOD_HAS_NO_PARAMS');?>',
	FORM_FIELD_WIDTH_VAL: '<?=GetMessage("FORM_FIELD_WIDTH_VAL")?>',
	FORM_FIELD_HEIGHT_VAL: '<?=GetMessage("FORM_FIELD_HEIGHT_VAL")?>',
	FORM_ANSWER_VAL: '<?=GetMessage("FORM_ANSWER_VAL")?>',
	FORM_SORT_VAL: '<?=GetMessage("FORM_SORT_VAL")?>',
	FORM_DEF_VAL: '<?=GetMessage("FORM_DEF_VAL")?>',
	FORM_FIELD_DEF_VAL: '<?=GetMessage("FORM_FIELD_DEF_VAL")?>',
	FORM_FIELD_SIZE_VAL: '<?=GetMessage("FORM_FIELD_SIZE_VAL")?>',
	FORM_FIELD_MULTIPLE_WARNING: '<?=GetMessage("FORM_FIELD_MULTIPLE_WARNING")?>'
}
...
?>
	<tr>
		<td colspan="2">
			<input type="radio" id="USE_DEFAULT_TEMPLATE_Y" name="USE_DEFAULT_TEMPLATE" value="Y" <?=$str_USE_DEFAULT_TEMPLATE == "Y" ? "CHECKED" : ""?> onClick="document.getElementById('form_tpl_editor').style.display='none'" /> <label for="USE_DEFAULT_TEMPLATE_Y"><?=GetMessage("FORM_USE_DEFAULT_TEMPLATE")?></label><br />
			<input type="radio" id="USE_DEFAULT_TEMPLATE_N" name="USE_DEFAULT_TEMPLATE" value="N" <?=$str_USE_DEFAULT_TEMPLATE == "N" ? "CHECKED" : ""?> onClick="document.getElementById('form_tpl_editor').style.display='block'" /> <label for="USE_DEFAULT_TEMPLATE_N"><?=GetMessage("FORM_USE_CUSTOM_TEMPLATE")?></label>
		</td>
	</tr>
	<tr>
...
oBXEditorUtils.addTaskBar('BXFormElementsTaskbar', 2, "<?=GetMessage('FORM_TASKBARSET_TITLE')?>", []);
if (window.arButtons['Optimize'])
	...
	<?else:?>
	<tr>
		<td align="center" colspan="2"><?echo InputType("radio","FORM_TEMPLATE_TYPE","text",$str_DESCRIPTION_TYPE,false)?>&nbsp;<?echo GetMessage("FORM_TEXT")?>/&nbsp;<? echo InputType("radio","FORM_TEMPLATE_TYPE","html",$str_DESCRIPTION_TYPE,false)?>HTML</td>
	</tr>
	<tr>
		<td align="center" colspan="2"><textarea name="FORM_TEMPLATE" style="width:100%" rows="23"><?echo $str_FORM_TEMPLATE?></textarea></td>
	</tr>
	<?endif;?>
<?
endif;
...
?>
	...
	//echo "<tr><td colspan=2>"; print_r($arrSelect); echo "</td></tr>";
	reset($arGroups);
