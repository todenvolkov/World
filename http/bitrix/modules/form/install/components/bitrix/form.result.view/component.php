...
if (CModule::IncludeModule("form"))
{
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
		...
		foreach ($arResult["arQuestions"] as $arQuestion)
		{
			...
			{
				//reset($arResult["RESULT"][$FIELD_SID]["ANSWER_VALUE"]);
				$count = count($arResult["RESULT"][$FIELD_SID]["ANSWER_VALUE"]);
				...
				foreach ($arResult["RESULT"][$FIELD_SID]["ANSWER_VALUE"] as $key => $arrA)
				{
					...
					{
						$arResultAnswer["USER_TEXT"] = $DB->FormatDate($arrA["USER_DATE"], FORMAT_DATETIME, FORMAT_DATE);
					}					
					...
					if (intval($arrA["USER_FILE_ID"])>0) 
					{
						if ($arrA["USER_FILE_IS_IMAGE"]=="Y") 
						{
							...
							$arResultAnswer["ANSWER_IMAGE"]["URL"] = CFile::GetPath($arrA["USER_FILE_ID"]);
							
							if (
								file_exists($_SERVER["DOCUMENT_ROOT"].$arResultAnswer["ANSWER_IMAGE"]["URL"]) 
								&& 
								list(
								...
								) = @getimagesize($_SERVER["DOCUMENT_ROOT"].$arResultAnswer["ANSWER_IMAGE"]["URL"])
							)
							{
						...
						else
						{
							...
							$arResultAnswer["ANSWER_FILE"]["SIZE_FORMATTED"] = round($size,2)." ".$a[$pos];
			...
			if (intval($arResult["arQuestions"][$FIELD_SID]["IMAGE_ID"])>0)
			{
				...
				$arResult["QUESTIONS"][$FIELD_SID]["IMAGE"]["URL"] = CFile::GetPath($arResult["arQuestions"][$FIELD_SID]["IMAGE_ID"]); // image url
