...
class CAllForm extends CForm_old
{
	...
	function GetDateValue($FIELD_NAME, $arAnswer, $arrVALUES=false)
	{
		...
		else
		{
			...
			elseif (preg_match("/NOW_TIME/i",$arAnswer["FIELD_PARAM"])) $value = GetTime(time(),"FULL");
			else $value = $arAnswer["VALUE"];
		}
		return $value;
	}
	...
	function Set($arFields, $FORM_ID=false, $CHECK_RIGHTS="Y")
	{
		...
		if (CForm::CheckFields($arFields, $FORM_ID, $CHECK_RIGHTS))
		{
			...
			$arFields_i["TIMESTAMP_X"] = date($DB->DateFormatToPHP(CSite::GetDateFormat("FULL")), time());
...
".GetMessage("FORM_GENERATED_AUTOMATICALLY")."
	...
	function GetByID_admin($WEB_FORM_ID, $current_section = false)
	{
		...
		if ($arForm = $dbForm->Fetch())
		{
			...
			$arForm['ADMIN_MENU'][] = array(
				"ICON"	=> $current_section == 'result' ? 'btn_current' : '',
			...
			$arForm['ADMIN_MENU'][] = array(
				"ICON"	=> $current_section == 'question' ? 'btn_current' : '',
			...
			{
				$arForm['ADMIN_MENU'][] = array(
					"ICON"	=> $current_section == 'field' ? 'btn_current' : '',
				...
				$arForm['ADMIN_MENU'][] = array(
					"ICON"	=> $current_section == 'status' ? 'btn_current' : '',
