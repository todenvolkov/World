...
function fill_all_values($sid, $type, $mess=false, $site=false)
{
	...
	while ($zr = $z->Fetch())
	{
		$arrTickets[$sid][$zr["ID"]]["NAME"] = "[<a title='".GetMessage("MAIN_ADMIN_MENU_EDIT")."' href='/bitrix/admin/ticket_dict_edit.php?ID=".$zr["ID"]."'>".$zr["ID"]."</a>] ".htmlspecialchars($zr["NAME"]);
...
while ($arTicket = $rsTickets->Fetch())
{
	...
	if (intval($arTicket["RESPONSIBLE_USER_ID"])>0)
	{
		...
		$R_NAME = "[<a href='/bitrix/admin/user_edit.php?ID=".$arTicket["RESPONSIBLE_USER_ID"]."'>".$arTicket["RESPONSIBLE_USER_ID"]."</a>] (".htmlspecialchars($arUser["LOGIN"]).") ".htmlspecialchars($arUser["NAME"])." ".htmlspecialchars($arUser["LAST_NAME"]);
	}
	else $R_NAME = "(".GetMessage("SUP_NO").")";
	$arrT["RESPONSIBLE"][intval($arTicket["RESPONSIBLE_USER_ID"])]["NAME"] = $R_NAME;
...
<table border="0" cellspacing="1" cellpadding="3" width="100%" class="list-table">
	...
	while (list($id, $arr) = each($arrR)):
		if (intval($CLOSE_TICKETS)>0) $procent = round(($counter*100)/$CLOSE_TICKETS,2);
		else $procent = 0;
	?>
	<tr valign="top">
		<td><?=$arr["NAME"]?></td>
		
		<td align="right">&nbsp;<a title="<?echo ($OPEN_TICKETS>0?round(($arr["COUNTER_OPEN_RED"]*100)/$OPEN_TICKETS,2):"0")."% ".GetMessage("SUP_FROM_OPEN_TICKETS")?>" href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&find_close=N&find_lamp[]=red&find_lamp[]=yellow&set_filter=Y"><?=(intval($arr["COUNTER_OPEN_RED"])>0) ? $arr["COUNTER_OPEN_RED"] : ""?></a></td>
		
		<td align="right">&nbsp;<a title="<?echo ($OPEN_TICKETS>0?round(($arr["COUNTER_OPEN_GREEN"]*100)/$OPEN_TICKETS,2):"0")."% ".GetMessage("SUP_FROM_OPEN_TICKETS")?>" href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&find_close=N&find_lamp[]=green&find_lamp[]=green_s&set_filter=Y"><?=intval($arr["COUNTER_OPEN_GREEN"])>0 ? $arr["COUNTER_OPEN_GREEN"] : ""?></a></td>
		

		<td align="right">&nbsp;<a title="<?echo ($CLOSE_TICKETS>0 ? round(($arr["COUNTER_CLOSE"]*100)/$CLOSE_TICKETS,2) : "0")."% ".GetMessage("SUP_FROM_CLOSE_TICKETS")?>" href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&find_close=Y&set_filter=Y"><?=intval($arr["COUNTER_CLOSE"])>0 ? $arr["COUNTER_CLOSE"] : ""?></a></td>
		...
		<td align="right"><span title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>">&nbsp;<?
			if (intval($arr["OVERDUE_MESSAGES_CLOSE"])>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>"  href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&find_overdue_messages1=1&find_close=Y&set_filter=Y" ><?=$arr["OVERDUE_MESSAGES_CLOSE"]?></a><?
			endif;
			?></span></td>
		<?endif;?>

		<td align="right">&nbsp;<a title="<?echo ($OPEN_TICKETS>0?round(($arr["COUNTER_OPEN"]*100)/$OPEN_TICKETS,2):"0")."% ".GetMessage("SUP_FROM_OPEN_TICKETS")?>" href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&find_close=N&set_filter=Y"><?=intval($arr["COUNTER_OPEN"])>0 ? $arr["COUNTER_OPEN"] : ""?></a></td>
		...
		<td align="right">&nbsp;<?
			if (intval($arr["OVERDUE_MESSAGES_OPEN"])>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>"  href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&find_overdue_messages1=1&find_close=N&set_filter=Y" ><?=$arr["OVERDUE_MESSAGES_OPEN"]?></a><?
			endif;
			?></td>
		<?endif;?>


		<td align="right">&nbsp;<a title="<?=(($OPEN_TICKETS+$CLOSE_TICKETS)>0 ? round((($arr["COUNTER_OPEN"]+$arr["COUNTER_CLOSE"])*100)/($OPEN_TICKETS+$CLOSE_TICKETS),2): "0")."% ".GetMessage("SUP_FROM_ALL_TICKETS")?>" href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&set_filter=Y"><?=(intval($arr["COUNTER_OPEN"]+$arr["COUNTER_CLOSE"])>0) ? $arr["COUNTER_OPEN"]+$arr["COUNTER_CLOSE"] : ""?></a></td>
		...
		<td align="right">&nbsp;<?
			if (intval($arr["OVERDUE_MESSAGES_OPEN"]+$arr["OVERDUE_MESSAGES_CLOSE"])>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>"  href="ticket_list.php?lang=<?=LANGUAGE_ID?>&find_<?=strtolower($key)?>_id=<?=$id?>&find_overdue_messages1=1&set_filter=Y" ><?=$arr["OVERDUE_MESSAGES_OPEN"]+$arr["OVERDUE_MESSAGES_CLOSE"]?></a><?
			endif;
			?></td>
		<?endif;?>
	</tr>
	...
	<tr valign="top" class="head">
		...
		<td align="right">&nbsp;<?if (intval($OPEN_TICKETS_RED)>0):?><a href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_lamp[]=red&find_lamp[]=yellow&set_filter=Y"><?echo intval($OPEN_TICKETS_RED)?></a><?endif;?></td>

		<td align="right">&nbsp;<?if(intval($OPEN_TICKETS_GREEN)>0):?><a href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_lamp[]=green&find_lamp[]=green_s&set_filter=Y"><?echo intval($OPEN_TICKETS_GREEN)?></a><?endif;?></td>



		<td align="right">&nbsp;<?if(intval($CLOSE_TICKETS)>0):?><a href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_close=Y&set_filter=Y"><?=$CLOSE_TICKETS?></a><?endif;?></td>
		...
		<td align="right">&nbsp;<?
			if (intval($CLOSE_OVERDUE_MESSAGES)>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>" href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_overdue_messages1=1&find_close=Y&set_filter=Y" ><?=intval($CLOSE_OVERDUE_MESSAGES)?></a><?
			endif;
			?></td>
		<?endif;?>

		<td align="right">&nbsp;<?if(intval($OPEN_TICKETS)>0):?><a href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_close=N&set_filter=Y"><?=$OPEN_TICKETS?></a><?endif;?></td>
		...
		<td align="right">&nbsp;<?
			if(intval($OPEN_OVERDUE_MESSAGES)>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>" href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_overdue_messages1=1&find_close=N&set_filter=Y" ><?=$OPEN_OVERDUE_MESSAGES?></a><?
			endif;
			?></td>
		<?endif;?>


		<td align="right">&nbsp;<?if (intval($OPEN_TICKETS+$CLOSE_TICKETS)>0):?><a href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&del_filter=Y"><?=$OPEN_TICKETS+$CLOSE_TICKETS?></a><?endif;?></td>
		...
		<td align="right">&nbsp;<?
			if (intval($OPEN_OVERDUE_MESSAGES+$CLOSE_OVERDUE_MESSAGES)>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>" href="/bitrix/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_overdue_messages1=1&set_filter=Y" ><?=intval($OPEN_OVERDUE_MESSAGES+$CLOSE_OVERDUE_MESSAGES)?></a><?
			endif;
			?></td>
		<?endif;?>
	</tr>
...
<?$filter->Begin();?>
<tr valign="center">
	<td width="0%" nowrap><?echo GetMessage("SUP_F_PERIOD")." (".CLang::GetDateFormat("SHORT")."):"?></td>
...
<tr> 
	...
	<td><?
		if ($bAdmin=="Y" || $bDemo=="Y")	:
			...
			?><br><input type="text" name="find_responsible" size="47" value="<?=htmlspecialchars($find_responsible)?>"><?=InputType("checkbox", "find_responsible_exact_match", "Y", $find_responsible_exact_match, false, "", "title='".GetMessage("SUP_EXACT_MATCH")."'")?>&nbsp;<?=ShowFilterLogicHelp()?><?
		else : 
