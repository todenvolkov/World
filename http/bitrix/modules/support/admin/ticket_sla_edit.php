...
if ((strlen($save)>0 || strlen($apply)>0) && $bAdmin=="Y" && $_SERVER["REQUEST_METHOD"]=="POST" && check_bitrix_sessid())
{
	...
	if ($ID = CTicketSLA::Set($arFields, $ID))
	{
		...
		//{
			if (strlen($save)>0) LocalRedirect($LIST_URL."?lang=".LANGUAGE_ID); 
			else LocalRedirect($EDIT_URL."?ID=".$ID."&lang=".LANGUAGE_ID."&tabControl_active_tab=".urlencode($tabControl_active_tab));
		}
	}
...
while(list($sid, $arrS) = each($arrSites)):
	...
	?>
		arCategory["<?=$sid?>"]=Array(<?
			echo "0"; 
			foreach($arr as $ar) echo ",".addslashes(htmlspecialchars($ar["REFERENCE_ID"]));
			?>);
			<?
	endif;	
...
<?
	...
	$aTabs[] = array("DIV" => "edit4", "TAB" => GetMessage("SUP_WHAT_ACCESSIBLE_S"), "ICON"=>"ticket_edit", "TITLE"=> GetMessage("SUP_WHAT_ACCESSIBLE"));
...
?>
	...
	<tr valign="top"> 
		...
		if (intval($str_CREATED_USER_ID)>0) :
			...
			echo "&nbsp;&nbsp;[<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/bitrix/admin/user_edit.php?ID=".$str_CREATED_USER_ID."\">".$str_CREATED_USER_ID."</a>]&nbsp;(".htmlspecialchars($arUser["LOGIN"]).") ".htmlspecialchars($arUser["NAME"])." ".htmlspecialchars($arUser["LAST_NAME"]);
		endif;
	...
	<tr valign="top"> 
		...
		if (intval($str_MODIFIED_USER_ID)>0) :
			...
			echo "&nbsp;&nbsp;[<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/bitrix/admin/user_edit.php?ID=".$str_MODIFIED_USER_ID."\">".$str_MODIFIED_USER_ID."</a>]&nbsp;(".htmlspecialchars($arUser["LOGIN"]).") ".htmlspecialchars($arUser["NAME"])." ".htmlspecialchars($arUser["LAST_NAME"]);
		endif;
	...
	<tr valign="top"> 
		<td width="40%" align="right"><span class="required">*</span><?=GetMessage("SUP_NAME")?>:</td>
		<td width="60%"><input type="text" maxlength="255" name="NAME" size="50" value="<?=$str_NAME?>"></td>
	</tr>

	<tr valign="top"> 
		<td align="right"><?=GetMessage("SUP_SITE")?>:</td>
		<td><input name="arSITES[]" type="checkbox" value="ALL" onclick="OnSelectAll(this.checked, 'arSITES[]')" <?if ((is_array($arSITES) && (in_array('ALL', $arSITES) || count($arSITES) <= 0)) || !is_array($arSITES)) echo "checked"?>>&nbsp;(<?=GetMessage("SUP_ALL_CURRENT_FUTURE")?>)<br><?
		reset($arrSites);
		while(list($sid, $arrS) = each($arrSites)):
			$checked = ((is_array($arSITES) && in_array($sid, $arSITES)) || ($ID<=0 && $def_site_id==$sid)) ? "checked" : "";
			?>
			<input onClick="OnSiteClick()" type="checkbox" name="arSITES[]" value="<?=htmlspecialcharsex($sid)?>" id="<?=htmlspecialcharsex($sid)?>" class="typecheckbox" <?=$checked?> <?=$disabled?>> 
			<label for="<?=htmlspecialchars($sid)?>"><?echo '[<a title="'.GetMessage("MAIN_ADMIN_MENU_EDIT").'" href="/bitrix/admin/site_edit.php?LID='.htmlspecialchars($sid).'&lang='.LANGUAGE_ID.'">'.htmlspecialcharsex($sid).'</a>]&nbsp;'.htmlspecialcharsex($arrS["NAME"])?></label>
			<br>
			<?
		endwhile;
		?></td>
	...
	<tr>
		<td align="right" valign="top"><?=GetMessage("SUP_USER_GROUPS")?>:</td>
		<td><?
...
<?$tabControl->BeginNextTab();?>
				...
				<tr valign="top">
					...
					<td align="center" nowrap>
						<table border="0" cellspacing="0" cellpadding="0" width="100%" id="table<?=$i?>">
							...
							?>
							<tr>
								<td><input type="text" id="MINUTE_FROM_<?=$i?>_<?=$j?>" name="MINUTE_FROM_<?=$i?>_<?=$j?>" size="5" value="<?=htmlspecialchars($arTime["FROM"])?>"></td>
								<td align="center" valign="middle" nowrap><nobr>&nbsp;-&nbsp;</nobr></td>
								<td><input type="text" name="MINUTE_TILL_<?=$i?>_<?=$j?>" id="MINUTE_TILL_<?=$i?>_<?=$j?>" size="5" value="<?=htmlspecialchars($arTime["TILL"])?>"></td>
							...
							?>
							<tr>
								<td><input type="text" name="MINUTE_FROM_<?=$i?>_<?=$j?>" id="MINUTE_FROM_<?=$i?>_<?=$j?>" size="5" value="<?=htmlspecialchars(${"MINUTE_FROM_".$i."_".$j})?>"></td>
								<td align="center" valign="middle" nowrap><nobr>&nbsp;-&nbsp;</nobr></td>
								<td><input type="text" name="MINUTE_TILL_<?=$i?>_<?=$j?>" id="MINUTE_TILL_<?=$i?>_<?=$j?>" size="5" value="<?=htmlspecialchars(${"MINUTE_TILL_".$i."_".$j})?>"></td>
								<td><a title="<?=GetMessage("MAIN_ADMIN_MENU_COPY")?>" href="javascript: Copy(<?=$i?>, <?=$j?>)"><img src="/bitrix/images/support/copy.gif" width="15" height="15" border=0 hspace="2" alt="<?=GetMessage("MAIN_ADMIN_MENU_COPY")?>"></a></td>
							</tr>
							<?
								$j++;
	...
	<?$tabControl->BeginNextTab();?>
			...
			<tr>
				<td valign="top"><?=GetMessage("SUP_CATEGORY")?></td>
				<td nowrap><div id="category_0"><input name="arCATEGORIES[]" type="checkbox" value="0" onclick="OnSelectAll(this.checked, 'arCATEGORIES[]')" <?if ((is_array($arCATEGORIES) && (in_array(0, $arCATEGORIES) || count($arCATEGORIES) <= 0)) || !is_array($arCATEGORIES)) echo "checked"?>>&nbsp;(<?=GetMessage("SUP_ALL_CURRENT_FUTURE")?>)<br></div><?
					foreach($arCategory as $ar):
						?><div id="category_<?=$ar["ID"]?>"><?echo InputType("checkbox", "arCATEGORIES[]", $ar["ID"], $arCATEGORIES)." ".htmlspecialchars($ar["NAME"])." [<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/bitrix/admin/ticket_dict_edit.php?ID=".intval($ar["ID"])."&find_type=C&lang=".LANGUAGE_ID."\">".intval($ar["ID"])."</a>]<br>";										
						?></div><?
					endforeach;
				?></td>
			</tr>
			<tr>
				...
				<td nowrap><div id="criticality_0"><input name="arCRITICALITIES[]" type="checkbox" value="0" onclick="OnSelectAll(this.checked, 'arCRITICALITIES[]')" <?if ((is_array($arCRITICALITIES) && (in_array(0, $arCRITICALITIES) || count($arCRITICALITIES) <= 0)) || !is_array($arCRITICALITIES)) echo "checked"?>>&nbsp;(<?=GetMessage("SUP_ALL_CURRENT_FUTURE")?>)<br></div><?
					foreach($arCriticality as $ar):
						?><div id="criticality_<?=$ar["ID"]?>"><?echo InputType("checkbox", "arCRITICALITIES[]", $ar["ID"], $arCRITICALITIES)." ".htmlspecialchars($ar["NAME"])." [<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/bitrix/admin/ticket_dict_edit.php?ID=".intval($ar["ID"])."&find_type=C&lang=".LANGUAGE_ID."\">".intval($ar["ID"])."</a>]<br>";
						?></div><?
					endforeach;
				?></td>
			</tr>
			<tr>
				...
				<td nowrap><div id="mark_0"><input name="arMARKS[]" type="checkbox" value="0" onclick="OnSelectAll(this.checked, 'arMARKS[]')" <?if ((is_array($arMARKS) && (in_array(0, $arMARKS) || count($arMARKS) <= 0)) || !is_array($arMARKS)) echo "checked"?>>&nbsp;(<?=GetMessage("SUP_ALL_CURRENT_FUTURE")?>)<br></div><?

					foreach($arMark as $ar):
						?><div id="mark_<?=$ar["ID"]?>"><?echo InputType("checkbox", "arMARKS[]", $ar["ID"], $arMARKS)." ".htmlspecialchars($ar["NAME"])." [<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/bitrix/admin/ticket_dict_edit.php?ID=".intval($ar["ID"])."&find_type=C&lang=".LANGUAGE_ID."\">".intval($ar["ID"])."</a>]<br>";										
						?></div><?
					endforeach;
				?></td>
	</tr>
...
<?echo BeginNote();?>
<span class="required">*</span> - <?=GetMessage("SUP_REQUIRED_FIELD_ALT")?>
<?echo EndNote();?>
