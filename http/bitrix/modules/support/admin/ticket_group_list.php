...
while ($arGroup = $rsData->GetNext())
{
	...
	/*
	$arActions[] = array(
		'TEXT'=>GetMessage('SUP_GL_USERADD'),
		'ACTION'=>$lAdmin->ActionRedirect('/bitrix/admin/ticket_usergroup_edit.php?lang='.LANGUAGE_ID.'&GROUP_ID='.$arGroup['ID'].'&back_url='.urlencode($APPLICATION->GetCurPageParam('', array('mode', 'table_id'))))
	);
	$arActions[] = array(
		'TEXT'=>GetMessage('SUP_GL_USERLIST'),
		'ACTION'=>$lAdmin->ActionRedirect('/bitrix/admin/ticket_usergroup_list.php?lang='.LANGUAGE_ID.'&set_filter=Y&FIND_GROUP_ID='.$arGroup['ID'])
	);
...
<tr> 
	...
	<td><input type="text" name="FIND_NAME" size="47" value="<?=htmlspecialchars($FIND_NAME)?>"><?=InputType("checkbox", "FIND_NAME_EXACT_MATCH", "Y", $FIND_NAME_EXACT_MATCH, false, "", "title='".GetMessage('SUP_GL_EXACT_MATCH')."'")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr> 
	<td>ID:</td>
