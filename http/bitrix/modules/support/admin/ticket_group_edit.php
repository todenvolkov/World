...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=='POST' && $bAdmin && check_bitrix_sessid())
{
	...
	if ($bOK && isset($_POST[$FMUTagName]['VALS']) && is_array($_POST[$FMUTagName]['VALS']))
	{
		...
		foreach ($UIDS as $k => $v)
		{
			$USERS[] = array('USER_ID' => $v, 'CAN_VIEW_GROUP_MESSAGES' => $_POST[$FMUTagName]['CHECKS'][$k], 'CAN_MAIL_GROUP_MESSAGES' => $_POST[$FMUTagName]['MAIL'][$k]);
		}
...
if ($ID > 0)
{
	$aMenu[] = 	array(
		'ICON'	=> 'btn_edit',
		'TEXT'	=> GetMessage('SUP_GE_USERGROUPS_ADD'), 
		'LINK'	=> '/bitrix/admin/ticket_usergroup_edit.php?lang='.LANG.'&GROUP_ID='.$ID,
	);
	$aMenu[] = 	array(
		'ICON'	=> 'btn_list',
		'TEXT'	=> GetMessage('SUP_GE_USERGROUPS_LIST'), 
		'LINK'	=> '/bitrix/admin/ticket_usergroup_list.php?lang='.LANG.'&GROUP_ID='.$ID,
	);
}
*/
...
<tr valign="top"> 
	<td align="right" width="40%"><span class="required">*</span><?=GetMessage('SUP_GE_NAME')?>:</td>
...
<tr valign="top"> 
	...
	<td>
		...
		<td><input type="checkbox" name="<?=$FMUTagName?>[CHECKS][<?=$i?>]"<?if ($val['CAN_VIEW_GROUP_MESSAGES'] == "Y" || strlen($val['USER_ID']) <= 0){?> checked<?}?> value="Y"></td>
		<td><input type="checkbox" name="<?=$FMUTagName?>[MAIL][<?=$i?>]"<?if ($val['CAN_MAIL_GROUP_MESSAGES'] == "Y" || strlen($val['USER_ID']) <= 0){?> checked<?}?> value="Y"></td>
		</tr>
		<?
			$i++;
		}
