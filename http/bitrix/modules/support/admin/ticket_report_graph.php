...
{
	if ($arTicket["DATE_CREATE_SHORT"]!=$PREV_CREATE && strlen($PREV_CREATE)>0) $show_graph = "Y";
	if (strlen($arTicket["DATE_CLOSE"])<=0) $OPEN_TICKETS++;
	else
	{
		$CLOSE_TICKETS++;
		$day_sec = 86400;
...
<tr> 
	...
	<td><?
		if ($bAdmin=="Y" || $bDemo=="Y")	:
			...
			?><br><input class="typeinput" type="text" name="find_responsible" size="47" value="<?=htmlspecialchars($find_responsible)?>"><?=InputType("checkbox", "find_responsible_exact_match", "Y", $find_responsible_exact_match, false, "", "title='".GetMessage("SUP_EXACT_MATCH")."'")?>&nbsp;<?=ShowFilterLogicHelp()?><?
		else : 
...
<tr valign="top">
	...
	<td width="0%" nowrap  valign="top">
		<table border="0" cellspacing="2" cellpadding="0" width="0%">
			<tr>
				<td valign="top" align="center" width="<?=$w?>">
