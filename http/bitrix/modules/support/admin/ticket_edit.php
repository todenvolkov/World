...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && check_bitrix_sessid())
{
	...
	if (CheckFields())
	{
		...
		if ($bSupportTeam=="Y") 
		{
			$rsTicket = CTicket::GetByID($ID, $SITE_ID, $check_rights = "N", $get_user_name = "N", $get_extra_names = "N");
		...
		if ($bDemo!="Y" && $bAdmin!="Y" && $bSupportTeam=="Y" && (intval($ID) > 0 && $RESPONSIBLE_USER_ID!=$USER->GetID()) )  
		{
			// отправим его на список обращений
		...
		if (intval($ID)>0)
		{
			...
			{
				// добавляем к прошлому и новому тикету сообщения о выделении сообщения в отдельную ветку
				$intLastTicketID 	 = IntVal($_SESSION['TICKET_ID']);
				$stLastTicketTitle	 = htmlspecialcharsEx($_SESSION["TICKET_TITLE"]);
				$intSplitMesageID	 = IntVal($_SESSION['MESSAGE_NUM']);
				$stSplitMesageDate	 = MakeTimeStamp($_SESSION['MESSAGE_DATE'], "DD.MM.YYYY HH:MI:SS") ? $_SESSION['MESSAGE_DATE'] : '';
				...
				$arFields = array(
					...
					"MESSAGE_SOURCE_ID"				=> $MESSAGE_SOURCE_ID,
					"MESSAGE_CREATED_USER_ID"		=> $USER->GetID(),
					"MESSAGE_CREATED_MODULE_NAME"	=> "support",
					"MESSAGE_CREATED_GUEST_ID"		=> intval($_SESSION["SESS_GUEST_ID"])>0 ? intval($_SESSION["SESS_GUEST_ID"]) : "null",
					"MESSAGE_SOURCE_ID"				=> $MESSAGE_SOURCE_ID,
					"MESSAGE"						=> GetMessage("SUP_SPLIT_MESSAGE_USER_1", array("#MESSAGE_DATE#" => $stSplitMesageDate, "#TITLE#" => '# '.$ID.' "'.$TITLE.'"')),
				);
				CTicket::AddMessage($intLastTicketID, $arFields, $arrFILES, "N");
				...
				$arFields_log = array(
					...
					"MESSAGE_CREATED_GUEST_ID"		=> intval($_SESSION["SESS_GUEST_ID"])>0 ? intval($_SESSION["SESS_GUEST_ID"]) : "null",
					"MESSAGE_SOURCE_ID"				=> $MESSAGE_SOURCE_ID,
					"MESSAGE"						=> GetMessage("SUP_SPLIT_MESSAGE_LOG_1", array("#MESSAGE_ID#" => $intSplitMesageID, "#TITLE#" => '<a href="'.$TICKET_EDIT_URL.'?ID='.$ID.'&lang='.LANGUAGE_ID.'"> # '.$ID.' "'.$TITLE.'"</a>')),
				);
				CTicket::AddMessage($intLastTicketID, $arFields_log, $arFiles_log, "N");
				...
				CTicket::AddMessage($ID, $arFields, $arFields, "N");
				...
				$arFields_log = array(
					...
					"MESSAGE_CREATED_GUEST_ID"		=> intval($_SESSION["SESS_GUEST_ID"])>0 ? intval($_SESSION["SESS_GUEST_ID"]) : "null",
					"MESSAGE_SOURCE_ID"				=> $MESSAGE_SOURCE_ID,
					"MESSAGE"						=> GetMessage("SUP_SPLIT_MESSAGE_LOG_2", array("#MESSAGE_ID#" => $intSplitMesageID, "#TITLE#" => '<a href="'.$TICKET_EDIT_URL.'?ID='.$intLastTicketID.'&lang='.LANGUAGE_ID.'"> # '.$intLastTicketID.' "'.$stLastTicketTitle.'"</a>')),
				);
				CTicket::AddMessage($ID, $arFields_log, $arFiles_log, "N");
				...
				if (strlen($save)>0) // если нажали сохранить, отправляем к новому тикету				
					LocalRedirect($TICKET_EDIT_URL."?ID=".$ID."&lang=".LANGUAGE_ID);
				elseif (strlen($apply)>0) // если нажали применить, отправляем к тикету из которого выделяли сообщение
			...
			else 
			{
				...
				{
					// если сотрудник техподдержки сменил ответственного то
...
<?
/***************************************************************************
                   Выделение сообщения в новое обращение
****************************************************************************/
if (isset($_GET['TICKET_ID']) && isset($_GET['MESSAGE_ID'])) 
{
   ...
   if ($ticket && $ticket->ExtractFields()) 
   {
	   ...
	   // перебивка переменных, сброс всех лишних данных
