...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $bAdmin=="Y" && check_bitrix_sessid())
{
	...
	if ($bOK)
	{
		if (strlen($save)>0) LocalRedirect("/bitrix/admin/ticket_dict_list.php?lang=".LANGUAGE_ID. "&find_type=".$_REQUEST['C_TYPE']);
		elseif ($new) LocalRedirect("/bitrix/admin/ticket_dict_edit.php?ID=".$ID. "&lang=".LANGUAGE_ID."&find_type=".$_REQUEST['C_TYPE']."&tabControl_active_tab=".urlencode($tabControl_active_tab));
	}
	else
	{
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/bitrix/admin/ticket_dict_list.php?lang=".LANGUAGE_ID."&find_type=".$find_type
	)
);
...
if(intval($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/bitrix/admin/ticket_dict_edit.php?lang=".LANGUAGE_ID."&find_type=".$find_type
		);
	...
	if ($bAdmin=="Y")
	{
		$aMenu[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("SUP_DELETE_RECORD_CONFIRM")."')) window.location='/bitrix/admin/ticket_dict_list.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&find_type=".$find_type ."&". bitrix_sessid_get()."';",
			);
	}
}
...
<?=bitrix_sessid_post()?>
<input type="hidden" name="find_type" value="<?=htmlspecialchars($find_type)?>">
...
<?$tabControl->BeginNextTab();?>
	...
	<tr valign="top">
		<td align="right" width="35%"><?=GetMessage("SUP_TYPE")?></td>
		<td width="65%"><?
			$arr = CTicketDictionary::GetTypeList();
			echo SelectBoxFromArray("C_TYPE", $arr, htmlspecialchars($str_C_TYPE),"","OnChange='C_TYPE_Change()' ");
			?></td>
	</tr>
	<tr valign="top">
		...
		while(list($sid, $arrS) = each($arrSites)):
			...
			?>
			<input type="checkbox" name="arrSITE[]" value="<?=htmlspecialcharsex($sid)?>" id="<?=htmlspecialcharsex($sid)?>" <?=$checked?> <?=$disabled?>>
	...
	<tr valign="top">
		<td align="right"><?=GetMessage("SUP_SORT")?></td>
		<td><input type="text" name="C_SORT" size="5" value="<?=$str_C_SORT?>"></td>
	</tr>
	<tr valign="top">
		<td align="right"><span class="required">*</span><?=GetMessage("SUP_NAME")?></td>
		<td><input type="text" name="NAME" size="60" maxlength="255" value="<?=$str_NAME?>"></td>
	</tr>
	<tr valign="top">
		<td align="right"><?=GetMessage("SUP_SID")?></td>
		...
		<td width="100%" colspan=2 align="center">
			...
			<table width="100%" cellspacing=0 cellpadding=0>
				<tr valign="top">
					...
					<td width="60%"><?echo InputType("checkbox", "SET_AS_DEFAULT", "Y", $str_SET_AS_DEFAULT, false); ?></td>
				</tr>
			</table>
			</div>
		</td>
		...
		<td width="100%" colspan=2 align="center">
			...
			<table width="100%" cellspacing=0 cellpadding=0>
				<tr valign="top">
					...
					<td width="60%"><?echo SelectBox("RESPONSIBLE_USER_ID", CTicket::GetSupportTeamList(), GetMessage("SUP_NO"), $str_RESPONSIBLE_USER_ID);?></td>
				</tr>
			</table>
			</div>
		</td>
	...
	<tr>
		<td width="100%" colspan=2 align="center">
			...
			<table width="100%" cellspacing="8" cellpadding="0">
				...
				<tr>
					<td align="right" width="40%" valign="top">event1:</td>
					<td width="60%"><input type="text" name="EVENT1" maxlength="255" size="30" value="<? echo $str_EVENT1;?>"></td>
				</tr>
				<tr>
					<td align="right" valign="top">event2:</td>
					<td><input type="text" name="EVENT2" maxlength="255" size="30" value="<? echo $str_EVENT2;?>"><br><?echo GetMessage("SUP_EVENT12")?></td>
				</tr>
				<tr>
					<td align="right" valign="top">event3:</td>
					<td><input type="text" name="EVENT3" maxlength="255" size="30" value="<? echo $str_EVENT3;?>"><br><?echo GetMessage("SUP_EVENT3")?></td>
				</tr>
			</table>
			</div>
		</td>
