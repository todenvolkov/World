...
class CSupportSuperCoupon
{
	function Generate($arParams = array())
	{
		...
		{
			$COUPON_FORMAT = $arParams['KEY_FORMAT'];
		}
		else 
		{ 
			$COUPON_FORMAT = COption::GetOptionString('support', 'SUPERTICKET_COUPON_FORMAT');
		}
		
		$COUNT = array_key_exists('COUNT_TICKETS', $arParams) ? intval($arParams['COUNT_TICKETS']) : 5;
		if ($COUNT <= 0) $COUNT = 5;
		
		$SLA_ID = array_key_exists('SLA_ID', $arParams) ? $arParams['SLA_ID'] : COption::GetOptionString("support", 'SUPERTICKET_DEFAULT_SLA');
		$SLA_ID = intval($SLA_ID);
		if ($SLA_ID <= 0) $SLA_ID = false;
		
		$coupon = false;
		$DB->StartTransaction();
		for ($i = 0; $i < 100; ++$i)
		{
			$coupon = preg_replace_callback('|#|'.BX_UTF_PCRE_MODIFIER, array('CSupportSuperCoupon', '_getrandsymbol'), $COUPON_FORMAT);
	...
	function UseCoupon($coupon)
	{
		...
		{
			$COUNT_TICKETS = intval($arCoupon['COUNT_TICKETS']);
			if ($COUNT_TICKETS > 0)
			{
				$COUNT_TICKETS--;
				$arFields = array(
