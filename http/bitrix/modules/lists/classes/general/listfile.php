...
class CListFile
{
	...
	function GetInputHTML($params = array())
	{
		...
		$strReturn = ' <input name="'.htmlspecialchars($input_name).'" size="'.$size.'" type="file" />';
		...
		if(is_array($this->_file))
		{
			...
			$strReturn .= '<br><input type="checkbox" name="'.htmlspecialchars($del_name).'" value="Y" id="'.htmlspecialchars($del_name).'" />';
			$strReturn .= ' <label for="'.htmlspecialchars($del_name).'">'.GetMessage('FILE_DELETE').'</label>';
		}
	...
	function GetImgHtml($params = array())
	{
		...
		if(is_array($this->_file))
		{
			...
			$html = '<img src="'.htmlspecialchars($src).'" width="'.$intWidth.'" height="'.$intHeight.'"';
	...
	function GetLinkHtml($params = array())
	{
		...
		{
			$src = $this->GetImgSrc($params);
			return ' [ <a href="'.htmlspecialchars($src).'">'.$params['download_text'].'</a> ] ';
		}
		else
		{
			return '';
