	...
	function CalcUserRatingForumActivity($arConfigs)
	{
		...
		if (isset($arConfigs['CONFIG']['ALL_TOPIC_COEF']) && $arConfigs['CONFIG']['ALL_TOPIC_COEF'] != 0) {
			$sqlAllTopic = "
				SELECT
					...
					COUNT(*)*".$arConfigs['CONFIG']['ALL_TOPIC_COEF']." as CURRENT_VALUE
				FROM b_forum_topic
				GROUP BY USER_START_ID
				UNION ALL ";
		}
		...
		$strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
			...
			FROM (
				...
				SELECT 
					AUTHOR_ID as ENTITY_ID, 
				    SUM(IF(TO_DAYS(POST_DATE) > TO_DAYS(NOW())-1, 1, 0))*".$arConfigs['CONFIG']['TODAY_POST_COEF']." + 
				    SUM(IF(TO_DAYS(POST_DATE) > TO_DAYS(NOW())-7, 1, 0))*".$arConfigs['CONFIG']['WEEK_POST_COEF']."+
				...
				SELECT 
				   USER_START_ID as ENTITY_ID, 
			       SUM(IF(TO_DAYS(START_DATE) > TO_DAYS(NOW())-1, 1, 0))*".$arConfigs['CONFIG']['TODAY_TOPIC_COEF']." + 
			       SUM(IF(TO_DAYS(START_DATE) > TO_DAYS(NOW())-7, 1, 0))*".$arConfigs['CONFIG']['WEEK_TOPIC_COEF']." + 
			       COUNT(*)*".$arConfigs['CONFIG']['MONTH_TOPIC_COEF']." as CURRENT_VALUE
				FROM b_forum_topic
