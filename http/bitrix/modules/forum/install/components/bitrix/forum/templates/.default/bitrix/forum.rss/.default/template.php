...
if ($arParams["MODE"] == "link"):
	...
	foreach($arResult["rss_link"] as $key => $val):
		?><a href="<?=$val["link"]?>" title="<?=($arParams["MODE_DATA"] == "topic" ? GetMessage("F_RSS_POST") : GetMessage("F_RSS"))?><?=$val["name"]?><?
			?>" class="forum-rss-<?=$key?>" target="_self"><span class="empty"></span></a><?
