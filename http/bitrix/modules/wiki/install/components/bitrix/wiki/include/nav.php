...
{            
	if ($arParams['IN_COMPLEX'] == 'Y') 
	{
		...
		if (!empty($sNavItem))
			$APPLICATION->AddChainItem($sNavItem, SITE_SERVER_NAME.$sSefFolder);
	}
	...
	if(!empty($arParams['SET_TITLE']) && $arParams['SET_TITLE'] == 'Y' && !empty($arResult['ELEMENT']['NAME']))
	{
		...
		$APPLICATION->AddChainItem($sPageName, CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_POST"],
			array(
				'wiki_name' => urlencode($arResult['ELEMENT']['NAME']), 
				'group_id' => CWikiSocnet::$iSocNetId)
			)
