...
include(GetLangFileName($GLOBALS["DOCUMENT_ROOT"]."/bitrix/modules/socialnetwork/lang/", "/options.php"));

include_once($GLOBALS["DOCUMENT_ROOT"]."/bitrix/modules/socialnetwork/include.php");

if ($REQUEST_METHOD=="GET" && strlen($RestoreDefaults)>0 && $SONET_RIGHT=="W")
{
...
if ($bIntranet)
{
	$arFeatures["files"] = GetMessage("SONET_FEATURE_FILES");
...
$arAllOptionsCommon = array(
	array("log_cleanup_days", GetMessage("SONET_LOG_CLEANUP_DAYS"), "7", Array("text", 3)),
);
...
$arAllOptionsUsers = array(
	...
	array("allow_forum_user", GetMessage("SONET_ALLOW_FORUM_USER"), "Y", Array("checkbox")),
	array("allow_blog_user", GetMessage("SONET_ALLOW_BLOG_USER"), "Y", Array("checkbox")),
	array("allow_photo_user", GetMessage("SONET_ALLOW_PHOTO_USER"), "Y", Array("checkbox")),
);
...
$arAllOptionsUsersBlocks["blog"][] = array("default_blog_operation_premoderate_comment_user", GetMessage("SONET_BLOG_OPERATION_PREMODERATE_COMMENT_USER"), SONET_RELATIONS_TYPE_AUTHORIZED, Array("select_user"));
$arAllOptionsUsersBlocks["blog"][] = array("default_blog_operation_write_comment_user", GetMessage("SONET_BLOG_OPERATION_WRITE_COMMENT_USER"), SONET_RELATIONS_TYPE_AUTHORIZED, Array("select_user"));
...
$arAllOptionsGroups = array(
	array("allow_forum_group", GetMessage("SONET_ALLOW_FORUM_GROUP"), "Y", Array("checkbox")),
	array("allow_blog_group", GetMessage("SONET_ALLOW_BLOG_GROUP"), "Y", Array("checkbox")),
	array("allow_photo_group", GetMessage("SONET_ALLOW_PHOTO_GROUP"), "Y", Array("checkbox")),
);

if ($bIntranet)
{
	$arAllOptionsGroups[] = array("allow_files_group", GetMessage("SONET_ALLOW_FILES_GROUP"), "Y", Array("checkbox"));
	$arAllOptionsGroups[] = array("allow_calendar_group", GetMessage("SONET_ALLOW_CALENDAR_GROUP"), "Y", Array("checkbox"));
	$arAllOptionsGroups[] = array("allow_tasks_group", GetMessage("SONET_ALLOW_TASKS_GROUP"), "Y", Array("checkbox"));
}

if (IsModuleInstalled('search'))
	$arAllOptionsGroups[] = array("allow_search_group", GetMessage("SONET_ALLOW_SEARCH_GROUP"), "Y", Array("checkbox"));
...
$arAllOptionsGroupsBlocks["blog"][] = array("default_blog_operation_write_post_group", GetMessage("SONET_BLOG_OPERATION_WRITE_POST_GROUP"), SONET_ROLES_MODERATOR, Array("select_group"));
...
$arAllOptionsGroupsBlocks["blog"][] = array("default_blog_operation_full_post_group", GetMessage("SONET_BLOG_OPERATION_FULL_POST_GROUP"), SONET_ROLES_OWNER, Array("select_group"));
...
$strWarning = "";
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $SONET_RIGHT=="W" && check_bitrix_sessid())
{
	COption::SetOptionString("socialnetwork", "log_cleanup_days", $log_cleanup_days);

	$cleanup_days = COption::GetOptionInt("socialnetwork", "log_cleanup_days", 7);
	if($cleanup_days > 0)
	{
	...
	while ($arSite = $dbSites->Fetch())
	{
		...
		for ($i = 0; $i < count($arAllOptionsUsers); $i++)
		{
			...
			if ($arAllOptionsUsers[$i][0] == "allow_blog_user")
			{
				if ($val == "Y")
				{
					$bBlogDisabledForAllSites = false;
					$bBlogEnabledForAnySite = true;
				}
			}
		...
		{		
			for ($i = 0; $i < count($arAllOptionsUsersBlocks[$feature]); $i++)
			{
		...
		{		
			for ($i = 0; $i < count($arAllOptionsUsersGender[$gender]); $i++)
			{
		...
		for ($i = 0; $i < count($arAllOptionsGroups); $i++)
		{
		...
		foreach ($arFeatures as $feature => $feature_name)
		{
			for ($i = 0; $i < count($arAllOptionsGroupsBlocks[$feature]); $i++)
			{
		...
		for ($i = 0; $i < count($arAllOptionsGroupsGender); $i++)
		{
...
while ($arSite = $dbSites->Fetch())
{
	...
	$aSiteTabs[] = array("DIV" => "opt_site_".$arSite["ID"], "TAB" => '['.$arSite["ID"].'] '.htmlspecialchars($arSite["NAME"]), 'TITLE' => GetMessage('SONET_OPTIONS_FOR_SITE').' ['.$arSite["ID"].'] '.htmlspecialchars($arSite["NAME"]));
	
	$aSubTabs = array();
	foreach ($arFeatures as $key => $value)
	{
		$aSubTabs[] = array("DIV" => "opt_user_feature_".$key."_".$arSite["ID"], "TAB" => $value, 'TITLE' => GetMessage('SONET_SUBTAB_USER_TITLE_FEATURE').' "'.$value.'"');
	}
...
<script>
	function SelectSite(id)
	{
		...
		document.getElementById('<?= CUtil::JSEscape(htmlspecialchars($siteList[$i]["ID"]));?>_Propery').style.display='none';
		<?endfor;?>
		document.getElementById(id+'_Propery').style.display='';
	}
</script>

<form method="POST" name="sonet_opt_form" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&lang=<?=LANGUAGE_ID?>" ENCTYPE="multipart/form-data"><?
?><?=bitrix_sessid_post()?><?
$tabControl->BeginNextTab();

	?>
	<?for ($i = 0; $i < count($arAllOptionsCommon); $i++):
		...
		for($j = 1; $j < $siteCount; $j++)
		{
			...
			<?for ($i = 0; $i < count($arAllOptions); $i++):
				...
				<tr>
					<td valign="top" width="50%" align="right"><?
						if ($type[0]=="checkbox")
							echo "<label for=\"".htmlspecialchars($Option[0]."_".$arSite["ID"])."\">".$Option[1]."</label>";
						else
							echo $Option[1];
					?>:</td>
					<td valign="middle" width="50%" align="left">
						<?if($type[0]=="checkbox"):?>
							<input type="checkbox" name="<?echo htmlspecialchars($Option[0]."_".$siteList[$j]["ID"])?>" id="<?echo htmlspecialchars($Option[0]."_".$siteList[$j]["ID"])?>" value="Y"<?if($val=="Y")echo" checked";?>>
						<?elseif($type[0]=="text"):?>
							<input type="text" size="<?echo $type[1]?>" value="<?echo htmlspecialchars($val)?>" name="<?echo htmlspecialchars($Option[0]."_".$siteList[$j]["ID"])?>">
						<?elseif($type[0]=="textarea"):?>
							<textarea rows="<?echo $type[1]?>" cols="<?echo $type[2]?>" name="<?echo htmlspecialchars($Option[0]."_".$siteList[$j]["ID"])?>"><?echo htmlspecialchars($val)?></textarea>
						<?elseif($type[0]=="select_fields"):?>
							<select <?=($type[1] == true ? "multiple" : "")?> size="<?echo $type[2]?>" name="<?echo htmlspecialchars($Option[0]."_".$siteList[$j]["ID"])?><?=($type[1] == true ? "[]" : "")?>">
			...
			<?for ($i = 0; $i < count($arAllOptionsUsers); $i++):
				...
				<tr>
					<td valign="top" width="50%" align="right"><?
						if ($type[0]=="checkbox")
							echo "<label for=\"".htmlspecialchars($Option[0]."_".$siteList[$j]["ID"])."\">".$Option[1]."</label>";
						else
							echo $Option[1];
						?>:</td>
						<td valign="middle" width="50%" align="left">
						...
						<?elseif($type[0]=="select_user_perm"):?>
							<select name="<?echo htmlspecialchars($Option[0]."_".$siteList[$j]["ID"])?>">
			...
			<tr>
				...
				foreach ($arAllOptionsUsersBlocks as $feature => $arAllOptionsUsersFeature):
					...
					?>
					<table cellspacing="7" cellpadding="0" border="0" width="100%">
					<?for ($i = 0; $i < count($arAllOptionsUsersFeature); $i++):
									...
									<?elseif($type[0]=="select_user"):?>
										...
										<?foreach ($arUserPermsVar as $permvar_key => $permvar_value):
											...
											if (
												...
												|| !in_array($permvar_key, $GLOBALS["arSocNetFeaturesSettings"][$feature]["operations"][$operation]["restricted"][SONET_ENTITY_USER])
											):
			...
			<tr>
				...
				foreach ($arAllOptionsUsersGender as $gender => $arOptionUserGender):
					...
					?>
					<table cellspacing="7" cellpadding="0" border="0" width="100%">
					<?for ($i = 0; $i < count($arOptionUserGender); $i++):
						...
						if (count($Option) > 0)
						{
							...
							<tr>
								...
								<td valign="middle" width="50%">
									<?if($type[0]=="image"):?>
										<?echo CFile::InputFile(htmlspecialchars($Option[0]."_".$siteList[$j]["ID"]), 20, $val);?><br>
			...
			<tr>
				...
				foreach ($arAllOptionsGroupsBlocks as $feature => $arAllOptionsGroupsFeature):
					...
					?>
					<table cellspacing="7" cellpadding="0" border="0" width="100%">
					<?for ($i = 0; $i < count($arAllOptionsGroupsFeature); $i++):
								...
								<td valign="middle" width="50%">
									...
									<?elseif($type[0]=="select_group"):?>
										...
										<?foreach ($arGroupPermsVar as $permvar_key => $permvar_value):
											...
											if (
												!array_key_exists("restricted", $GLOBALS["arSocNetFeaturesSettings"][$feature]["operations"][$operation]) 
												|| !in_array($permvar_key, $GLOBALS["arSocNetFeaturesSettings"][$feature]["operations"][$operation]["restricted"][SONET_ENTITY_GROUP])
											):
												?><option value="<?= $permvar_key ?>"<?= ($permvar_key == $val) ? " selected" : "" ?>><?= $permvar_value ?></option><?
											endif;
										endforeach;?>
...
<input type="submit" <?if ($SONET_RIGHT<"W") echo "disabled" ?> name="Update" value="<?echo GetMessage("MAIN_SAVE")?>">
