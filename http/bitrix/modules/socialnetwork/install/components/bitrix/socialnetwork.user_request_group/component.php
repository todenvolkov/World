	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if (!CSocNetUserToGroup::SendRequestToBeMember($GLOBALS["USER"]->GetID(), $arResult["Group"]["ID"], "", "", $bAutoSubscribe))
					{
						if ($e = $APPLICATION->GetException())
							$errorMessage .= $e->GetString();
					}
				...
				else
				{
					...
					if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["save"]) > 0 && check_bitrix_sessid())
					{
						...
						if (strlen($errorMessage) <= 0)
						{
							...
							if (!CSocNetUserToGroup::SendRequestToBeMember($GLOBALS["USER"]->GetID(), $arResult["Group"]["ID"], $_POST["MESSAGE"], $arResult["Urls"]["GroupRequests"], $bAutoSubscribe))
							{
								if ($e = $APPLICATION->GetException())
									$errorMessage .= $e->GetString();
							}
