...
else
{
	...
	if (StrLen($arResult["FatalError"]) <= 0)
	{
		...
		$is_my = false;
		if (
			array_key_exists($arParams["ENTITY_TYPE"], $GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"])
				&& array_key_exists("CLASS_MY_BY_ID", $GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]])
				&& array_key_exists("METHOD_MY_BY_ID", $GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]])				
			)
			$is_my = call_user_func(
				array(
					$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]]["CLASS_MY_BY_ID"],
					$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]]["METHOD_MY_BY_ID"]
				),
				$arParams["ENTITY_ID"]
			);
	...
	if (StrLen($arResult["FatalError"]) <= 0)
	{
		...
		$arResult["Urls"]["Entity"] = CComponentEngine::MakePathFromTemplate(
				$arParams[$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"]["URL_PARAM_KEY"]], 
				array(
					$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"]["URL_PATTERN"] => $arEvents["ENTITY_ID"]
				)
			);
		...
		if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["save"]) > 0 && check_bitrix_sessid())
		{
			...
			foreach ($arResult["Subscribe"] as $event_id_tmp => $arEventTmp)
			{
				...
				$bFound = false;
				if (
					array_key_exists($event_id_tmp, $GLOBALS["arSocNetLogEvents"])
					&& array_key_exists("FULL_SET", $GLOBALS["arSocNetLogEvents"][$event_id_tmp])
				)
				{
					$arEventID = $GLOBALS["arSocNetLogEvents"][$event_id_tmp]["FULL_SET"];
					$bFound = true;
				}
				else
				{
					...
					)
					{
						$idTmp = CSocNetLogEvents::Add(
							array(
								...
								"EVENT_ID" 		=> $event_id,
								"SITE_ID" 		=> (
													array_key_exists("HAS_SITE_ID", $GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]])
													&& $GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]]["HAS_SITE_ID"] == "Y"
													&& defined("SITE_ID") 
													&& strlen(SITE_ID) > 0 
													? 
														SITE_ID 
