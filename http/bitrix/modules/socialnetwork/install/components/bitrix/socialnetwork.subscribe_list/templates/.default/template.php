...
else
{
	...
	<? 
	$bFirstBlock = true;
	foreach ($GLOBALS["arSocNetAllowedSubscribeEntityTypes"] as $entity_type):
		...
		<tr>
			<td valign="top" width="30%" class="subscribe-list-entity-td">
				...
				?>
				<b><?=$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$entity_type]["TITLE_LIST"]?></b>
				<?
				if ($bHasEntities)
				{
					?></a><?
		...
		foreach ($arResult["EventsNew"][$entity_type] as $eventKey => $event):?>
			...
			<tr id="bx_sl_<?=$event["ENTITY_TYPE"]?>_<?=$event["ENTITY_ID"]?>_tr">
				<td valign="top" width="30%" class="subscribe-list-entity-td subscribe-list-entity-name">
					...
					$name = call_user_func(
						array(
							$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$event["ENTITY_TYPE"]]["CLASS_DESC_SHOW"],
							$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$event["ENTITY_TYPE"]]["METHOD_DESC_SHOW"]
						),
						$event["ENTITY_DESC"],
						$event["ENTITY_URL"],
						$arParams
				...
				<td valign="top" width="70%" class="subscribe-list-entity-td">
					...
					<tbody>
						...
						else
						{
							...
							<td width="5%">
								<a class="subscribe-list-del" href="javascript:void(0)" onclick="sonet_sl_del('bx_sl_<?=$event["ENTITY_TYPE"]?>_<?=$event["ENTITY_ID"]?>_tr', '<?=$event["ENTITY_TYPE"]?>', '<?=$event["ENTITY_ID"]?>', 'all'); return false;"></a>
							</td>
						</tr>
						<?
						if ($event_tmp != "all")
		...
		if (
			...
			<tr>
				<td valign="top" width="30%" class="subscribe-list-entity-td">
					...
					?>
					<b><?=$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$entity_type]["TITLE_LIST_MY"]?></b>
					<?
					if ($bHasEntities)
					{
						?></a><?
		...
		if (
			array_key_exists("HAS_CB", $GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$entity_type]) 
			&& $GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$entity_type]["HAS_CB"] == "Y"
					...
					<td valign="top" width="70%" class="subscribe-list-entity-td">
						<table width="100%">
							<tr>
								...
								<td width="5%">
									<a class="subscribe-list-del" href="javascript:void(0)" onclick="sonet_sl_del('bx_sl_<?=$event["ENTITY_TYPE"]?>_<?=$event["ENTITY_ID"]?>_cb_tr', '<?=$event["ENTITY_TYPE"]?>', '<?=$event["ENTITY_ID"]?>', 'all', 'Y'); return false;"></a>
								</td>
							</tr>
						</table>
