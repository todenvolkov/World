...
?>
	...
	<tr>
		<td><?echo GetMessage("SONET_IMAGE")?>:<br><small><?echo GetMessage("SONET_IMAGE_NOTE")?></small></td>
		<td>
			<input type="file" name="IMAGE1" size="30">
			<?
			if (strlen($f_IMAGE)>0)
			{
				?><br><img src="/bitrix/images/socialnetwork/<?echo ($f_SMILE_TYPE=="I")?"icon":"smile" ?>/<?echo $f_IMAGE?>" border="0" <?echo (IntVal($f_IMAGE_WIDTH)>0) ? "width=\"".$f_IMAGE_WIDTH."\"" : "" ?> <?echo (IntVal($f_IMAGE_WIDTH)>0) ? "height=\"".$f_IMAGE_HEIGHT."\"" : "" ?>><?
			}
			?>
		</td>
	</tr>
	...
	for ($i = 0; $i < count($arSysLangs); $i++):
		...
		<tr>
			<td>
				<span class="required">*</span><?echo GetMessage("SONET_NAME")?>:
			</td>
			<td>
