...
class CSocNetUserToGroup extends CAllSocNetUserToGroup
{
	...
	function Add($arFields)
	{
		...
		if (strlen($arInsert[0]) > 0)
		{
			...
			if ($arFields["INITIATED_BY_TYPE"] == SONET_INITIATED_BY_GROUP)
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		static $arFields = array(
			...
			"MESSAGE" => Array("FIELD" => "UG.MESSAGE", "TYPE" => "string"),
 			...
 			"GROUP_SITE_ID" => Array("FIELD" => "G.SITE_ID", "TYPE" => "string", "FROM" => "INNER JOIN b_sonet_group G ON (UG.GROUP_ID = G.ID)"),
