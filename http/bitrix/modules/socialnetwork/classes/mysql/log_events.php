...
class CSocNetLogEvents extends CAllSocNetLogEvents
{
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arParams = array())
	{
		...
		static $arFields = array(
			...
			"GROUP_SITE_ID" 	=> Array("FIELD" => "G.SITE_ID", "TYPE" => "string", "FROM" => "INNER JOIN b_sonet_group G on G.ID = LE.ENTITY_ID"), 
			"COMMON_GROUP_SITE_ID" 	=> Array("FIELD" => "G.SITE_ID", "TYPE" => "string_or_null", "FROM" => "LEFT JOIN b_sonet_group G ON G.ID = LE.ENTITY_ID")
		);
	...
	function GetUserLogEvents($userID, $arFilter = array())
	{
		...
		if (!Array_Key_Exists("ALL", $arFilter) || StrToUpper($arFilter["ALL"]) != "Y")
		{
			$strSql .= 
				...
				"		ON (L.ENTITY_TYPE = LE.ENTITY_TYPE AND L.ENTITY_ID = LE.ENTITY_ID AND (L.EVENT_ID = LE.EVENT_ID OR ((L.EVENT_ID = 'blog_post' OR L.EVENT_ID = 'blog_comment') AND LE.EVENT_ID = 'blog'))) ";
		}
		$strSql .= 
