...
class CAllSocNetFeaturesPerms
{
	...
	function SetPerm($featureID, $operation, $perm)
	{
		...
		$dbResult = CSocNetFeaturesPerms::GetList(
			...
			false,
			false,
			array("ID", "FEATURE_ENTITY_TYPE", "FEATURE_ENTITY_ID", "FEATURE_FEATURE", "OPERATION_ID")
		);
		...
		else
		{
			if (!$arResult)
			{
				...
				if ($arFeature)
					CSocNetEventUserView::SetFeature($arFeature["ENTITY_TYPE"], $arFeature["ENTITY_ID"], $arFeature["FEATURE"], $operation, $perm, true);
			}
			else
				CSocNetEventUserView::SetFeature($arResult["FEATURE_ENTITY_TYPE"], $arResult["FEATURE_ENTITY_ID"], $arResult["FEATURE_FEATURE"], $arResult["OPERATION_ID"], $perm, true);

			if($arResult)
				CSocNetSearch::SetFeaturePermissions($arResult["FEATURE_ENTITY_TYPE"], $arResult["FEATURE_ENTITY_ID"], $arResult["FEATURE_FEATURE"], $arResult["OPERATION_ID"], $perm);
			else
				CSocNetSearch::SetFeaturePermissions($arFeature["ENTITY_TYPE"], $arFeature["ENTITY_ID"], $arFeature["FEATURE"], $operation, $perm);
		}
	...
	/**********  COMMON METHODS  ***********/
	/***************************************/
	function CurrentUserCanPerformOperation($type, $id, $feature, $operation)
	{
		$userID = 0;
...
*/
	...
	function GetOperationPerm($type, $id, $feature, $operation)
	{
		...
		if (!array_key_exists($operation, $arSocNetFeaturesSettings[$feature]["operations"]))
			return false;
