...
class CSocNetSearchReindex extends CSocNetSearch
{
	...
	*/
	...
	function ReindexForum($entity_type, $last_id, $path_template)
	{
		...
		$rsForumTopics = $DB->Query("
			SELECT ID, SOCNET_GROUP_ID, OWNER_ID
			FROM b_forum_topic
			WHERE
				ID > ".intval($last_id)."
				".($entity_type == "G"?
					"AND SOCNET_GROUP_ID IS NOT NULL AND SOCNET_GROUP_ID > 0":
					"AND (SOCNET_GROUP_ID IS NULL OR SOCNET_GROUP_ID = 0) AND OWNER_ID IS NOT NULL"
				)."
			ORDER BY ID
		");
		while($arTopic = $rsForumTopics->Fetch())
		{
			$rsForumMessages = $DB->Query("
				SELECT ID
				FROM b_forum_message
				WHERE TOPIC_ID = ".$arTopic["ID"]."
			");
			while($arMessage = $rsForumMessages->Fetch())
			{
				...
				$arGroups = $this->GetSearchGroups(
					$entity_type,
					$entity_type=="G"? $arTopic["SOCNET_GROUP_ID"]: $arTopic["OWNER_ID"],
					'forum',
					'view'
				);
		...
		switch(substr($ID, 0, 1))
		{
			case "B":
				...
				if(
					...
					&& strlen($this->_params["PATH_TO_USER_BLOG"]) > 0
				)
				{
					...
					foreach($arFields["SITE_ID"] as $site_id => $url)
					{
						$arFields["SITE_ID"][$site_id] = str_replace(
							...
							$this->_params["PATH_TO_USER_BLOG"]
						);
					}

					$arFields["REINDEX_FLAG"] = true;
					CSearch::Index("blog", $ID, $arFields, true, $this->_sess_id);
					$this->_counter++;
				}
				break;
			case "P":
	...
	function UpdateForumTopicIndex($topic_id, $entity_type, $entity_id, $feature, $operation, $path_template)
	{
		...
		CSearch::ChangePermission("forum", $arGroups, false, false, $topic_id);
		...
		CSearch::ChangeIndex("forum", array("UPD" => $this->_sess_id, "PARAMS"=>$arParams), false, false, $topic_id);
	}
	...
	function ReindexIBlock($iblock_id, $entity_type, $feature, $operation, $path_template, $arFieldList, $last_id)
	{
		...
		while($ar = $rsElements->Fetch())
		{
			if(!array_key_exists($ar["IBLOCK_SECTION_ID"], $arSections))
			{
				...
				if($entity_type == "G")
					$arSections[$ar["IBLOCK_SECTION_ID"]] = intval($arSection["SOCNET_GROUP_ID"]);
				else
					$arSections[$ar["IBLOCK_SECTION_ID"]] = intval($arSection["CREATED_BY"]);
			}
			...
			if($entity_id)
			{
				...
				foreach($arFieldList as $field)
				{
					if($field == "PREVIEW_TEXT" || $field == "DETAIL_TEXT")
						$body .= ($ar[$field."_TYPE"]=="html"? HTMLToTxt($ar[$field]): $ar[$field])."\n\r";
					elseif($field == $this->_file_property)
					{
						$arFile = CIBlockElement::__GetFileContent($ar[$this->_file_property."_VALUE"]);
						if(is_array($arFile))
						{
							$body .= $arFile["CONTENT"]."\n\r";
							$ar["TAGS"] .= ",".$arFile["PROPERTIES"][COption::GetOptionString("search", "page_tag_property")];
						}
					}
