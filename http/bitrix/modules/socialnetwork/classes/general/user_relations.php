...
class CAllSocNetUserRelations
{
	...
	function GetRelatedUsers($userID, $relation, $arNavStartParams = false)
	{
		global $DB, $arSocNetAllowedRelations;
		...
		$dbResult = CSocNetUserRelations::GetList(
			...
			array("ID", "FIRST_USER_ID", "SECOND_USER_ID", "DATE_CREATE", "DATE_UPDATE", "INITIATED_BY",
				"FIRST_USER_NAME", "FIRST_USER_LAST_NAME", "FIRST_USER_PERSONAL_PHOTO", "FIRST_USER_PERSONAL_GENDER", "FIRST_USER_SECOND_NAME", "FIRST_USER_LOGIN", "FIRST_USER_EMAIL",
				"SECOND_USER_NAME", "SECOND_USER_LAST_NAME", "SECOND_USER_PERSONAL_PHOTO", "SECOND_USER_PERSONAL_GENDER", "SECOND_USER_SECOND_NAME", "SECOND_USER_LOGIN", "SECOND_USER_EMAIL"
			)
		);
	...
	function GetRelation($firstUserID, $secondUserID)
	{
		...
		if (!array_key_exists($firstUserID."_".$secondUserID, $arSocNetUserRelationsCache1))
		{
			...
			$dbResult = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
			if ($arResult = $dbResult->Fetch())
				$arSocNetUserRelationsCache[$firstUserID."_".$secondUserID] = $arResult["RELATION"];
			else
	...
	function ConfirmRequestToBeFriend($senderUserID, $relationID, $bAutoSubscribe = true)
	{
		...
		foreach ($arUserID as $userID)
			CSocNetEventUserView::SetUser($userID, false, false, true);
	...
	function BanUser($senderUserID, $targetUserID)
	{
		...
		elseif (CSocNetUser::IsUserModuleAdmin($targetUserID))
		{
	...
	function __SpeedFileDelete($userID)
	{
		...
		$fileName = $_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/managed_cache/socnet/c/".IntVal($userID / 1000)."/".$userID."_f";
