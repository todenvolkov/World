...
class CAllSocNetGroup
{
	...
	function CheckFields($ACTION, &$arFields, $ID = 0)
	{
		...
		elseif (is_set($arFields, "SITE_ID"))
		{
			...
			if (!$dbResult->Fetch())
			{
				$GLOBALS["APPLICATION"]->ThrowException(str_replace("#ID#", $arFields["SITE_ID"], GetMessage("SONET_GG_ERROR_NO_SITE")), "ERROR_NO_SITE");
				return false;
			}
		}
	...
	function Delete($ID)
	{
		...
		if ($bSuccess)
			$bSuccess = $DB->Query("DELETE FROM b_sonet_log WHERE ENTITY_TYPE = 'G' AND ENTITY_ID = ".$ID."", true);
		if ($bSuccess)
...
//			unset($GLOBALS["BLOG4OWNER_CACHE_".$arBlog["OWNER_ID"]]);
		}
...
//			$GLOBALS["USER_FIELD_MANAGER"]->Delete("BLOG_BLOG", $ID);
	...
	function SetStat($ID)
	{
		...
		CSocNetGroup::Update($ID, array("NUMBER_OF_MEMBERS" => $num));
	}
	...
	function GetByID($ID)
	{
		...
		if (array_key_exists("SONET_GROUP_CACHE", $GLOBALS) 
		...
		$dbResult = CSocNetGroup::GetList(Array(), Array("ID" => $ID), false, false, array("ID", "SITE_ID", "NAME", "DESCRIPTION", "DATE_CREATE", "DATE_UPDATE", "ACTIVE", "VISIBLE", "OPENED", "CLOSED", "SUBJECT_ID", "OWNER_ID", "KEYWORDS", "IMAGE_ID", "NUMBER_OF_MEMBERS", "INITIATE_PERMS", "SPAM_PERMS", "DATE_ACTIVITY", "SUBJECT_NAME", "UF_*"));
