...
//			$text = preg_replace("#(^|\s)((http|https|news|ftp)://[-_:A-Za-z0-9@]+(\.[-_/=:A-Za-z0-9@&?=%]+)+)#ie", "\$this->convert_anchor_tag('\\2', '\\2', '\\1')", $text);
		...
		while (is_array($arImages) && list($IMAGE_ID, $FILE_ID)=each($arImages))
		{
			...
			$text = str_replace("[IMG ID=$IMAGE_ID]",CFile::ShowImage($FILE_ID,null,null,"title=\"".htmlspecialchars($f['TITLE'])."\""), $text);
			$text = str_replace("[img id=$IMAGE_ID]",CFile::ShowImage($FILE_ID,null,null,"title=\"".htmlspecialchars($f['TITLE'])."\""), $text);
		}
		*/
	...
	function convert_image_tag($url = "")
	{
		...
		if (!$bErrorIMG && !preg_match("/^(http|https|ftp):\/\/[-_:A-Za-z0-9@]+\.*([-_\/=:A-Za-z0-9@%.]+)+$/i", $url))
			$bErrorIMG = True;
...
class CSocNetTools
{
	...
	function ResizeImage($aFile, $sizeX, $sizeY)
	{
		...
		if ($imgX > $sizeX || $imgY > $sizeY)
		{
			...
			$file = "/".COption::GetOptionString("main", "upload_dir", "upload")."/socialnetwork/"."resized_".$aFile["FILE_NAME"];
			switch ($image_type)
			{
				case IMAGETYPE_JPEG:
