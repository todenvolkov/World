...
class CAllSocNetEventUserView
{
	...
	{
		global $arSocNetLogEvents, $arSocNetAllowedEntityTypes, $arSocNetFeaturesSettings, $DB;
	...
	function IsEntityEmpty($entityType, $entityID)
	{
		...
		$dbResult = CSocNetEventUserView::GetList(array(), array("ENTITY_TYPE" => $entityType, "ENTITY_ID" => $entityID));
		if (intval($dbResult->SelectedRowsCount()) > 0)
			return false;
		else
			return true;		
	}
	...
	function CheckPermissions($table, $user_id)
	{
		...
		if ($user_id === false)
			$strUser = " AND EUV.USER_ANONYMOUS = 'Y'";
		else
			$strUser = " AND 
				(
					(EUV.USER_ANONYMOUS = 'N' AND EUV.USER_ID = ".intval($user_id).") 
					OR (EUV.USER_ANONYMOUS = 'N' AND EUV.USER_ID = 0) 
					OR EUV.USER_ANONYMOUS = 'Y'
				)";
			
		return "INNER JOIN b_sonet_event_user_view EUV ON
