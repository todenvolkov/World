...
class CAllSocNetUserToGroup
{
	...
	function Delete($ID, $bSendExclude = false)
	{
		...
		if ($bSuccess)
		{
			$DB->Query("DELETE FROM b_sonet_event_user_view WHERE USER_ID = ".$arUser2Group["USER_ID"]." AND ENTITY_TYPE = '".SONET_ENTITY_GROUP."' AND ENTITY_ID = ".$arUser2Group["GROUP_ID"], true);

			if ($bSendExclude)
			{
				if (in_array($arUser2Group["ROLE"], array(SONET_ROLES_MODERATOR, SONET_ROLES_USER)))
				{
					if ($arUser2Group["GROUP_VISIBLE"] == "Y")
					{
						$logID = CSocNetLog::Add(
							array(
								...
								"ENTITY_ID" 		=> $arUser2Group["USER_ID"],
								"EVENT_ID" 			=> "system_groups",
								"=LOG_DATE" 		=> $GLOBALS["DB"]->CurrentTimeFunction(),
								"TITLE_TEMPLATE"	=> false,
								"TITLE" 			=> "exclude_user",
								"MESSAGE" 			=> $arUser2Group["GROUP_ID"],
								"URL" 				=> false,
								"MODULE_ID" 		=> false,
								"CALLBACK_FUNC" 	=> false,
					...
					if (intval($logID) > 0 && intval($tmpID) <= 0)
					{
						$tmpID = $logID;
	...
	function DeleteNoDemand($userID)
	{
		...
		if ($bSuccess)
			$DB->Query("DELETE FROM b_sonet_event_user_view WHERE USER_ID = ".$userID, true);
	...
	function SendEvent($userGroupID, $mailTemplate = "SONET_INVITE_GROUP")
	{
		...
		$dbRelation = CSocNetUserToGroup::GetList(
			...
			false,
			false,
			array("ID", "USER_ID", "GROUP_ID", "ROLE", "DATE_CREATE", "MESSAGE", "INITIATED_BY_TYPE", "INITIATED_BY_USER_ID", "GROUP_NAME", "USER_NAME", "USER_LAST_NAME", "USER_EMAIL", "USER_LID", "GROUP_SITE_ID")
		);
		$arRelation = $dbRelation->Fetch();
		if (!$arRelation)
			return false;

		$siteID = $arRelation["GROUP_SITE_ID"];
	...
	function SendRequestToBeMember($userID, $groupID, $message, $RequestConfirmUrl = "", $bAutoSubscribe = true)
	{
		...
		if ($arGroup["OPENED"] == "Y")
		{
			CSocNetEventUserView::Entity2UserAdd(SONET_ENTITY_GROUP, $groupID, $userID,  SONET_ROLES_USER);
			if ($bAutoSubscribe)
	...
	function SendRequestToJoinGroup($senderID, $userID, $groupID, $message)
	{
		...
		$bCanInitiate = ($GLOBALS["USER"]->IsAdmin() || ($userRole
	...
	function ConfirmRequestToBeMember($userID, $groupID, $arRelationID, $bAutoSubscribe = true)
	{
		...
		$bUserIsMember = ($userRole && in_array($userRole, array(SONET_ROLES_OWNER, SONET_ROLES_MODERATOR, SONET_ROLES_USER)));
		$bCanInitiate = ($GLOBALS["USER"]->IsAdmin() || CSocNetUser::IsCurrentUserModuleAdmin() || ($userRole
		...
		for ($i = 0; $i < count($arRelationID); $i++)
		{
			...
			if (CSocNetUserToGroup::Update($arRelation["ID"], $arFields))
			{
				...
				CSocNetEventUserView::Entity2UserAdd(SONET_ENTITY_GROUP, $arRelation["GROUP_ID"], $arRelation["USER_ID"], SONET_ROLES_USER);
			}
			else
			{
				$errorMessage = "";
	...
	function UserConfirmRequestToBeMember($senderUserID, $relationID, $bAutoSubscribe = true)
	{
		global $APPLICATION;
		...
		$dbResult = CSocNetUserToGroup::GetList(
			...
			false,
			false,
			array("ID", "INITIATED_BY_USER_ID", "GROUP_ID", "GROUP_VISIBLE", "GROUP_SITE_ID", "GROUP_NAME")
		);

		if ($arResult = $dbResult->Fetch())
		{

			$site_id = $arResult["GROUP_SITE_ID"];
			if (strlen($site_id) <= 0)
				$site_id = SITE_ID;
			...
			{
				if ($bAutoSubscribe)
					CSocNetLogEvents::AutoSubscribe($senderUserID, SONET_ENTITY_GROUP, $arResult["GROUP_ID"]);
				...
				CSocNetEventUserView::Entity2UserAdd(SONET_ENTITY_GROUP, $arResult["GROUP_ID"], $senderUserID, SONET_ROLES_USER);
			}
			else
			{
				$errorMessage = "";
		...
		CSocNetUserToGroup::__SpeedFileCheckMessages($senderUserID);

		return true;
	}

	function UserRejectRequestToBeMember($senderUserID, $relationID)
	{
		global $APPLICATION;
		...
		$dbResult = CSocNetUserToGroup::GetList(
			...
			false,
			false,
			array("ID", "INITIATED_BY_USER_ID", "GROUP_NAME")
		);
	...
	function TransferModerator2Member($userID, $groupID, $arRelationID, $currentUserIsAdmin)
	{
		...
		for ($i = 0; $i < count($arRelationID); $i++)
		{
			...
			if (CSocNetUserToGroup::Update($arRelation["ID"], $arFields))
			{
				...
				$DB->Query("DELETE FROM b_sonet_event_user_view WHERE USER_ID = ".$arRelation["USER_ID"]." AND ENTITY_TYPE = '".SONET_ENTITY_GROUP."' AND ENTITY_ID = ".$arRelation["GROUP_ID"], true);
				CSocNetEventUserView::Entity2UserAdd(SONET_ENTITY_GROUP, $arRelation["GROUP_ID"], $arRelation["USER_ID"], SONET_ROLES_USER);
			}
			else
			{
	...
	function TransferMember2Moderator($userID, $groupID, $arRelationID, $currentUserIsAdmin)
	{
		...
		for ($i = 0; $i < count($arRelationID); $i++)
		{
			...
			if (CSocNetUserToGroup::Update($arRelation["ID"], $arFields))
			{
				...
				$DB->Query("DELETE FROM b_sonet_event_user_view WHERE USER_ID = ".$arRelation["USER_ID"]." AND ENTITY_TYPE = '".SONET_ENTITY_GROUP."' AND ENTITY_ID = ".$arRelation["GROUP_ID"], true);
				CSocNetEventUserView::Entity2UserAdd(SONET_ENTITY_GROUP, $arRelation["GROUP_ID"], $arRelation["USER_ID"], array(SONET_ROLES_USER, SONET_ROLES_MODERATOR));
			}
			else
			{
				$errorMessage = "";
	...
	function UnBanMember($userID, $groupID, $arRelationID, $currentUserIsAdmin)
	{
		...
		if ($bSuccess)
			CSocNetEventUserView::Entity2UserAdd(SONET_ENTITY_GROUP, $groupID, $arRelation["USER_ID"], SONET_ROLES_USER);
	...
	function DeleteRelation($userID, $groupID)
	{
		...
		if ($arResult = $dbResult->Fetch())
		{
			...
			if (CSocNetUserToGroup::Delete($arResult["ID"]))
			{
				...
				$GLOBALS["DB"]->Query("DELETE FROM b_sonet_event_user_view WHERE USER_ID = ".$userID." AND ENTITY_TYPE = '".SONET_ENTITY_GROUP."' AND ENTITY_ID = ".$groupID, true);
			}
			else
			{
				$errorMessage = "";
	...
	function __SpeedFileCreate($userID)
	{
		...
		$filePath = $_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/managed_cache/socnet/c/".IntVal($userID / 1000)."/";
		$fileName = $userID."_g";
	...
	function __SpeedFileDelete($userID)
	{
		...
		$fileName = $_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/managed_cache/socnet/c/".IntVal($userID / 1000)."/".$userID."_g";
