...
class CAllSocNetLogEvents
{
	...
	function GetSQL($user_id, $arMyEntities, $transport, $visible)
	{
		global $DB;
		...
		foreach($arMyEntities as $entity_type_tmp => $arMyEntity)
		{
			...
			{
				$strMyEntities[$entity_type_tmp] = "L.ENTITY_ID IN (".implode(",", $arMyEntity).")";
				$strNotMyEntities[$entity_type_tmp] = "(L.ENTITY_ID NOT IN (".implode(",", $arMyEntity).") AND L.ENTITY_TYPE = '".$entity_type_tmp."')";
			}
		}
		...
		{
			$strCBFilterEntityType = "L.ENTITY_TYPE IN (".implode(",", $arCBFilterEntityType).") AND ";
			$strNotCBFilterEntityType = "L.ENTITY_TYPE NOT IN (".implode(",", $arCBFilterEntityType).") OR ";
		}
		else
		{
		...
		EXISTS(
			SELECT ID 
			FROM b_sonet_log_events AS SLE 
			WHERE 
				SLE.USER_ID = ".$user_id." 
				AND SLE.ENTITY_TYPE = L.ENTITY_TYPE
				AND SLE.ENTITY_CB = 'N'
				AND SLE.ENTITY_ID = L.ENTITY_ID
				AND SLE.EVENT_ID = L.EVENT_ID
				".$transportFilter."
				".$visibleFilter."
		)
		OR 
		...
		if (count($strMyEntities) > 0)
		{
			foreach ($strMyEntities as $entity_type_tmp	=> $strMyEntity)
			{
				$strSQL .=	(strlen($strMyEntity) > 0 ?
								"
								(
									".$strNotMyEntities[$entity_type_tmp]."
									OR EXISTS(
										SELECT ID 
										FROM b_sonet_log_events AS SLE 
										WHERE 
