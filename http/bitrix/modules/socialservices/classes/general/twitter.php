...
class CSocServTwitter extends CSocServAuth
{
	...
	{
		$disabled = !function_exists("hash_hmac");
		return array(
			"ID" => self::ID,
	...
	{
		$url = $GLOBALS['APPLICATION']->GetCurPageParam('auth_service_id='.self::ID, array("logout", "auth_service_error", "auth_service_id"));
		return '<a href="javascript:void(0)" onclick="BX.util.popup(\''.htmlspecialchars(CUtil::JSEscape($url)).'\', 800, 450)" class="bx-ss-button twitter-button"></a><span class="bx-spacer"></span><span>'.GetMessage("socserv_tw_note").'</span>';
	}
	...
	public function Authorize()
	{
		...
		else
		{
			...
			if(($arResult = $tw->GetAccessToken()) !== false && $arResult["user_id"] <> '')
			{
				...
				$dbUsers = $GLOBALS["USER"]->GetList($by, $ord, array('XML_ID'=>$arResult["user_id"], 'EXTERNAL_AUTH_ID'=>self::ID));
		...
		$aRemove = array("logout", "auth_service_error", "auth_service_id", "oauth_token", "oauth_verifier");
...
class CTwitterInterface
{
	...
	public function GetRequestToken($callback)
	{
 	...
 	public function RedirectAuthUrl()
 	{
 		if(!$this->token)
 			return false;
 		LocalRedirect(self::AUTH_URL."?oauth_token=".urlencode($this->token), true);
 	}
...
AddEventHandler("socialservices", "OnAuthServicesBuildList", array("CSocServTwitter", "GetDescription"));
?>