...
while ($arResultItem = $dbResultList->NavNext(true, "f_"))
{
	...
	$row->AddField(
		"ID",
		'<a href="bizproc_task.php?id='.$f_ID.$s.'&back_url='.urlencode($APPLICATION->GetCurPage()).'" title="'.GetMessage("BPATL_VIEW").'">'.$f_ID.'</a>'
	);
	...
	$arActions[] = array(
		...
		"ACTION" => $lAdmin->ActionRedirect('bizproc_task.php?id='.$f_ID.$s.'&back_url='.urlencode($APPLICATION->GetCurPage()).''),
		"DEFAULT" => true
	);
