...
if (($arID = $lAdmin->GroupAction()) && $bizprocModulePermissions >= "W")
{
...
<form name="find_form" method="GET" action="<?echo $APPLICATION->GetCurPage()?>?">
	...
	<input type="hidden" name="view_document_url" value="<?= htmlspecialchars($viewDocumentUrl) ?>">
...
?>
	<tr>
		...
		<td><?echo CalendarPeriod("filter_modified_1", htmlspecialchars($filter_modified_1), "filter_modified_2", htmlspecialchars($filter_modified_2), "find_form", "Y")?></td>
	</tr>
	<tr>
		...
		<td><input type="text" name="filter_user_id" value="<?echo htmlspecialcharsex($filter_user_id)?>" size="3">&nbsp;<?
			...
			?><select name="filter_user_id1">
				...
				while ($arGrRes = $dbGrRes->GetNext())
					echo "<option value='".$arGrRes["USER_ID"]."'".($filter_user_id1 == $arGrRes["USER_ID"] ? " selected" : "").">(".htmlspecialcharsex($arGrRes["USER_LOGIN"].") ".$arGrRes["USER_NAME"]." ".$arGrRes["USER_LAST_NAME"])."</option>";
			?></select>
		</td>
	</tr>
