...
else
{
	...
	if (strlen($backUrl) <= 0)
		$backUrl = "/bitrix/admin/bizproc_task_list.php";
	...
	{
		$arErrorsTmp = array();
		if (CBPDocument::PostTaskForm($arTask, $allowAdminAccess ? $userId : $USER->GetID(), $_REQUEST + $_FILES, $arErrorsTmp, $USER->GetFullName()))
		{
			$showType = "Success";
			if (strlen($backUrl) > 0)
			{
	...
	<form method="post" name="task_form1" action="<?= GetPagePath(false, true) ?>" enctype="multipart/form-data">
		...
		<input type="hidden" name="workflow_id" value="<?= htmlspecialchars($arTask["WORKFLOW_ID"]) ?>">
