			...
			<select id="BPSId1S" size="13" style="width:100%" ondblclick="BPSVInsert(this.value)">
				<?foreach($arWorkflowParameters as $fieldId => $documentField):?>
					<?if($arFilter===false || in_array($documentFieldTypes[$documentField["Type"]]["BaseType"], $arFilter)):
						if($_POST['fieldType']!='user' && in_array($documentFieldTypes[$documentField["Type"]]["BaseType"], array('user', 'file')))
							$fieldId .= '_printable';
						?>
						<option value="{=<?=htmlspecialchars($arWorkflowTemplate[0]['Name'])?>:<?=$fieldId?>}<?if($_POST['fieldType']=='user')echo '; '?>"><?=htmlspecialchars($documentField['Name'])?></option>
					<?endif?>
				<?endforeach?>
			</select>
		</td>
				...
				<?foreach($arWorkflowVariables as $fieldId => $documentField):?>
					<?if($arFilter===false || in_array($documentFieldTypes[$documentField["Type"]]["BaseType"], $arFilter)):
						if($_POST['fieldType']!='user' && in_array($documentFieldTypes[$documentField["Type"]]["BaseType"], array('user', 'file')))
							$fieldId .= '_printable';
						?>
						<option value="{=Variable:<?=$fieldId?>}<?if($_POST['fieldType']=='user')echo '; '?>"><?=htmlspecialchars($documentField['Name'])?></option>
					<?endif?>
				<?endforeach?>
			</select>
		</td>
			...
			<select id="BPSId2S" size="13" style="width:100%" ondblclick="BPSVInsert(this.value)">
				<?foreach($documentFields as $fieldId => $documentField):?>
					<?if($arFilter===false || in_array($documentField["BaseType"], $arFilter)):?>
						<option value="{=Document:<?=$fieldId?>}"><?=htmlspecialchars($documentField['Name'])?></option>
					<?endif?>
				<?endforeach?>
			</select>
		</td>
	...
	<tr id="BPSId5" style="display:none">
		...
		function BPSKeyd(e)
		{
			...
			<select id="BPSId5S" size="<?=($_REQUEST['only_users'] == 'Y' ? 14 : 11)?>" style="width:100%" ondblclick="BPSVInsert(this.value)">
				...
				while ($i < $cnt)
				{
					$str = "SELECT ID, LOGIN, NAME, LAST_NAME, EMAIL FROM b_user WHERE ID IN (0";
					$cnt1 = min($cnt, $i + $mcnt);
					for ($j = $i; $j < $cnt1; $j++)
					...
					{
						$n = $user['NAME'].((strlen($user['NAME']) > 0 && strlen($user['LAST_NAME']) > 0) ? " " : "").$user['LAST_NAME'];
						if (strlen($n) <= 0)
							$n = $user['LOGIN'];
						?>
						<option value="<?= $n ?> [<?=$user['ID']?>]; "><?=$user['LAST_NAME']?> <?=$user['NAME']?> &lt;<?=$user['EMAIL']?>&gt; [<?=$user['ID']?>]</option>
						<?
					}
				}
				?>
...
function BPSVInsert(v)
{
	...
	else
	{
		...
		var toField = tdocument.getElementById('<?=AddSlashes(htmlspecialchars($_POST["fieldName"]))?>');

		toField.focus();
		if(tdocument.selection && tdocument.selection.createRange)
		{
			tdocument.selection.createRange().text = v;
		}
