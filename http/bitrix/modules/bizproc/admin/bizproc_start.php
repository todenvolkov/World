...
else
{
	...
	elseif ($showMode == "WorkflowParameters")
	{
		...
		<form method="post" name="start_workflow_form1" action="<?= GetPagePath(false, true) ?>" enctype="multipart/form-data">
			...
			<input type="hidden" name="document_type" value="<?= htmlspecialchars($documentType[2]) ?>">
			<input type="hidden" name="document_id" value="<?= htmlspecialchars($documentId[2]) ?>">
			...
			?>
			<tr>
				<td align="right" width="40%"><?= GetMessage("BPABS_NAME") ?>:</td>
				<td width="60%">
			...
			<tr>
				<td align="right" width="40%"><?= GetMessage("BPABS_DESCRIPTION") ?>:</td>
				<td width="60%">
			...
			foreach ($arWorkflowTemplates[$workflowTemplateId]["PARAMETERS"] as $parameterKey => $arParameter)
			{
				?>
				<tr>
					<td align="right" width="40%" valign="top"><?= $arParameter["Required"] ? "<span class=\"required\">*</span> " : ""?><?= htmlspecialchars($arParameter["Name"]) ?>:<?if (strlen($arParameter["Description"]) > 0) echo "<br /><small>".htmlspecialchars($arParameter["Description"])."</small><br />";?></td>
						...
						switch ($arParameter["Type"])
						{
							...
							case "select":
								...
								if (is_array($arParameter["Options"]) && count($arParameter["Options"]) > 0)
								{
									...
									{
										?><option value="<?= htmlspecialchars($key) ?>"<?= (!$arParameter["Multiple"] && $key == $arParametersValues[$parameterKey] || $arParameter["Multiple"] && is_array($arParametersValues[$parameterKey]) && in_array($key, $arParametersValues[$parameterKey])) ? " selected" : "" ?>><?= htmlspecialchars($value) ?></option><?
									}
								}
								?>
								</select><?
