		...
		<select name="target_state_name_1">
			...
			foreach ($arStates as $key => $value)
			{
				if ($key == $arCurrentValues["target_state_name"])
					$fl = true;
				?><option value="<?= htmlspecialchars($key) ?>"<?= ($key == $arCurrentValues["target_state_name"]) ? " selected" : "" ?>><?= $value ?></option><?
			}
			?>
		</select><br />
		<input type="text" id="id_target_state_name" name="target_state_name" value="<?= !$fl ? htmlspecialchars($arCurrentValues["target_state_name"]) : "" ?>">
