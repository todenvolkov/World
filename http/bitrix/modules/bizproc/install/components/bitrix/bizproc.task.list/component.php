...
if (strlen($arResult["FatalErrorMessage"]) <= 0)
{
	...
	while ($arRecord = $dbRecordsList->GetNext())
	{
		...
		if (count(array_intersect($arSelectFields, array("WORKFLOW_NAME", "WORKFLOW_STATE", "WORKFLOW_STARTED", "WORKFLOW_STARTED_BY"))) > 0)
		{
			...
			$arRecord["WORKFLOW_STARTED"] = $arState["STARTED"];
			...
			if (intval($arState["STARTED_BY"]) > 0)
			{
				...
				$arRecord["WORKFLOW_STARTED_BY"] = $arUserTmp["NAME"];
				if (strlen($arRecord["WORKFLOW_STARTED_BY"]) > 0)
					$arRecord["WORKFLOW_STARTED_BY"] .= " ";
				$arRecord["WORKFLOW_STARTED_BY"] .= $arUserTmp["LAST_NAME"];
				if (strlen($arRecord["WORKFLOW_STARTED_BY"]) <= 0)
					$arRecord["WORKFLOW_STARTED_BY"] = $arUserTmp["LOGIN"];
...
if ($arParams["SHOW_TRACKING"] == "Y")
{
	function __bwl_ParseStringParameterTmp1($matches, $documentType)
	{
		...
		if ($matches[1] == "user")
		{
			...
			{
				$result = htmlspecialchars(CBPHelper::ConvertUserToPrintableForm(intval(substr($user, $l))));
			}
			else
			{
