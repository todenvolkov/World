...
abstract class CBPActivity
{
	...
	public function GetVariable($name)
	{
		...
		if (array_key_exists($name, $rootActivity->arVariables))
			...
			throw new Exception(str_replace("#NAME#", htmlspecialchars($name), GetMessage("BPSWA_EMPTY_NAME")));
	}
	...
	function __get($name)
	{
		...
		{
			throw new Exception(str_replace("#NAME#", htmlspecialchars($name), GetMessage("BPCGACT_NO_PROPERTY")));
		}
	}
	...
	protected function CollectNestedActivities()
	{
		return null;
	}
	...
	protected function WriteToTrackingService($message = "", $modifiedBy = 0)
	{
		$trackingService = $this->workflow->GetService("TrackingService");
		$trackingService->Write($this->GetWorkflowInstanceId(), CBPTrackingType::Custom, $this->name, $this->executionStatus, $this->executionResult, ($this->IsPropertyExists("Title") ? $this->Title : ""), $message, $modifiedBy);
	}
