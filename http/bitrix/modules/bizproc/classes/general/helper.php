...
class CBPHelper
{
	private static function UsersArrayToStringInternal($arUsers, $arWorkflowTemplate, $arAllowableUserGroups)
	{
		if (is_array($arUsers))
		{
			...
			if (count($r) == 2)
			{
				...
				{
					if (in_array($r[0], array("Document", "Template", "Variable"))
		...
		else
		{
			...
			if ($userId > 0)
			{
				...
				if ($ar = $db->Fetch())
				{
					$str = $ar["NAME"].((strlen($ar["NAME"]) > 0 && strlen($ar["LAST_NAME"]) > 0) ? " " : "").$ar["LAST_NAME"];
					if (strlen($str) <= 0)
						$str = $ar["LOGIN"];
					$str = $str." [".$ar["ID"]."]";
	...
	public static function UsersStringToArray($strUsers, $documentType, &$arErrors, $callbackFunction = null)
	{
		...
		foreach ($arUsers as $user)
		{
			...
			{
				$bCorrectUser = true;
				$arResult[] = array($arMatches[1], $arMatches[2]);
				if (count($arUsers) == 1)
				{
					$arResult = $arResult[0];
					break;
			...
			else
			{
				...
				else
				{
					...
					elseif ($cnt > 1)
					{
						...
						$arErrors[] = array(
							"code" => "Ambiguous",
							"message" => str_replace("#USER#", htmlspecialchars($user), GetMessage("BPCGHLP_AMBIGUOUS_USER")),
						);
					}
			...
			if (!$bCorrectUser)
			{
				if ($bNotFoundUser)
				{
					$arErrors[] = array(
						"code" => "NotFound",
						"message" => str_replace("#USER#", htmlspecialchars($user), GetMessage("BPCGHLP_INVALID_USER")),
					);
				}
			}
		}
				...
				if (is_array($result))
				{
					...
					foreach ($result as $r)
					{
						...
						if ($arImg = $dbImg->Fetch())
							$newResult[] = "[url=/bitrix/bizproc_show_file.php?f=".htmlspecialchars($arImg["FILE_NAME"])."&i=".$r."]".htmlspecialchars($arImg["ORIGINAL_NAME"])."[/url]";
					}
				}
				else
				{
					...
					if ($arImg = $dbImg->Fetch())
						$newResult = "[url=/bitrix/bizproc_show_file.php?f=".htmlspecialchars($arImg["FILE_NAME"])."&i=".$result."]".htmlspecialchars($arImg["ORIGINAL_NAME"])."[/url]";
				}
				break;
	...
	public function ConvertUserToPrintableForm($userId)
	{
	...
	public function GetJSFunctionsForFields($objectName, $arDocumentFields, $arDocumentFieldTypes)
	{
		...
		<?= $objectName ?>.GetGUIFieldEdit = function(field, value, showAddButton, inputName)
		{
			...
			else if (type == "datetime" || type == "date")
			{
				...
				s += '<img src="<?= ADMIN_THEMES_PATH ?>/<?= ADMIN_THEME_ID ?>/images/calendar/icon.gif" alt="<?= GetMessage("BPCGHLP_CALENDAR") ?>" class="calendar-icon" onclick="jsAdminCalendar.Show(this, \'' + inputName + '\', \'\', \'\', ' + ((type == "datetime") ? 'true' : 'false') + ', <?= time() + date("Z") ?>);" onmouseover="this.className+=\' calendar-icon-hover\';" onmouseout="this.className = this.className.replace(/\s*calendar-icon-hover/ig, \'\');">';
				s += '</a></span>';
			}
			else // type == "S"
			{
	...
	static public function GetGUIFieldEdit($documentType, $formName, $fieldName, $fieldValue, $arDocumentField, $bAllowSelection)
	{
		...
		if (!array_key_exists("CBPHelperAddShowParameterInit", $GLOBALS))
		{
			$GLOBALS["CBPHelperAddShowParameterInit"] = 1;
		...
		{
			if (!array_key_exists("CBPHelperCloneRowPrinted", $GLOBALS) && $arDocumentField["Multiple"])
			{
				$GLOBALS["CBPHelperCloneRowPrinted"] = 1;
				?>
				<script language="JavaScript">
				<!--
				function CBPHelperCloneRow(tableID)
				{
			...
			if ($arDocumentField["Multiple"])
				echo '<table width="100%" border="0" cellpadding="2" cellspacing="2" id="CBPHelper_'.$fieldName.'_Table">';
			...
			foreach ($fieldValue as $value)
			{
				...
				switch ($fieldType)
				{
					...
					case "datetime":
						...
						echo CAdminCalendar::CalendarDate($fieldNameName, $value, 19, ($fieldType == "date"));
						break;
					default:
				...
				if ($bAllowSelection)
				{
					if (!in_array($fieldType, array("file", "bool")))
					{
						?><input type="button" value="..." onclick="BPAShowSelector('<?= $fieldNameId ?>', '<?= CUtil::JSEscape($arDocumentField["BaseType"]) ?>');"><?
					}
				}
			...
			if ($arDocumentField["Multiple"])
				echo '<input type="button" value="'.GetMessage("BPCGHLP_ADD").'" onclick="CBPHelperCloneRow(\'CBPHelper_'.$fieldName.'_Table\')"/><br />';
			...
			if ($bAllowSelection)
			{
				if (in_array($fieldType, array("file", "bool")))
				{
					...
					?>">
					<input type="button" value="..." onclick="BPAShowSelector('id_<?= $fieldNameId ?>_1', '<?= CUtil::JSEscape($arDocumentField["BaseType"]) ?>');">
					<?
				}
			}
		}
	...
	static function SetGUIFieldEdit($documentType, $fieldName, $arRequest, &$arErrors, $arDocumentField = null)
	{
		...
		elseif (array_key_exists($fieldName, $arRequest))
		{
			...
			if (!is_array($arValue) || is_array($arValue) && ($fieldType == "file") && array_key_exists("name", $arValue))
				...
				elseif ($fieldType == "double")
				{
					if (strlen($value) > 0)
					{
						...
						else
						{
							...
							$arErrors[] = array(
								"code" => "ErrorValue",
								"message" => str_replace("#NAME#", $arDocumentField["Name"], GetMessage("BPCGWTL_INVALID11")),
								"parameter" => $fieldName,
							);
						}
					}
