...
class CAllBPWorkflowTemplateLoader
{
	...
	private function ValidateTemplate($arActivity, $user)
	{
		...
		if (array_key_exists("Children", $arActivity) && count($arActivity["Children"]) > 0)
		{
			...
			{
				$arErrors = $arErrors + CBPActivity::CallStaticMethod($arActivity["Type"], "ValidateChild", array($arChildActivity["Type"], $bFirst));
				$bFirst = false;
	...
	private static function ParseDocumentTypeStates($arTemplatesListItem)
	{
		...
		if (is_array($result["STATE_PERMISSIONS"]))
		{
			...
			{
				if (is_array($result["STATE_PERMISSIONS"][$key]) && count($result["STATE_PERMISSIONS"][$key]) == 2)
				{
					$arKeys1 = array_keys($result["STATE_PERMISSIONS"][$key]);
					if ($arKeys1[0] == 0 && $arKeys1[1] == 1 && $result["STATE_PERMISSIONS"][$key][0] == "Variable")
					{
						if (array_key_exists($result["STATE_PERMISSIONS"][$key][1], $arTemplatesListItem["VARIABLES"]))
							$result["STATE_PERMISSIONS"][$key] = $arTemplatesListItem["VARIABLES"][$result["STATE_PERMISSIONS"][$key][1]]["Default"];
					}
				}
			}
		}
	...
	public static function CheckWorkflowParameters($arTemplateParameters, $arPossibleValues, $documentType, &$arErrorsTmp)
	{
		$arErrorsTmp = array();
		...
		*/
		...
		{
			if (array_key_exists($parameterKey, $arPossibleValues))
			{
				$arValue = $arPossibleValues[$parameterKey];
				if (!is_array($arValue))
				...
				foreach ($arValue as $value)
				{
					switch ($arParameter["Type"])
					{
						case "int":
							if (strlen($value) > 0)
							{
								...
								else
								{
									...
									$arErrorsTmp[] = array(
										"code" => "ErrorValue",
										"message" => str_replace("#NAME#", $arParameter["Name"], GetMessage("BPCGWTL_INVALID1")),
						...
						case "double":
							if (strlen($value) > 0)
							{
								...
								else
								{
									...
									$arErrorsTmp[] = array(
										"code" => "ErrorValue",
										"message" => str_replace("#NAME#", $arParameter["Name"], GetMessage("BPCGWTL_INVALID2")),
										"parameter" => $parameterKey,
									);
								}
							}
						...
						case "select":
							...
							elseif (!array_key_exists($value, $arParameter["Options"]))
							{
								$value = null;
								$arErrorsTmp[] = array(
									"code" => "ErrorValue",
									"message" => str_replace("#NAME#", $arParameter["Name"], GetMessage("BPCGWTL_INVALID3")),
									"parameter" => $parameterKey,
								);
							}
							break;
						case "bool":
							if ($value !== true && $value !== false)
							{
								if (strlen($value) > 0)
								{
									...
									elseif (in_array($value, array("n", "no", "false")))
									{
										$value = false;
									}
									else
									{
										...
										$arErrorsTmp[] = array(
											"code" => "ErrorValue",
											"message" => str_replace("#NAME#", $arParameter["Name"], GetMessage("BPCGWTL_INVALID4")),
											"parameter" => $parameterKey,
										);
									}
								}
						...
						case "datetime":
							if (strlen($value) > 0)
							{
								$format = CLang::GetDateFormat(strtolower($arParameter["Type"]) == "date" ? "SHORT" : "FULL", false);
								if (!CheckDateTime($value, $format))
								{
									$value = null;
									$arErrorsTmp[] = array(
										"code" => "ErrorValue",
										"message" => str_replace(array("#NAME#", "#FORMAT#"), array($arParameter["Name"], $format), GetMessage("BPCGWTL_INVALID5")),
										"parameter" => $parameterKey,
									);
								}
							}
						...
						case "user":
							...
							{
								$arErrorsTmp1 = array();
								$value = CBPHelper::UsersStringToArray($value, $documentType, $arErrorsTmp1);
								if (count($arErrorsTmp1) > 0)
									$arErrorsTmp = $arErrorsTmp + $arErrorsTmp1;
							}
							else
							{
								$value = null;
						...
						default:
							$arErrorsTmp[] = array(
								"code" => "UnknownType",
								"message" => str_replace("#NAME#", $arParameter["Name"], GetMessage("BPCGWTL_INVALID7")),
								"parameter" => $parameterKey,
							);
					}

					if ($arParameter["Multiple"])
					{
						if (!is_array($arWorkflowParameters[$parameterKey]))
							$arWorkflowParameters[$parameterKey] = array();
						if ($value != null)
						{
							if ($arParameter["Type"] == "user")
								$arWorkflowParameters[$parameterKey] += $value;
							else
								$arWorkflowParameters[$parameterKey][] = $value;
						}
					}
			...
			{
				$arWorkflowParameters[$parameterKey] = null;
			}
	...
	public static function ExportTemplate($id, $bCompress = true)
	{
		...
		if ($ar = $db->Fetch())
		{
			...
			$arDocumentFields = $documentService->GetDocumentFields($ar["DOCUMENT_TYPE"]);
...
*/
	public static function ImportTemplate($id, $documentType, $autoExecute, $name, $description, $datum)
	{
		...
		/*if (!self::WalkThroughWorkflowTemplate($datumTmp["TEMPLATE"], array("CBPWorkflowTemplateLoader", "ImportTemplateChecker"), $user))
			return false;)*/
