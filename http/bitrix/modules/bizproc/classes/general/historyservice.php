	...
	extends CBPRuntimeService
{
	...
	private static function GenerateFilePath($documentId)
	{
		...
		$dest = "/".(COption::GetOptionString("main", "upload_dir", "upload"))."/bizproc/";
	...
	{
		$h = new CBPHistoryService();
		return $h->GetTemplatesList($arOrder, $arFilter, $arGroupBy, $arNavStartParams, $arSelectFields);
	}
	...
	public static function PrepareFileForHistory($documentId, $arFileId, $historyIndex)
	{
		...
		foreach ($arFileId as $fileId)
		{
			...
			CheckDirPath($_SERVER["DOCUMENT_ROOT"].$dest."/".$historyIndex."/");
			if (!copy($_SERVER["DOCUMENT_ROOT"].$ar["SRC"], $_SERVER["DOCUMENT_ROOT"].$dest."/".$historyIndex."/".$ar["FILE_NAME"]))
				continue;

			$result[] = $dest."/".$historyIndex."/".$ar["FILE_NAME"];
		}
