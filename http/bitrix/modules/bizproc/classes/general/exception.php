	...
	extends CBPArgumentException
{
	...
	{
		if (strlen($message) <= 0)
			$message = str_replace("#PARAM#", htmlspecialchars($paramName), GetMessage("BPCGERR_NULL_ARG"));
	...
	extends CBPArgumentException
{
	...
	public function __construct($paramName, $actualValue = null, $message = "")
	{
		...
		{
			if ($actualValue === null)
				$message = str_replace("#PARAM#", htmlspecialchars($paramName), GetMessage("BPCGERR_INVALID_ARG"));
			else
				$message = str_replace(array("#PARAM#", "#VALUE#"), array(htmlspecialchars($paramName), htmlspecialchars($actualValue)), GetMessage("BPCGERR_INVALID_ARG1"));
		}
	...
	extends CBPArgumentException
{
	...
	public function __construct($paramName, $correctType = null, $message = "")
	{
		...
		{
			if ($correctType === null)
				$message = str_replace("#PARAM#", htmlspecialchars($paramName), GetMessage("BPCGERR_INVALID_TYPE"));
			else
				$message = str_replace(array("#PARAM#", "#VALUE#"), array(htmlspecialchars($paramName), htmlspecialchars($correctType)), GetMessage("BPCGERR_INVALID_TYPE1"));
		}
