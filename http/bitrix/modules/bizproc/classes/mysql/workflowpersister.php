		...
		if ($arResult = $dbResult->Fetch())
		{
			if ($arResult["UPDATEABLE"] == "Y")
			{
				$DB->Query(
					...
					"	OWNER_ID = '".$DB->ForSql($this->serviceInstanceId)."', ".
					"	OWNED_UNTIL = '".$DB->FormatDate(date($GLOBALS["DB"]->DateFormatToPHP(FORMAT_DATETIME), $this->GetOwnershipTimeout()), CLang::GetDateFormat("FULL", false), "Y-M-D H:I:S")."' ".
					"WHERE ID = '".$DB->ForSql($instanceId)."'"
				);
		...
		else
		{
			...
			if ($arResult = $dbResult->Fetch())
			{
				if ($arResult["UPDATEABLE"] == "Y")
				{
					$DB->Query(
						...
						"	OWNED_UNTIL = ".($bUnlocked ? "NULL" : "'".$DB->FormatDate(date($GLOBALS["DB"]->DateFormatToPHP(FORMAT_DATETIME), $this->GetOwnershipTimeout()), CLang::GetDateFormat("FULL", false), "Y-M-D H:I:S")."'")." ".
						"WHERE ID = '".$DB->ForSql($id)."' "
					);
				}
				else
			...
			else
			{
				$DB->Query(
					...
					"VALUES ('".$DB->ForSql($id)."', '".$DB->ForSql($buffer)."', ".intval($status).", ".$DB->CurrentTimeFunction().", ".($bUnlocked ? "NULL" : "'".$DB->ForSql($this->serviceInstanceId)."'").", ".($bUnlocked ? "NULL" : "'".$DB->FormatDate(date($GLOBALS["DB"]->DateFormatToPHP(FORMAT_DATETIME), $this->GetOwnershipTimeout()), CLang::GetDateFormat("FULL", false), "Y-M-D H:I:S")."'").")"
				);
			}
		}
	}
