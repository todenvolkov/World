...
?>
	<tr>
		...
		<td width="60%"><?
			...
			?><a href="workflow_edit.php?lang=<?=LANG?>&ID=<?=$zr["ID"]?>" title="<?=GetMessage('FLOW_VIEW_DOC_ALT')?>"><?=$str_DOCUMENT_ID?></a><?
			else :
			?><?=$str_DOCUMENT_ID?><?
			endif;
		?></td>
...
?>
	...
	<tr>
		...
		<td><a href="<?=htmlspecialchars($zr["FILENAME"])?>" title="<?=GetMessage('FLOW_VIEW_FILE_ALT')?>"><?=htmlspecialchars($zr["FILENAME"])?></a></td>
	</tr>
	<tr>
		<td><?=GetMessage("FLOW_DOCUMENT_TITLE")?></td>
		<td><?=htmlspecialchars($zr["TITLE"])?></td>
	</tr>
	<tr>
		<td><?=GetMessage("FLOW_DOCUMENT_DATE_ENTER")?></td>
		<td><?=$zr["DATE_ENTER"]?>&nbsp;&nbsp;[<a href="user_edit.php?ID=<?=$zr["ENTERED_BY"]?>&lang=<?=LANG?>" title="<?=GetMessage('FLOW_USER_ALT')?>"><?=$zr["ENTERED_BY"]?></a>]&nbsp;<?echo htmlspecialchars($zr["EUSER_NAME"])?></td>
	</tr>
	<tr>
		<td><?=GetMessage("FLOW_DOCUMENT_DATE_MODIFY")?></td>
		<td><?=$zr["DATE_MODIFY"]?>&nbsp;&nbsp;[<a href="user_edit.php?ID=<?=$zr["MODIFIED_BY"]?>&lang=<?=LANG?>"  title="<?=GetMessage('FLOW_USER_ALT')?>"><?=$zr["MODIFIED_BY"]?></a>]&nbsp;<? echo htmlspecialchars($zr["MUSER_NAME"])?></td>
	</tr>
	<? if (strlen($zr["DATE_LOCK"])>0) : ?>
	<tr>
		<td><?=GetMessage("FLOW_DOCUMENT_DATE_LOCK")?></td>
		<td><?=$zr["DATE_LOCK"]?>&nbsp;&nbsp;[<a href="user_edit.php?ID=<?=$zr["LOCKED_BY"]?>&lang=<?=LANG?>" title="<?=GetMessage('FLOW_USER_ALT')?>"><?=$zr["LOCKED_BY"]?></a>]&nbsp;<? echo htmlspecialchars($zr["LUSER_NAME"])?>&nbsp;<?if ($zr["LOCKED_BY"]==$USER->GetID()):?><span class="required">(!)</span><?endif;?></td>
	</tr>
	<? endif; ?>
	<tr>
		<td><?=GetMessage("FLOW_DOCUMENT_STATUS")?></td>
		<td>[<a href="workflow_status_edit.php?ID=<?=$zr["STATUS_ID"]?>&lang=<?=LANG?>" title="<?=GetMessage('FLOW_STATUS_ALT')?>"><?=$zr["STATUS_ID"]?></a>]&nbsp;<? echo htmlspecialchars($zr["STATUS_TITLE"])?></td>
	</tr>
<?
$tabControl->EndTab();
endif;
$tabControl->Buttons();
$backurl = "'workflow_history_list.php?lang=".LANGUAGE_ID."'";
?><input type="button" name="" value="<?=GetMessage('FLOW_BACK')?>" onClick="window.location=<?=$backurl?>">
<?$tabControl->End();?>
<?require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php"); ?>
