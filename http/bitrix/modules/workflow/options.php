...
?>
	...
	<tr>
		...
		elseif($type[0]=="text"):
			...
			if (strlen($Option[4])>0) :
				?>&nbsp;<label for="<?echo htmlspecialchars($Option[0])?>_clear"><?=GetMessage("FLOW_CLEAR")?>:</label><input type="checkbox" name="<?echo htmlspecialchars($Option[0])?>_clear" id="<?echo htmlspecialchars($Option[0])?>_clear" value="Y"><?
			endif;
	...
	<?endfor;?>
	<tr>
		<td valign="top"><?echo GetMessage("FLOW_ADMIN")?></td>
		<td valign="middle"><?echo SelectBox("WORKFLOW_ADMIN_GROUP_ID", CGroup::GetDropDownList(""), GetMessage("MAIN_NO"), htmlspecialchars($WORKFLOW_ADMIN_GROUP_ID));?></td>
	</tr>

<?$tabControl->BeginNextTab();?>
<?require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/admin/group_rights.php");?>
<?$tabControl->Buttons();?>
<input <?if ($WORKFLOW_RIGHT<"W") echo "disabled" ?> type="submit" name="Update" value="<?=GetMessage("FLOW_SAVE")?>">
