...
?>
		...
		<tr>
			...
			<td valign="top" width="50%"><?
					...
					?>
						...
						?>&nbsp;<label for="<?echo htmlspecialchars($Option[0])?>_clear"><?=GetMessage("AD_CLEAR")?>:</label><input type="checkbox" name="<?echo htmlspecialchars($Option[0])?>_clear" id="<?echo htmlspecialchars($Option[0])?>_clear" value="Y"><?
						endif;
						?><?
						if (strlen($Option[4])>0) :
	...
	?>
		<tr>
			...
			<td valign="middle"><input type="text" size="30" maxlength="255" value="<?=$UPLOAD_SUBDIR?>" name="UPLOAD_SUBDIR"><br>
				<input type="checkbox" name="move_loaded_banners" id="move_loaded_banners" value="Y"><label for="move_loaded_banners"><?=GetMessage("AD_MOVE_LOADED_BANNERS")?></label></td>
		</tr>
<?$tabControl->BeginNextTab();?>
<?require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/admin/group_rights.php");?>
<?$tabControl->Buttons();?>
