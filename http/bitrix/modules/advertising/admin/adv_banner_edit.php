...
<input type="hidden" name="CONTRACT_ID" value="<?=htmlspecialchars($arContract["ID"])?>">
<?if(!$bCopy):?>
...
?>
	...
	<tr valign="top">
		...
		<td width="60%">[<a title="<?=GetMessage("AD_CONTRACT_SETTINGS")?>" href="adv_contract_edit.php?ID=<?=$arContract["ID"]?>&action=view&lang=<?=LANGUAGE_ID?>"><?=$arContract["ID"]?></a>] <?=htmlspecialchars($arContract["NAME"])?></td>
	</tr>
	...
	<tr valign="top">
		...
		<td><?
			if ($isEditMode) :
				...
				{
					$ref[] = "[".$arType["SID"]."] ".htmlspecialchars($arType["NAME"]);
					$ref_id[] = $arType["SID"];
				}
				...
				echo "[<a href='adv_type_edit.php?SID=".urlencode($str_TYPE_SID)."&lang=".LANGUAGE_ID."&action=view' title='".GetMessage("ADV_TYPE_VIEW")."'>".htmlspecialchars($str_TYPE_SID)."</a>] ".$str_TYPE_NAME;
			endif;
			?></td>
	</tr>
...
<?endif?>
	...
	function DisableFixClick()
	{
		...
		{
			document.getElementById("MAX_CLICK_COUNT").disabled = true;
			document.getElementById("RESET_CLICK_COUNT").disabled = true;
		}
		else
		{
			document.getElementById("MAX_CLICK_COUNT").disabled = false;
			document.getElementById("RESET_CLICK_COUNT").disabled = false;
		}
		...
		<td><?
			if (is_array($arrSTAT_ADV) && count($arrSTAT_ADV)>0)
			{
				foreach ($arrSTAT_ADV as $aid)
				{
					...
					echo htmlspecialchars($ref[$key])."<br>";
				}
			}
			else
				echo GetMessage("ADV_NOT_SET");
...
?>
	...
	if ($isEditMode):?>
		<tr valign="top">
			<td>event2:</td>
			<td><input type="text" name="STAT_EVENT_2" id="STAT_EVENT_2" maxlength="255" size="30" value="<?=$str_STAT_EVENT_2?>" onfocus="t=this" <?if($FIX_STAT!="Y") echo "disabled";?>><br><?=GetMessage("AD_EVENT12")?></td>
		</tr><?
	else :
	...
	if ($isEditMode):
		...
		<tr valign="top">
			<td>event3:</td>
			<td><input type="text" name="STAT_EVENT_3" id="STAT_EVENT_3" maxlength="255"  value="<?=$str_STAT_EVENT_3?>" onfocus="t=this" style="width:80%;" <?if($FIX_STAT!="Y") echo "disabled";?>><br><?=GetMessage("AD_EVENT3")?>
