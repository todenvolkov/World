			...
			if (!function_exists("ImageCreate")) :
				...
				foreach($arShow as $ctype) :
					...
					<table cellspacing=0 cellpadding=0 class="graph">
						...
						<tr>
							<td valign="top"><img class="graph" src="/bitrix/admin/adv_diagram.php?<?=GetFilterParams($FilterArr)?>&diameter=<?echo $diameter?>&diagram_type=<?echo $diagram_type?>&counter_type=<?echo $counter_type?>" width="<?echo $diameter?>" height="<?echo $diameter?>"></td>
							<td valign="top">
								<table cellpadding=0 cellspacing=0 border=0 class="legend">
									...
									<tr>
										...
										<td  nowrap><?echo "[<a href='/bitrix/admin/adv_banner_edit.php?ID=".$arrS["ID"]."&lang=".LANGUAGE_ID."&action=view' title='".GetMessage("AD_BANNER_VIEW")."'>".$arrS["ID"]."</a>] ".$arrS["NAME"];?>
										</td>
									</tr>
									<?
										endif;
