...
if (strlen($strError)>0)
{
	...
	$str_SHOW_PAGE = htmlspecialchars($SHOW_PAGE);
	$str_NOT_SHOW_PAGE = htmlspecialchars($NOT_SHOW_PAGE);
}
...
?>
	...
	if ($ID>0) :
		...
		$lamp = '<div class="lamp-'.$str_LAMP.'" title="'.$lamp_alt.'"></div>';
		?>
		<tr valign="top">
	...
	<tr valign="top">
		...
		if (intval($str_CREATED_BY)>0) :
			...
			$arUser = $rsUser->Fetch();
			echo "&nbsp;&nbsp;[<a href='/bitrix/admin/user_edit.php?ID=".$str_CREATED_BY."&lang=".LANGUAGE_ID."' title='".GetMessage("AD_USER_ALT")."'>".$str_CREATED_BY."</a>]&nbsp;(".htmlspecialchars($arUser["LOGIN"]).") ".htmlspecialchars($arUser["NAME"])." ".htmlspecialchars($arUser["LAST_NAME"]);
		endif;
		?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top">
		...
		if (intval($str_MODIFIED_BY)>0) :
			...
			$arUser = $rsUser->Fetch();
			echo "&nbsp;&nbsp;[<a href='/bitrix/admin/user_edit.php?ID=".$str_MODIFIED_BY."&lang=".LANGUAGE_ID."' title='".GetMessage("AD_USER_ALT")."'>".$str_MODIFIED_BY."</a>]&nbsp;(".htmlspecialchars($arUser["LOGIN"]).") ".htmlspecialchars($arUser["NAME"])." ".htmlspecialchars($arUser["LAST_NAME"]);
		endif;
		?></td>
	</tr>
	<?endif;?>
	...
	<?else:?>
	<tr valign="top">
		<td><?=GetMessage("AD_SHOW_INTERVAL")." (".CSite::GetDateFormat("SHORT")."):"?></td>
		<td><?
		echo CalendarPeriod("DATE_SHOW_FROM", htmlspecialchars($str_DATE_SHOW_FROM), "DATE_SHOW_TO", htmlspecialchars($str_DATE_SHOW_TO), "form1", "N");?></td>
	</tr>
	<?endif;?>
...
<?$tabControl->BeginNextTab();?>
		...
		<td width="60%"><?
			if ($isEditMode) :
				...
				while(list($sid, $arrS) = each($arrSites)):
					...
					?>
					<input type="checkbox" name="arrSITE[]" value="<?=htmlspecialchars($sid)?>" id="site_<?=htmlspecialchars($sid)?>" <?=$checked?> <?=$disabled?>>
					<?echo '[<a href="/bitrix/admin/site_edit.php?LID='.urlencode($sid).'&lang='.LANGUAGE_ID.'" title="'.GetMessage("AD_SITE_ALT").'">'.htmlspecialcharsex($sid).'</a>]&nbsp;<label for="site_'.htmlspecialchars($sid).'">'.htmlspecialcharsex($arrS["NAME"])?></label>
					<br>
					<?
				endwhile;
	...
	<tr valign="top">
		...
		<td width="60%"><input name="arrTYPE[]" type="checkbox" value="ALL" onclick="OnSelectAll_typies(this.checked)" <?if ($ID>0 && in_array("ALL", $arrTYPE) || $ID<=0) echo 'checked="checked"'?> id="alltypies"><label for="alltypies"><?=GetMessage("AD_ALL_TYPIES")?></label><br><img src="/bitrix/images/1.gif" width="1" height="3"><br>
		<?
		$rsType = CAdvType::GetList($v1="s_sort", $v2="asc", array("ACTIVE" => "Y"), $v3);
		$i = 0;
		while ($arType = $rsType->Fetch())
		{
			?><input <?if ($ID > 0 && in_array($arType["SID"], $arrTYPE)) echo "checked"?> type="checkbox" name="arrTYPE[]" value="<?=htmlspecialchars($arType["SID"])?>" id="arType_<?=$i?>"><?echo "[<a href='/bitrix/admin/adv_type_edit.php?lang=".LANGUAGE_ID."&SID=".$arType["SID"]."&action=view' title='".GetMessage("AD_TYPE_ALT")."'>".$arType["SID"]."</a>] "."<label for='".$arType."_".$i."'>".$arType["NAME"]?></label><br>
			<?
			$i++;
		}
	...
	<tr valign="top">
		...
		<td><input type="text" name="MAX_VISITOR_COUNT" size="8" value="<?=$str_MAX_VISITOR_COUNT?>"><?if ($ID>0):?>&nbsp;<?=GetMessage("AD_VISITORS_2")?>&nbsp;<?=intval($str_VISITOR_COUNT)?><?endif;?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top">
		...
		<td><input type="text" name="MAX_SHOW_COUNT" size="8" value="<?=$str_MAX_SHOW_COUNT?>"><?if ($ID>0):?>&nbsp;<?=GetMessage("AD_SHOW_COUNT")?>&nbsp;<?=intval($str_SHOW_COUNT)?><?endif;?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top">
		...
		<td><input type="text" name="MAX_CLICK_COUNT" size="8" value="<?=$str_MAX_CLICK_COUNT?>"><?if ($ID>0):?>&nbsp;<?=GetMessage("AD_CLICKED")?>&nbsp;<?=intval($str_CLICK_COUNT)?><?endif;?></td>
	</tr>
	<?endif;?>
...
<?$tabControl->BeginNextTab();?>
	...
	<tr valign="top">
		<td width="40%"><?=GetMessage("AD_SHOW_PAGES");?></td>
		<td width="60%"><textarea name="SHOW_PAGE" cols="45" rows="6" wrap="OFF"><?=$str_SHOW_PAGE?></textarea><br><?=GetMessage("AD_SHOW_PAGES_ALT")?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top">
		<td><?=GetMessage("AD_NOT_SHOW_PAGES");?></td>
		<td><textarea name="NOT_SHOW_PAGE" cols="45" rows="6" wrap="OFF"><?=$str_NOT_SHOW_PAGE?></textarea><br><?=GetMessage("AD_NOT_SHOW_PAGES_ALT")?></td>
	</tr>
	<?endif;?>
...
<?$tabControl->BeginNextTab();?>
	...
	?>
	<tr valign="top">
		<td width="40%"><?=GetMessage("AD_VIEW_STATISTICS")?><br><IMG SRC="/bitrix/images/advertising/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
		<td width="60%"><?
	...
	<?if ($isEditRightsMode):?>
	<tr valign="top">
		<td><?=GetMessage("AD_MANAGE_BANNERS")?><br><IMG SRC="/bitrix/images/advertising/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
		<td><?
	...
	<?if ($isEditMode && ($isAdmin || ($isDemo && !$isOwner))):?>
	<tr valign="top">
		<td><?=GetMessage("AD_EDIT_CONTRACT")?><br><IMG SRC="/bitrix/images/advertising/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
...
<?$tabControl->BeginNextTab();?>
	<?if ($isEditMode) :?>
	<tr>
		<td colspan="2" align="center"><textarea style="width:85%" name="ADMIN_COMMENTS" rows="7" wrap="VIRTUAL"><?echo $str_ADMIN_COMMENTS?></textarea></td>
	</tr>
	<?else:?>
	<tr>
