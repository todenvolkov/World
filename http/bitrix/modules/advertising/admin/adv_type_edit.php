...
?>
	...
	<tr valign="top">
		...
		if (intval($str_CREATED_BY)>0) :
			...
			$arUser = $rsUser->Fetch();
			echo "&nbsp;&nbsp;[<a href='/bitrix/admin/user_edit.php?ID=".$str_CREATED_BY."' title='".GetMessage("AD_USER_ALT")."'>".$str_CREATED_BY."</a>]&nbsp;(".htmlspecialchars($arUser["LOGIN"]).") ".htmlspecialchars($arUser["NAME"])." ".htmlspecialchars($arUser["LAST_NAME"]);
		endif;
		?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top">
		...
		if (intval($str_MODIFIED_BY)>0) :
			...
			$arUser = $rsUser->Fetch();
			echo "&nbsp;&nbsp;[<a href='/bitrix/admin/user_edit.php?ID=".$str_MODIFIED_BY."' title='".GetMessage("AD_USER_ALT")."'>".$str_MODIFIED_BY."</a>]&nbsp;(".htmlspecialchars($arUser["LOGIN"]).") ".htmlspecialchars($arUser["NAME"])." ".htmlspecialchars($arUser["LAST_NAME"]);
		endif;
		?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top">
		<td><?
			if ($isEditMode) :
				?><span class="required">*</span><?=GetMessage("AD_SID")?><br><?=GetMessage("AD_SID_ALT")?><?
			else :
