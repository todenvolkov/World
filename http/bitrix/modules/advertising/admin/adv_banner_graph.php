...
if (!function_exists("ImageCreate")) :
	...
	<table border="0" cellspacing="2" cellpadding="10" class="graph">
		<tr>
			<td><img src="/bitrix/admin/adv_graph.php?<?=GetFilterParams($FilterArr)?>&width=<?=$width?>&height=<?echo $height?>&lang=<?=LANGUAGE_ID?>" width="<?=$width?>" height="<?=$height?>"></td>
		</tr>
		<tr>
			<td>
			<table cellpadding="3" cellspacing="1" border="0" class="list-table">
				...
				//echo "<pre>"; print_r($arrLegend); echo "</pre>";
				//echo "<pre>"; print_r($arShow); echo "</pre>";
				//echo "<pre>"; print_r($arrLegend); echo "</pre>";
				...
				<tr valign="center">
					...
					switch ($arrS["TYPE"]) :
						...
						case "BANNER" :
							...
							<td nowrap width="100%"><img src="/bitrix/images/1.gif" width="3" height="1"><?
								if ($arrS["COUNTER_TYPE"]=="DETAIL") :
									echo "[<a href='/bitrix/admin/adv_banner_edit.php?ID=".$arrS["ID"]."&lang=".LANGUAGE_ID."&action=view' title='".GetMessage("AD_BANNER_VIEW")."'>".$arrS["ID"]."</a>] ".$arrS["NAME"];
								else :
...
<tr>
	...
	<td valign="top"><?
		if (count($banner_ref_id)>1)
		{
			...
			echo SelectBoxFromArray("find_banner_summa", $arr, htmlspecialchars($find_banner_summa), "", "style='width:100%'")."<br>";
		}
...
<tr>
	...
	<td valign="top"><?
		...
		{
			$arr = array("reference"=>array(GetMessage("AD_SEPARATED"), GetMessage("AD_SUMMA")), "reference_id"=>array("N","Y"));
			echo SelectBoxFromArray("find_group_summa", $arr, htmlspecialchars($find_group_summa), "", "style='width:100%'")."<br>";
		}
