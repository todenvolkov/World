...
global
	...
	$GET_PAGE_WEIGHT_ARRAY;

$arrADV_KEYWORDS = array();
$strAdvCurUri = false;
$arrViewedBanners = array(0);
$arrWeightSum = array(); // сумма весов баннеров по контрактам
...
class CAdvContract_all
{
	...
	*****************************************************************/
	...
	function Set($arFields, $CONTRACT_ID, $CHECK_RIGHTS="Y")
	{
		...
		if (CAdvContract::CheckFields($arFields, $CONTRACT_ID, $CHECK_RIGHTS))
		{
			...
			if (intval($CONTRACT_ID)>0)
			{
				...
				if (in_array("DATE_MODIFY", $arrKeys) && CheckDateTime($arFields["DATE_MODIFY"])) $arFields_i["DATE_MODIFY"] = $DB->CharToDateFunction($arFields["DATE_MODIFY"]);
				else $arFields_i["DATE_MODIFY"] = $DB->GetNowFunction();

				if (in_array("MODIFIED_BY", $arrKeys)) $arFields_i["MODIFIED_BY"] = intval($arFields["MODIFIED_BY"]);
				else $arFields_i["MODIFIED_BY"] = $USER_ID;
	...
	function Delete($CONTRACT_ID, $CHECK_RIGHTS="Y")
	{
		...
		else $strError .= GetMessage("AD_ERROR_NOT_ENOUGH_PERMISSIONS_CONTRACT")."<br>";
	}
	...
	function GetStatList($by, $order, $arFilter)
	{
		...
		if (CAdvBanner::CheckDynamicFilter($arFilter))
		{
			if (is_array($arFilter))
			{
				...
				$bannersIn = '';
				foreach($arFilter['CONTRACT_ID'] as $banner_id)
				{
					 $bannersIn .= intval($banner_id) . ',';
				}
				$bannersIn = 'C.ID in (' . substr($bannersIn, 0, -1) . ')';
				if (is_array($arFilter['CONTRACT_ID']) && count($arFilter['CONTRACT_ID'])) $arSqlSearch[] = $bannersIn;
			}
		}
...
class CAdvBanner_all
{
	...
	function Delete($BANNER_ID, $CHECK_RIGHTS="Y")
	{
		...
		if ($arBanner = $rsBanner->Fetch())
		{
			...
			if ($CHECK_RIGHTS=="Y")
			{
				...
				if (in_array("ADD", $arrPERM)) $ok = true;
			}
			else $ok = true;
			...
			else $strError .= GetMessage("AD_ERROR_NOT_ENOUGH_PERMISSIONS_BANNER")."<br>";
		}
	}
	...
	function GetGroupArray($BANNER_ID)
	{
		...
		$strSql = "
			...
			";
		$rs = $DB->Query($strSql, false, $err_mess.__LINE__);
		while ($ar = $rs->Fetch()) $arrRes[] = $ar["GROUP_ID"];
		return $arrRes;
	}
	...
	function GetStatAdvArray($BANNER_ID)
	{
		...
		$strSql = "
			...
			";
		$rs = $DB->Query($strSql, false, $err_mess.__LINE__);
		while ($ar = $rs->Fetch()) $arrRes[] = $ar["STAT_ADV_ID"];
		return $arrRes;
	}
	...
	{//echo "Egggog -----------------------------------<p>";
		$err_mess = (CAdvBanner_all::err_mess())."<br>Function: CheckFields<br>Line: ";
		global $DB, $strError, $MESS, $APPLICATION, $USER;
		$str = "";
		if ($CHECK_RIGHTS=="Y")
		{
			$isAdmin = CAdvContract::IsAdmin();
		...
		if ($CONTRACT_ID>0)
		{
			...
			else
			{
				$arrPERM = CAdvContract::GetUserPermissions($CONTRACT_ID);
				$arrPERM = $arrPERM[$CONTRACT_ID];
				if (in_array("ADD", $arrPERM)) $access = "Y";
			}
			...
			if ($access=="Y")
			{
				...
				if (in_array("arrIMAGE_ID", $arrKeys))
				{
					...
					if (strlen($strRes)>0) $str .= $strRes."<br>";
				}
			...
			{
				if ($BANNER_ID>0) $str .= GetMessage("AD_ERROR_NOT_ENOUGH_PERMISSIONS_BANNER")."<br>";
				else  $str .= GetMessage("AD_ERROR_NOT_ENOUGH_PERMISSIONS_CONTRACT")."<br>";
			}
		}
		else $str .= GetMessage("AD_ERROR_INCORRECT_CONTRACT_ID")."<br>";
	...
	function Set($arFields, $BANNER_ID, $CHECK_RIGHTS="Y")
	{
		...
		if (CAdvBanner::CheckFields($arFields, $BANNER_ID, $CHECK_RIGHTS))
		{
			if ($CHECK_RIGHTS=="Y")
			{
				...
				if ($isManager) $CHECK_CONTRACT_RIGHTS = "N";
			}
			else
			{
				$USER_ID = 0;
			...
			if ($BANNER_ID>0)
			{
				$rsBanner = CAdvBanner::GetByID($BANNER_ID, $CHECK_RIGHTS);
				$arBanner = $rsBanner->Fetch();
				if (!in_array("CONTRACT_ID", $arrKeys)) $CONTRACT_ID = intval($arBanner["CONTRACT_ID"]);
				else $CONTRACT_ID = intval($arFields["CONTRACT_ID"]);
			}
			else $CONTRACT_ID = intval($arFields["CONTRACT_ID"]);
			...
			if ($CONTRACT_ID>0)
			{
				...
				if (in_array("MAX_VISITOR_COUNT", $arrKeys))
				{
					...
					else $arFields_i["MAX_VISITOR_COUNT"] = "null";
				}
				...
				if (in_array("SHOWS_FOR_VISITOR", $arrKeys))
				{
					...
					else $arFields_i["SHOWS_FOR_VISITOR"] = "null";
				}
				...
				if (in_array("MAX_SHOW_COUNT", $arrKeys))
				{
					...
					else $arFields_i["MAX_SHOW_COUNT"] = "null";
				}
				...
				if (in_array("MAX_CLICK_COUNT", $arrKeys))
				{
					...
					else $arFields_i["MAX_CLICK_COUNT"] = "null";
				}
				...
				if ($BANNER_ID>0)
				{
					...
					if ($email_notify=="Y" && (!$isAdmin || !$isManager || $SEND_EMAIL=="Y"))
					{
						...
						if ($arBanner=$rsBanner->Fetch())
						{
							...
							if (intval($arBanner["IMAGE_ID"])>0)
							{
								$rsImage = CFile::GetByID($arBanner["IMAGE_ID"]);
								if ($arImage = $rsImage->Fetch())
								{
									$IMAGE_LINK = "http://".$_SERVER["HTTP_HOST"]."/".(COption::GetOptionString("main", "upload_dir", "upload"))."/".$arImage["SUBDIR"]."/".$arImage["FILE_NAME"];
								}
							}

							$FLASHIMAGE_LINK = "";
							if (intval($arBanner["FLASH_IMAGE"])>0)
							{
								$rsFlashImage = CFile::GetByID($arBanner["FLASH_IMAGE"]);
								if ($arImage = $rsFlashImage->Fetch())
								{
									$FLASHIMAGE_LINK = "http://".$_SERVER["HTTP_HOST"]."/".(COption::GetOptionString("main", "upload_dir", "upload"))."/".$arImage["SUBDIR"]."/".$arImage["FILE_NAME"];
								}
							}
							...
							$arEventFields = array(
								...
								"STATUS"				=> GetMessage("AD_STATUS_".$arBanner["STATUS_SID"]),
								"STATUS_COMMENTS"		=> $arBanner["STATUS_COMMENTS"],
								"NAME"				=> $arBanner["NAME"],
								"GROUP_SID"			=> $arBanner["GROUP_SID"],
	...
	{
		return CAdvBanner::SetKeywords($keywords, $TYPE_SID, "REQUIRED");
	}

	function SetDesiredKeywords($keywords, $TYPE_SID="")
	{
		return CAdvBanner::SetKeywords($keywords, $TYPE_SID, "DESIRED");
	}
	...
	function GetRandom($TYPE_SID)
	{
		...
		global $APPLICATION, $USER, $REMOTE_ADDR, $DB, $GET_PAGE_WEIGHT_ARRAY, $arrViewedBanners, $arrADV_VIEWED_BANNERS, $arrWeightSum;
		...
		if ($GET_PAGE_WEIGHT_ARRAY!="Y")
		{
			// получим массив весов для текущей страницы
			$GET_PAGE_WEIGHT_ARRAY="Y";
			...
			{
				$cookie_name = "BANNERS";
				$arr = explode(",",$GLOBALS["APPLICATION"]->get_cookie($cookie_name));
				if (is_array($arr) && count($arr)>0)
				{
					foreach($arr as $str)
					{
			...
			while($ar=$rs->Fetch())
			{
				...
				if ((intval($ar["SHOWS_FOR_VISITOR"])>0 && intval($arrCookie_counter[$ar["BANNER_ID"]])<intval($ar["SHOWS_FOR_VISITOR"])) || intval($ar["SHOWS_FOR_VISITOR"])<=0)
				{
					...
					if (count($arrRequiredPageKeywords)>0 ||
						...
						count($arrDesiredPageKeywords_all)>0) $arKeywordsSet[$ar["TYPE_SID"]] = "Y";

					$arrBannerKeywords = preg_split('/[\n\r]+/', $ar["BANNER_KEYWORDS"]);
					if (is_array($arrBannerKeywords)) TrimArr($arrBannerKeywords);

					if ($DONT_USE_CONTRACT == "Y" || !array_key_exists("CONTRACT_KEYWORDS", $ar))
					{
						$arrContractKeywords = preg_split('/[\n\r]+/', $ar["CONTRACT_KEYWORDS"]);
						if (is_array($arrContractKeywords)) TrimArr($arrContractKeywords);
		...
		if (is_array($arrWSum) && count($arrWSum)>0)
		{
			...
			if ($DONT_USE_CONTRACT == "N" || !array_key_exists("0", $arrWSum))
			{
				...
				//echo "СУММА ВЕСОВ КОНТРАКТОВ = ".$intSum."<br>";
			...
			//echo "ВЫБРАЛИ КОНТРАКТ # ".$CONTRACT_ID."<br>";
					...
					if (count($stubs) > 0) {
						...
						if($rndWeight<$intStep or $rndWeight>$intStep+$infUniform["WEIGHT"])
						{
							...
							while ($infUniform = next($stubs)) {
								if($rndWeight>=$intStep && $rndWeight<=$intStep+$infUniform["WEIGHT"]) break;
								$intStep += $infUniform["WEIGHT"];
							}
						}
					}
	...
	function GetUniformityCoef($arBanner)
	{
		...
		//debug
		//echo "Progress: {$arProgress} = {$rot}/{$tim}.";
	...
	{
		global $strClickURL, $nRandom1, $nRandom2, $nRandom3, $nRandom4, $nRandom5;
	...
	function GetHTML($arBanner, $bNoIndex=false)
	{
		...
		if(intval($arBanner["IMAGE_ID"])>0)
		{
			...
			if ($arImage = $rsImage->Fetch())
			{
				...
				$path = "/".(COption::GetOptionString("main", "upload_dir", "upload"))."/".$arImage["SUBDIR"]."/".$arImage["FILE_NAME"];
				switch($file_type)
				{
					case "FLASH":
						$arParams = array();
						...
						if (strlen(trim($arBanner["URL"]))>0)
						{
							...
							if (strlen(trim($arBanner["URL_TARGET"]))>0)
							{
								...
								$a_target = ' target="'.htmlspecialchars($arBanner["URL_TARGET"]).'" ';
							}
						}
						if (strlen(trim($arBanner["IMAGE_ALT"]))>0)
						{
							...
							$a_title = " title=\"".htmlspecialchars($alt)."\" ";
						}
...
'<OBJECT
	...
	HEIGHT="'.$arImage["HEIGHT"].'">
		...
		<EMBED
			...
			PLUGINSPAGE="http://www.macromedia.com/go/getflashplayer">
						...
						{
							$altImgPath = '';
							if ($arBanner["FLASH_IMAGE"] > 0)
							{
								$altImg = CFile::GetByID($arBanner["FLASH_IMAGE"]);
								if ($arAltImage = $altImg->Fetch())
								{
									$altImgPath = "/".(COption::GetOptionString("main", "upload_dir", "upload"))."/".$arAltImage["SUBDIR"]."/".$arAltImage["FILE_NAME"];
								}
							}

							$strReturn = '';
							$test_id = RandString(10);
							if(!$flashJs)
							{
								$flashJs = true;
							...
							$strReturn .= '<script type="text/javascript">bx_adv_includeFlash("'.$test_id.'", "'.
								...
								CUtil::JSEscape(htmlspecialchars($arBanner["URL_TARGET"])).'", "'.
								CUtil::JSEscape(htmlspecialchars($alt)).'");</script>';
						}
						break;
	...
	function FixShowAll()
	{
		global $DB, $CACHE_ADVERTISING, $arrADV_VIEWED_BANNERS;
			...
			is_array($CACHE_ADVERTISING["BANNERS_CNT"]))
		{
			...
			if (!$bEqualBanID && !empty($CACHE_ADVERTISING["BANNERS_CNT"]))
			{
				...
				$group_inc = implode(",", $CACHE_ADVERTISING["BANNERS_CNT"]);
			...
			if(is_array($arrADV_VIEWED_BANNERS) && count($arrADV_VIEWED_BANNERS) > 0)
			{
				...
				$cookie_name = "BANNERS";
				$GLOBALS["APPLICATION"]->set_cookie($cookie_name, $cookie_value);
			}
		}
	}
	...
	function SetCookie($arBanner, &$inc_banner_counter, &$inc_contract_counter)
	{
		...
		if (intval($arBanner["ID"])>0)
		{
			...
			if (is_array($arrADV_VIEWED_BANNERS))
			{
				...
				$arrContracts = array();
				reset($arrCookie);
				while (list($key,$arr)=each($arrCookie)) $arrContracts[] = $arr["CONTRACT_ID"];
			...
			if (is_array($arrContracts) && !in_array($arBanner["CONTRACT_ID"], $arrContracts))
	...
	function GetStatList($by, $order, $arFilter)
	{
		...
		if (CAdvBanner::CheckDynamicFilter($arFilter))
		{
			if (is_array($arFilter))
			{
				...
				$bannersIn = '';
				foreach($arFilter['BANNER_ID'] as $banner_id)
				{
					 $bannersIn .= intval($banner_id) . ',';
				}
				$bannersIn = 'D.BANNER_ID in (' . substr($bannersIn, 0, -1) . ')';
				if (is_array($arFilter['BANNER_ID']) && count($arFilter['BANNER_ID'])) $arSqlSearch[] = $bannersIn;
			}
		}
...
class CAdvType_all
{
	...
	function CheckFilter($arFilter)
	{
		$err_mess = (CAdvType_all::err_mess())."<br>Function: CheckFilter<br>Line: ";
		global $strError;
		$str = "";
		$find_date_modify_1 = $arFilter["DATE_MODIFY_1"];
		$find_date_modify_2 = $arFilter["DATE_MODIFY_2"];
		...
		if (CAdvType::CheckFields($arFields, $OLD_SID, $CHECK_RIGHTS))
		{
			...
			if (count($arFields_i)>0)
			{
				if (strlen($OLD_SID)>0)
				{
					...
					if (in_array("MODIFIED_BY", $arrKeys)) $arFields_i["MODIFIED_BY"] = intval($arFields["MODIFIED_BY"]);
					else $arFields_i["MODIFIED_BY"] = $USER->GetID();
					...
					$ok = $DB->Query($strSql, false, $err_mess.__LINE__);
					if (in_array("SID", $arrKeys))
					{
						$SID = $arFields["SID"];
				...
				{
					if (in_array("DATE_CREATE", $arrKeys) && CheckDateTime($arFields["DATE_CREATE"])) $arFields_i["DATE_CREATE"] = $DB->CharToDateFunction($arFields["DATE_CREATE"]);
					else $arFields_i["DATE_CREATE"] = $DB->GetNowFunction();

					if (in_array("CREATED_BY", $arrKeys)) $arFields_i["CREATED_BY"] = intval($arFields["CREATED_BY"]);
					else $arFields_i["CREATED_BY"] = $USER->GetID();
