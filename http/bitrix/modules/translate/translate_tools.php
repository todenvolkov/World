...
{
	global $arLangDirs, $SHOW_LANG_DIFF;
	if (is_array($arDirs))
	{
		if ($SHOW_LANG_DIFF)
		{
...
function GetTLangFiles($path)
{
	global $IS_LANG_DIR, $arTLangs, $arFiles, $arDirFiles, $arLangDirFiles;
...
function TSEARCH($file, &$count)
{
	...
	else if ($arSearchParam['bSearchMnemonic']) 
	{
		$_bMnemonic = true;
	}
	...
	foreach ($_mess as $_sMn =>  $_sMe) 
	{
		...
		{ 
	 	    $_bResult = true;
			$res = array();	 
			if ($arSearchParam['is_replace']) // Replace
			{
