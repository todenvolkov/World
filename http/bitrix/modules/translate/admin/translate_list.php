...
if (check_bitrix_sessid()) {
	...
	} else if (array_key_exists('tr_search', $_REQUEST)) 
	{		
	...
	if (!$arSearchParam) {
		...
		for ($i=0; $i<=sizeof($arrChain)-1; $i++) :
			...
			if (strlen($arrChain[$i]["PATH"])>0):
				...
				?><a href="?lang=<?=LANG?>&path=<?=urlencode($last_path)?>&<?=bitrix_sessid_get()?>"  title="<?=GetMessage("TR_FOLDER_TITLE")?>"><?=htmlspecialchars($arrChain[$i]["NAME"])?></a><?
			else:
				?><?=htmlspecialchars($arrChain[$i]["NAME"])?><?
			endif;
		endfor;
	}
	?></p>
...
if (is_array($arLangDirFiles)) :
	...
	if ($IS_LANG_DIR)
	{
		// заменено, так как при перекдлючении языка в пути не менялся язык языковой директории
...
<tr>
	...
	<td style="padding-left:5px;"><input class="form-text" type="text" name="path" id="path_to" size="50" value="<?=htmlspecialchars($path)?>"></td>
...
$url = $APPLICATION->GetPopupLink(
					array(
						'URL' => "/bitrix/admin/translate_search.php?lang=".LANGUAGE_ID."&bxpublic=Y&path=".$path,
								'PARAMS' => array(
									'width' => 470,
									'height' => 310,
									'resizable' => false
...
if ($IS_LANG_DIR) {
	$aContext[] = array(
			...
			"LINK"	=> "translate_check_files.php?lang=".LANGUAGE_ID . '&' . bitrix_sessid_get() . "&path=" . htmlspecialchars($path),
...
?>
    <form action="<?=$APPLICATION->GetCurPageParam('go_path='.htmlspecialchars($path), array('go_path'))?>" name="form3" method="POST" enctype="multipart/form-data">
	<tr>
...
<?
	$tabControl->Buttons();
?>
	<input type="submit" id="tr_submit" value="<?=GetMessage("TR_UPLOAD_SUBMIT_BUTTON")?>">
