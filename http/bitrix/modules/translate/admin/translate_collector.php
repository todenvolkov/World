...
<?=bitrix_sessid_post()?>
	<tr>
		<td width="40%"><span class="required">*</span><?echo GetMessage("TR_SELECT_LANGUAGE")?>:</td>
		<td width="60%">
	...
	<tr>
		<td><span class="required">*</span><?echo GetMessage("TR_COLLECT_DATE")?>:</td>
		<td><input type="text" name="lang_date" size="10" maxlength="8" value="<?= htmlspecialchars($lang_date) ?>"></td>
	</tr>
	<? if (!$butf) { ?>
	<tr>
...
<?=bitrix_sessid_post()?>
	<tr>
		<td width="10%" nowrap><span class="required">*</span>
		<?=GetMessage("TR_UPLOAD_FILE")?>:</td>
		<td valign="top" width="90%"><input type="file" name="tarfile"></td>
	</tr>
	<tr>
		<td width="40%"><span class="required">*</span><?=GetMessage("TR_SELECT_LANGUAGE")?> <?=GetMessage("TR_SELECT_LANGUAGE_DESCRIPTION")?>:</td>
		<td width="60%">
		<select name="language_id">
			<?
			$rsLang = CLanguage::GetList($by="sort", $order="desc");
...
$tabControl->Buttons();
?>
<input type="submit" id="tr_submit" value="<?=isset($_REQUEST["tabControl_active_tab"]) && $_REQUEST["tabControl_active_tab"] == 'download' ? GetMessage("TR_DOWNLOAD_LANGUAGE") : GetMessage("TR_COLLECT_LANGUAGE")?>">
<?	
$tabControl->End();
?>
