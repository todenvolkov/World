...
if($strError == "")
{
	...
	if (is_array($arSlash))
	{
		...
		foreach($arSlash_tmp as $dir)
		{
			...
			else
			{
				$arPath[] = $dir;
				if ($i>2) $chain .= " / ";
				$chain .= "<a href=\"translate_list.php?lang=".LANGUAGE_ID."&path="."/".implode("/",$arPath)."/"."&".bitrix_sessid_get()."\" title=\"".GetMessage("TRANS_CHAIN_FOLDER")."\">".$dir."</a>";
			}
		}
	}
	...
	if(count($arFiles)>0)
	{
		...
		{
			// посчитаем сумму и расхождения для файла
			while (list($f,$arLns)=each($arFilesLng))
			{
				...
				{
					foreach ($arLn as $lg) if (!in_array($lg, $arKEYS)) $arKEYS[] = $lg;
				}
				$total = sizeof($arKEYS);
	...
	// соберем в массив все что надо записать в файл
	if ($REQUEST_METHOD=="POST" && (strlen($save)>0 || strlen($apply)>0) && $TRANS_RIGHT=="W" && check_bitrix_sessid())
	{
		if (is_array($KEYS))
		{
			...
			// соберем все в переменные и запишем в файлы
...
if ($ONLY_ERROR=="N"):
	$aMenu[] = array(
		...
		"LINK"	=> "/bitrix/admin/translate_edit.php?file=".htmlspecialchars($file)."&lang=".LANGUAGE_ID."&show_error=Y",
...
elseif ($ONLY_ERROR=="Y"):
	$aMenu[] = array(
		...
		"LINK"	=> "/bitrix/admin/translate_edit.php?file=".htmlspecialchars($file)."&lang=".LANGUAGE_ID,
...
$aMenu[] = array(
		...
		"LINK"	=> "/bitrix/admin/translate_show_php.php?file=".htmlspecialchars($file)."&lang=".LANGUAGE_ID,
		"TITLE"	=> GetMessage("TR_FILE_SHOW_TITLE"),
	);

$aMenu[] = array(
		"TEXT"	=> GetMessage("TR_FILE_EDIT"),
		"LINK"	=> "/bitrix/admin/translate_edit_php.php?file=".htmlspecialchars($file)."&lang=".LANGUAGE_ID,
...
?>
<p><?=$chain?></p>
<form name="form1" method="POST" action="<?=$APPLICATION->GetCurPage()?>?show_error=<?=htmlspecialchars($show_error)?>&file=<?=htmlspecialchars($file)?>&lang=<?=LANGUAGE_ID?>">
<?=bitrix_sessid_post()?>
<?
$tabControl->Begin();
		...
		<td width="100%" colspan="2">
			<table border="0" cellspacing="3" cellpadding="3" width="100%">
				...
				<tr>
					...
					<td valign="top" align="left" width="65%" nowrap><b><?=htmlspecialchars(basename($file))?></b></td>
				</tr>
				<tr>
					...
					<td colspan="2" valign="top" align="left" width="100%" nowrap>
						<table border="0" cellspacing="0" cellpadding="0" width="100%">
							...
							<tr>
								...
								?>&nbsp;<label for="DEL_<?=$key_del?>"><?=GetMessage("TRANS_DELETE")?></label><input type="checkbox" name="<?="DEL_".$key?>" value="Y" <?=$s?> id="<?='DEL_'.$key_del?>"> <?//echo InputType("checkbox","DEL_".$key,"Y","",false,"",$s." id='DEL_".$key_del."'") ?></td>
							<tr>
							...
							<?
								...
								while (list($j,$arLng)=each($arTLanguages)) :
									...
									if(isset(${$key."_".$arLng["LID"]}) && $key."_".$arLng["LID"] != $arMESS[$arLng["LID"]][$key])
										$valMsg = htmlspecialchars(${$key."_".$arLng["LID"]});
									else
										$valMsg = htmlspecialchars($arMESS[$arLng["LID"]][$key]);
							?>
							<tr>
								<td valign="top">[<?=$arLng["LID"]?>]&nbsp;<?=$arLng["NAME"]?>:&nbsp;</td>
								<td colspan="2">
									<input type="hidden" name="<?echo $key."_".$arLng["LID"]."_PREV"?>" value="<?=htmlspecialchars($arMESS[$arLng["LID"]][$key])?>">
							...
							<?
								...
								while (list($j,$arLng)=each($arTLanguages)) :
									...
									?>
									<input type="hidden" name="<?echo $key."_".$arLng["LID"]."_PREV"?>" value="<?=htmlspecialchars($arMESS[$arLng["LID"]][$key])?>">
									<input type="hidden" name="<?echo $key."_".$arLng["LID"]?>" value="<?=htmlspecialchars($arMESS[$arLng["LID"]][$key])?>">
									<?
									endif;
								endwhile;
