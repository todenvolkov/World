...
class CXMLCreator {
	...
	function createTagAttributed($heavyTag, $value = null)
	{
		...
		if ($attrs and strlen($attrs))
		{
			...
			while ($validi = strpos(trim($attrsSplit[$i]), "="))
			{
				...
				// attr:ns=
				$attrName = substr($attrsSplit[$i], 0, $validi);
				// attrs:ns
				$attrValue = $attrsSplit[$i+1];
	...
	function encodeValueLight( $name, $value)
    {
    	...
    	if (strtolower(get_class($value)) == "cxmlcreator")
    	{
    		$node->addChild($value);
    	}
	...
	function setAttribute($attrName, $attrValue) 
	{
		...
		$attrValue = $APPLICATION->ConvertCharset(CDataXML::xmlspecialchars($attrValue), LANG_CHARSET, 'utf-8');
	...
	{
		if (!$this->tag) return "";
		$xml  = "<" . $this->tag . $this->_getAttributs() . ">";
		$xml .= $this->startCDATA;
		$xml .= $this->data;
		$xml .= $this->endCDATA;
