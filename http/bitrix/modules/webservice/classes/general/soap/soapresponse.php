...
class CSOAPResponse extends CSOAPEnvelope
{
    ...
    function decodeStream( $request, $stream )
    {
        ...
        {
            ShowError("Error: BitrixXMLParser. "
		...
		{
			ShowError( "Error: Can't parse request xml data. ");
            return;
		}
        ...
        if ( get_class( $dom ) == "CDataXMLDocument" || get_class( $dom ) == "cdataxmldocument")
        {
			...
			{
				ShowError( "Could not understand type of class decoded" );
			}
        }
        else
        {
            ShowError( "Could not process XML in response" );
        }
    }
    ...
    function payload( )
    {
        ...
        if ( get_class( $this->Value ) == 'CSOAPFault' or get_class( $this->Value) == 'csoapfault')
        {
        ...
        else
        {
            ...
            if (!isset($this->typensVars[$this->Name]["output"]) or !count($this->typensVars[$this->Name]["output"]))
            {
            	...
            	{
	            	ShowError("payload() can't find output type declaration.", "SoapRespnose::payload()");
					return;
            	}
            	else
            	{
            ...
            else
            {
	            ...
	            foreach ($this->typensVars[$this->Name]["output"] as $returnType => $returnParam)
	            {
		            if (!$returnType)
		            {
		            	ShowError("payload() can't find output type declaration for {$this->Name}.", "SoapRespnose::payload()");
						return;
		            }
