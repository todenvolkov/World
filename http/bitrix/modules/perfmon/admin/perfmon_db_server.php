...
{
	$vars = array();
	$rs = $DB->Query("SHOW GLOBAL VARIABLES");
	while($ar = $rs->Fetch())
	...
	else
	{
		...
		$data[0]["ITEMS"][] = array(
			...
			"KPI_VALUE" => perfmon_SizeFormat($calc['server_buffers']),
		...
		$data[0]["ITEMS"][] = array(
			...
			"KPI_VALUE" => perfmon_SizeFormat($calc['per_thread_buffers']),
		...
		$data[0]["ITEMS"][] = array(
			...
			"KPI_VALUE" => perfmon_SizeFormat($calc['total_possible_used_memory']),
		...
		if($arVersion[1] >= 5)
		{
			$rs = $DB->Query("SELECT IFNULL(SUM(INDEX_LENGTH),0) IND_SIZE FROM information_schema.TABLES WHERE TABLE_SCHEMA NOT IN ('information_schema') AND ENGINE = 'MyISAM'");
			$ar = $rs->Fetch();
			if($ar["IND_SIZE"] > 0)
			{
				$total_myisam_indexes = $ar["IND_SIZE"];
				$calc['total_myisam_indexes'] = perfmon_SizeFormat($ar["IND_SIZE"]);
		...
		{
			$calc['total_myisam_indexes'] = "<font class=\"errortext\">N/A</font>";
		...
		if($total_myisam_indexes > 0)
		{
			...
			$data[0]["ITEMS"][] = array(
				...
				"KPI_RECOMMENDATION" => GetMessage("PERFMON_KPI_REC_KEY_MISS", array(
					"#PARAM_VALUE#" => perfmon_SizeFormat($vars["key_buffer_size"]),
		...
		$data[0]["ITEMS"][] = array(
			...
			"KPI_VALUE" => perfmon_SizeFormat($vars['query_cache_size']),
			"KPI_RECOMMENDATION" => $rec,
		);
		...
		if($vars['query_cache_size'] > 0)
		{
			...
			else
			{
				...
				$rec = GetMessage("PERFMON_KPI_REC_QCACHE", array(
					...
					"#PARAM_VALUE#" => perfmon_SizeFormat($vars['query_cache_limit']),
					"#GOOD_VALUE#" => "20%",
				));
			}
			...
			if($stat['Com_select'] > 0)
			{
				$data[0]["ITEMS"][] = array(
					...
					"KPI_RECOMMENDATION" => GetMessage("PERFMON_KPI_REC_QCACHE_PRUNES", array(
						...
						"#PARAM_VALUE#" => perfmon_SizeFormat($vars['query_cache_size']),
						"#PARAM_NAME#" => "<span class=\"perfmon_code\">query_cache_size</span>",
						"#PARAM_VALUE_HIGH#" => "128M",
					)),
				);
		...
		if($calc['total_sorts'] > 0)
		{
			...
			$data[0]["ITEMS"][] = array(
				...
				"KPI_RECOMMENDATION" => GetMessage("PERFMON_KPI_REC_SORTS_DISK", array(
					...
					"#GOOD_VALUE#" => "10",
					"#PARAM1_VALUE#" => perfmon_SizeFormat($vars['sort_buffer_size']),
					"#PARAM1_NAME#" => "<span class=\"perfmon_code\">sort_buffer_size</span>",
					"#PARAM2_VALUE#" => perfmon_SizeFormat($vars['read_rnd_buffer_size']),
		...
		if($calc['joins_without_indexes_per_day'] > 250)
		{
			$data[0]["ITEMS"][] = array(
				...
				"KPI_RECOMMENDATION" => GetMessage("PERFMON_KPI_REC_JOINS", array(
					...
					"#PARAM_VALUE#" => perfmon_SizeFormat($vars['join_buffer_size']),
		...
		if($stat['Created_tmp_tables'] > 0)
		{
			...
			if($calc['pct_temp_disk'] > $pct_temp_disk && $calc['max_tmp_table_size'] < 256*1024*1024)
			{
				...
				$rec = GetMessage("PERFMON_KPI_REC_TMP_DISK_1", array(
					...
					"#PARAM1_VALUE#" => perfmon_SizeFormat($vars['tmp_table_size']),
					"#PARAM2_NAME#" => "<span class=\"perfmon_code\">max_heap_table_size</span>",
					"#PARAM2_VALUE#" => perfmon_SizeFormat($vars['max_heap_table_size']),
				));
			}
		...
		if($stat['Table_locks_immediate'] > 0)
		{
			...
			$data[0]["ITEMS"][] = array(
				...
				"KPI_VALUE" => (
					...
					"<font class=\"errortext\">".$calc['pct_table_locks_immediate']."%</font>"
				),
		...
		if($vars['concurrent_insert'] == "OFF")
		{
			$data[0]["ITEMS"][] = array(
				...
				"KPI_VALUE" => "<font class=\"errortext\">OFF</font>",
		...
		elseif($vars['concurrent_insert'] == "0")
		{
			$data[0]["ITEMS"][] = array(
				"KPI_NAME" => GetMessage("PERFMON_KPI_NAME_INSERTS"),
				"KPI_VALUE" => "<font class=\"errortext\">0</font>",
				"KPI_RECOMMENDATION" => GetMessage("PERFMON_KPI_REC_INSERTS", array(
					"#PARAM_NAME#" => "<span class=\"perfmon_code\">concurrent_insert</span>",
					"#REC_VALUE#" => "1",
				)),
			...
			if($stat['Innodb_buffer_pool_reads'] > 0 && $stat['Innodb_buffer_pool_read_requests'] > 0)
			{
				...
				$data[0]["ITEMS"][] = array(
					...
					"KPI_RECOMMENDATION" => GetMessage("PERFMON_KPI_REC_INNODB_BUFFER", array(
						...
						"#PARAM_VALUE#" => perfmon_SizeFormat($vars['innodb_buffer_pool_size']),
					)),
				);
			}
		...
		while($arRes = $rsData->NavNext(true, "f_"))
		{
			...
			foreach($arRes as $key => $value)
			{
				if($key == "KPI_VALUE" && array_key_exists("IS_OK", $arRes) && !$arRes["IS_OK"])
					$row->AddViewField($key, "<font class=\"errortext\">".$value."</font>");
				else
