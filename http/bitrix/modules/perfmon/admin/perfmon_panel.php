...
elseif(isset($_REQUEST["test"]) && $_REQUEST["test"] === "cluster")
{
	...
	)
	{
		...
		elseif($threads > $threads_to || time() > $_SESSION["treads_test_end"]) //Finished
		{
			?><script>
				ThreadsStop();
			</script><?
		...
		?>
			...
			<?if(!$bFinish):?>
			<tr>
				<td colspan="7"><?echo GetMessage("PERFMON_PANEL_MEASURE")?></td>
			</tr>
			<?endif;?>
			</table>
			</div>
...
function ThreadsTest(threads)
{
	...
	url += '&threads_from=' + BX('threads_from').value;
	url += '&threads_to=' + BX('threads_to').value;
	url += '&threads_step=' + BX('threads_step').value;
	url += '&server_name=' + BX('server_name').value;
	url += '&server_url=' + BX.util.urlencode(BX('server_url').value);
	url += '&threads_duration=' + BX('threads_duration').value;
	if(threads)
	{
		url += '&threads='+threads;
	}
