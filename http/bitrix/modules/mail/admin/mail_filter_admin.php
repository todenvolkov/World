...
include(GetLangFileName($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/mail/lang/", "/admin/mail_filter_admin.php"));
	...
	else
	{
		...
		if($arRes["ACTION_TYPE"] <> '')
		{
			...
			if ($arModFilter = $res->Fetch()):
				$strType = htmlspecialchars($arModFilter["NAME"])."<br>";
			endif;
		}
		else
		{
		...
		while($ar = $res->Fetch())
		{
			...
			if (strpos($ar["STRINGS"], "\n")>0)
			{
				...
				$strCond .= " ".$strNameTmp2." {".htmlspecialchars(substr($ar["STRINGS"], 0, 30)).(strlen(trim($ar["STRINGS"]))>30?"...":"")."}";
			}
			else
				$strCond .= " ".$strNameTmp1." &quot;".htmlspecialchars(substr($ar["STRINGS"], 0, 30)).(strlen($ar["STRINGS"])>30?"...":"")."&quot;";
		}
