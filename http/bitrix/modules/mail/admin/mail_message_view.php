...
if($dbr_arr = $dbr->ExtractFields("str_")):
	...
	{
		echo "<pre>".nl2br(htmlspecialchars($dbr_arr["FULL_TEXT"]))."</pre>";
		die();
	}
...
<?$tabControl->BeginNextTab();?>
	...
	<tr valign="top">
		<td align="right" width="1%" nowrap><?echo GetMessage("MAIL_MSG_VIEW_DATE")?></td>
		<td width="99%"><?=$str_FIELD_DATE?></td>
	</tr>
	<?if(strlen($dbr_arr["FIELD_FROM"])>0):?>
	<tr valign="top">
		<td align="right" nowrap><?echo GetMessage("MAIL_MSG_VIEW_FROM")?></td>
		<td><?=TxtToHTML($dbr_arr["FIELD_FROM"])?></td>
	</tr>
	<?endif?>
	<?if(strlen($dbr_arr["FIELD_TO"])>0):?>
	<tr valign="top">
		<td align="right" nowrap><?echo GetMessage("MAIL_MSG_VIEW_TO")?></td>
		<td><?=TxtToHTML($dbr_arr["FIELD_TO"])?></td>
	</tr>
	<?endif?>
	<?if(strlen($dbr_arr["FIELD_CC"])>0):?>
	<tr valign="top">
		<td align="right" nowrap><?echo GetMessage("MAIL_MSG_VIEW_CC")?></td>
		<td><?=TxtToHTML($dbr_arr["FIELD_CC"])?></td>
	</tr>
	<?endif?>
	<?if(strlen($dbr_arr["FIELD_BCC"])>0):?>
	<tr valign="top">
		<td align="right" nowrap><?echo GetMessage("MAIL_MSG_VIEW_BCC")?></td>
		<td><?=TxtToHTML($dbr_arr["FIELD_BCC"])?></td>
	</tr>
	<?endif?>
	<tr valign="top">
		<td align="right" nowrap><?echo GetMessage("MAIL_MSG_VIEW_SUBJECT")?></td>
		<td>
		<script>
	...
	?>
	<tr valign="top">
		<td colspan="2" style="background:white;"><font style="font-family:Courier New, Arial, Helvetica, sans-serif; font-size:12px; color: #000000; text-decoration:none;"><?=preg_replace("'(^|\r\n)[\s]*([A-Za-z]*(&gt;)+)([^\r\n]+)'e", "_ConvReplies('\\2', '\\4')", TxtToHTML($dbr_arr["BODY"]))?></td>
	</tr>
	<?
	...
	?>
	<tr valign="top">
		<td align="right"><?echo GetMessage("MAIL_MSG_VIEW_ATTACHMENTS")?></td>
		<td>
	...
	<?endif?>
	<tr valign="top">
		<td align="right" width="1%" nowrap><?echo GetMessage("MAIL_MSG_VIEW_STATUS")?></td>
		<td width="99%" valign="top">
			...
			<?else:?>
				...
				<?endif?>
			<?endif?>
			<span title="<?=htmlspecialchars(preg_replace('/(.?) ([.0-9]+) ([0-9]+) ([0-9]+)/', '\\1 = \\2% (\\3 bad, \\4 good)', $dbr_arr["SPAM_WORDS"]))?>">
	...
	<?if($dbr_arr["SPAM"]=="Y"):?>
	<tr valign="top">
		<td align="right" width="1%" nowrap><?echo GetMessage("MAIL_MSG_VIEW_MARK_AS_NOTSPAM")?></td>
		<td width="99%"><input type="checkbox" name="MARK_AS_NOT_SPAM" value="Y"></td>
	</tr>
	<?elseif($dbr_arr["SPAM"]=="N"):?>
	<tr valign="top">
		<td align="right" width="1%" nowrap><?echo GetMessage("MAIL_MSG_VIEW_MARK_AS_SPAM")?></td>
		<td width="99%"><input type="checkbox" name="MARK_AS_SPAM" value="Y"></td>
	</tr>
	<?else:?>
	<tr valign="top">
		<td align="right" width="1%" nowrap><?echo GetMessage("MAIL_MSG_VIEW_SPAM_LEARN")?></td>
		<td width="99%">
	...
	<?endif?>
	<tr valign="top">
		<td align="right" width="1%" nowrap><?echo GetMessage("MAIL_MSG_VIEW_ACT_RULE")?></td>
		<td width="99%">
		...
		?><option value="<?=$flt_arr["ID"]?>"><?=htmlspecialchars($flt_arr["NAME"])?> [<?=htmlspecialchars($flt_arr["ID"])?>]</option><?
		endwhile?>
		</select>
		</td>
	</tr>
	<tr valign="top">
		<td align="right" width="1%" nowrap><?echo GetMessage("MAIL_MSG_VIEW_ACT_RULE_DELETE")?></td>
		<td width="99%"><input type="checkbox" name="DELETE_MESSAGE" value="Y"></td>
	</tr>
...
<input <?if ($MOD_RIGHT<"W") echo "disabled" ?> type="submit" name="save" value="<?echo GetMessage("MAIL_MSG_VIEW_SAVE")?>">
