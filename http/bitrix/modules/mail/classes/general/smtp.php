...
class CSMTPServer
{
	...
	function Start()
	{
		if(file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/cache/smtpd_stop.php"))
			@unlink($_SERVER["DOCUMENT_ROOT"]."/bitrix/cache/smtpd_stop.php");
		if(file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/cache/smtpd.php"))
			@unlink($_SERVER["DOCUMENT_ROOT"]."/bitrix/cache/smtpd.php");
	...
	function Listen()
	{
		...
		while (true)
		{
			...
			if($cnt>5)
			{
				...
				$f = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/cache/smtpd_stats.php", "w+");
				fwrite($f, '<?return unserialize("'.addslashes(serialize($stats)).'");?>');
				fclose($f);
