...
class CAllTest
{
	...
	function CheckFields(&$arFields, $ID = false)
	{
		...
		if ( is_set($arFields, "COMPLETED_SCORE") &&
		...
		//Defaults
		if (is_set($arFields, "QUESTIONS_FROM") && !in_array($arFields["QUESTIONS_FROM"], Array("A", "C", "L", "H", "S")))
	...
	function GetFilter($arFilter)
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "ATTEMPT_LIMIT":
				case "TIME_LIMIT":
					$arSqlSearch[] = CCourse::FilterCreate("T.".$key, $val, "number", $bFullJoin, $cOperationType);
					break;

				case "NAME":
				case "DESCRIPTION":
					$arSqlSearch[] = CCourse::FilterCreate("T.".$key, $val, "string", $bFullJoin, $cOperationType);
					break;
				...
				case "QUESTIONS_FROM":
				case "PASSAGE_TYPE":
					$arSqlSearch[] = CCourse::FilterCreate("T.".$key, $val, "string_equal", $bFullJoin, $cOperationType);
					break;
			}
