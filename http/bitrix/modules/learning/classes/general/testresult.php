...
class CAllTestResult
{
	...
	function AddResponse($TEST_RESULT_ID, $RESPONSE)
	{
		...
		$rsTestResult = CTestResult::GetByID($TEST_RESULT_ID);
	...
	function GetList($arOrder=Array(), $arFilter=Array())
	{
		...
		$strSql =
		"SELECT TR.*, Q.QUESTION_TYPE, Q.NAME as QUESTION_NAME, Q.POINT as QUESTION_POINT, L.COURSE_ID, Q.LESSON_ID ".
		"FROM b_learn_test_result TR ".
		"INNER JOIN b_learn_question Q ON TR.QUESTION_ID = Q.ID ".
	...
	function GetFilter($arFilter)
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "QUESTION_ID":
				case "POINT":
					$arSqlSearch[] = CCourse::FilterCreate("TR.".$key, $val, "number", $bFullJoin, $cOperationType);
					break;

				case "RESPONSE":
					$arSqlSearch[] = CCourse::FilterCreate("TR.".$key, $val, "string", $bFullJoin, $cOperationType);
					break;

				case "QUESTION_NAME":
					$arSqlSearch[] = CCourse::FilterCreate("Q.NAME", $val, "string", $bFullJoin, $cOperationType);
					break;

				case "ANSWERED":
				case "CORRECT":
					$arSqlSearch[] = CCourse::FilterCreate("TR.".$key, $val, "string_equal", $bFullJoin, $cOperationType);
					break;
			}
		}
	...
	function GetPercent($ATTEMPT_ID)
	{
		...
		$strSql =
		"SELECT ROUND(SUM(CASE WHEN TR.CORRECT = 'Y' THEN Q.POINT ELSE 0 END) * 100 / SUM(Q.POINT), 0) as PCNT ".
