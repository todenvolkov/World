...
class CAllLAnswer
{
	function CheckFields(&$arFields, $ID = false)
	{
		...
		if ( (is_set($arFields, "ANSWER") || $ID === false) && strlen($arFields["ANSWER"]) <= 0)
	...
	function GetFilter($arFilter)
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "SORT":
				case "QUESTION_ID":
					$arSqlSearch[] = CCourse::FilterCreate("CA.".$key, $val, "number", $bFullJoin, $cOperationType);
					break;

				case "ANSWER":
					$arSqlSearch[] = CCourse::FilterCreate("CA.".$key, $val, "string", $bFullJoin, $cOperationType);
					break;

				case "CORRECT":
					$arSqlSearch[] = CCourse::FilterCreate("CA.".$key, $val, "string_equal", $bFullJoin, $cOperationType);
					break;
			}
