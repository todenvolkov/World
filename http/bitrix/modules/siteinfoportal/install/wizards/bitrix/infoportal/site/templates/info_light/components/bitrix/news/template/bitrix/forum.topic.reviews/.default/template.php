...
<?if(!empty($arResult["ERROR_MESSAGE"]) || !empty($arResult["OK_MESSAGE"])) $styleDisplay = 'style="display:block"';?>
<div class="reviews-add-text" ><a href="#" onclick="ShowCommentForm('reviews-add-form'); return false"><?=GetMessage("F_ADD_MESSAGE")?></a></div>
<div id="reviews-add-form" class="reviews-add-form" <?=$styleDisplay?> >
<?
...
?>
	<a href="#review_anchor" title="<?=GetMessage("F_QUOTE_HINT")?>" class="reviews-button-small" <?
			?>onMouseDown="ShowCommentForm('reviews-add-form');quoteMessageEx('<?=$res["FOR_JS"]["AUTHOR_NAME"]?>', 'message_text_<?=$res["ID"]?>')"><?=GetMessage("F_QUOTE_FULL")?></a>
<?
		endif;
?>
		<a href="#review_anchor"  title="<?=GetMessage("F_NAME")?>"  class="reviews-button-small" <?
			?>onMouseDown="ShowCommentForm('reviews-add-form');reply2author('<?=$res["FOR_JS"]["AUTHOR_NAME"]?>,')"><?=GetMessage("F_NAME")?></a></div>
<?
