...
if (empty($sGalleryList))
{
	...
	$sError = str_replace(
		array("#CREATE_GALLERY#", "#UPLOAD#"), 
		array($arResult["LINK"]["NEW"], $arResult["MY_GALLERY"]["LINK"]["UPLOAD"]), 
		$sError); 
?>
...
if (!$bEmptyBest)
{
	if ($_REQUEST["return_array"] != "Y" && empty($_REQUEST["package_id"])): 
		ob_start(); 
	endif;
...
<?
	...
	{
?>
			<div class="photo-header-component"><?
...
<?
	...
	{
?>
				<span class="photo-header-link"> ( <a href="<?=CComponentEngine::MakePathFromTemplate($arParams["GALLERIES_URL"], 
					array("USER_ID" => "users"))?>"><?=GetMessage("P_ALL_PHOTOS")?></a> ) </span>
<?
	}
?>
			</div>
...
<?
	if ($sSortField == "PROPERTY_RATING"): 
		?><?=GetMessage("P_BEST_PHOTO_RATING")?><?
	elseif ($sSortField != "shows"): 
		?><?=GetMessage("P_BEST_PHOTO_COMMENTS")?><?
	else:
		?><?=GetMessage("P_BEST_PHOTO_SHOWS")?><?
	endif;
?>	
			</div>
		</div>
