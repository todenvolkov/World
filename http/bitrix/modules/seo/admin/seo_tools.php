...
?>
	<tr>
		<td width="50%" align="right"><?echo htmlspecialchars($arSearchers[$key]['NAME'])?>:&nbsp;&nbsp;</td>
...
?>
		<tr>
			...
			<td width="70%">
				<div style="height: 15px; border: solid 1px #<?echo $color = GetNextRGB($color, $cnt)?> !important; width: 100%; position: relative; cursor: pointer; text-align: left !important;" onclick="BXToggle('bx_phrase_ex_<? echo $arData['ID']?>'); return false;">
		...
		<tr id="bx_phrase_ex_<? echo $arData['ID']?>" style="display: none;">
			<td colspan="2" align="center"><table>
				<tr class="bx-th">
					<td style="text-align: left !important;"><?echo GetMessage('SEO_PAGE_PHRASES_SEARCHER')?></td>
...
?>
				<tr>
					<td style="text-align: left !important;"><? echo htmlspecialchars($arSearcherData['SEARCHER_NAME']);?></td>
...
?>
				<tr>
					<td style="text-align: left !important;"><a href="<?echo $url?>"><? echo htmlspecialchars(TruncateText($url, 100));?></td>
...
//HTML output
$aTabs = array(
	array("DIV" => "seo_edit1", "TAB" => GetMessage('SEO_TOOLS_TAB_PAGE'), "ICON" => "main_settings", "TITLE" => GetMessage('SEO_TOOLS_TAB_PAGE_TITLE').' '.htmlspecialchars($back_url, ENT_QUOTES)),
...
?>
		<b><?echo htmlspecialcharsEx($titleFinal)?></b>&nbsp;<?if ($titleChangerName != ''):?>(<?echo htmlspecialchars($titleChangerName)?>)&nbsp;<?endif;?><?if ($titleChangerLink):?><br /><a href="<?echo htmlspecialchars($titleChangerLink, ENT_QUOTES);?>"><?echo GetMessage('SEO_PAGE_CURRENT_TITLE_EDIT')?></a><?endif;?>
</tr>
<?
endif;
...
<tr>
	...
	?>
		<b><?echo htmlspecialcharsEx($titleWinFinal)?></b>&nbsp;<?if ($titleWinChangerName != ''):?>(<?echo htmlspecialchars($titleWinChangerName)?>)&nbsp;<?endif;?><?if ($titleWinChangerLink):?><br /><a href="<?echo htmlspecialchars($titleWinChangerLink, ENT_QUOTES)?>"><?echo GetMessage('SEO_PAGE_CURRENT_TITLE_EDIT')?></a><?endif;?>
	</tr>
	<?
	endif;
endif;
...
if ($prop_code = COption::GetOptionString('seo', 'property_keywords', 'keywords')) $arEditProperties['keywords'] = htmlspecialchars($prop_code);
if ($prop_code = COption::GetOptionString('seo', 'property_description', 'description')) $arEditProperties['description'] = htmlspecialchars($prop_code);
...
window.BXCallUpdateKeywordsStats = function(keywords, callback_name)
{
	...
	BX.ajax({
		url: '/bitrix/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=' + BX.util.urlencode(callback_name),
...
window.BXCallPageStats = function()
{
	...
	BX.ajax({
		url: '/bitrix/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&first=Y&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=window.BXSetStats',
		data: 'keywords=' + BX.util.urlencode(keywords),
		method: 'POST',
		dataType: 'script',
		emulateOnload: false
