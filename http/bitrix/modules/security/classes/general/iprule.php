...
class CSecurityIPRule
{
	...
	function UpdateRuleMasks($IPRULE_ID, $arInclMasks = false, $arExclMasks = false)
	{
		global $DB, $CACHE_MANAGER;
	...
	function GetRuleInclMasks($IPRULE_ID)
	{
		global $DB;
		$IPRULE_ID = intval($IPRULE_ID);
		$res = array();
	...
	function GetRuleInclIPs($IPRULE_ID)
	{
		global $DB;
		$IPRULE_ID = intval($IPRULE_ID);
		$res = array();
	...
	function GetRuleExclIPs($IPRULE_ID)
	{
		global $DB;
		$IPRULE_ID = intval($IPRULE_ID);
		$res = array();
	...
	function GetList($arSelect, $arFilter, $arOrder)
	{
		global $DB;
	...
	function IsActive()
	{
		...
		while($arEvent = $rsEvents->Fetch())
		{
			if(
				...
				&& $arEvent["TO_CLASS"] == "CSecurityIPRule"
			)
			{
				$bActive = true;
				break;
	...
	function CheckAntiFile($retun_message = false)
	{
