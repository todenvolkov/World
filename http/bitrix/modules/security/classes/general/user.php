...
class CSecurityUser
{
	...
	function Update($arFields)
	{
		...
		if($USER_ID)
		{
			...
			{
				$secret = substr(trim($arFields["SECRET"]), 0, 50);
				if(strlen($secret) <= 0)
				{
	...
	{
		$sha1_hash = CSecurityUser::hmacsha1(pack("NN", 0, $cnt), $secret);
	...
	{
		$bActive = false;
		$rsEvents = GetModuleEvents("main", "OnBeforeUserLogin");
		while($arEvent = $rsEvents->Fetch())
		{
			if(
				$arEvent["TO_MODULE_ID"] == "security"
				&& $arEvent["TO_CLASS"] == "CSecurityUser"
			)
			{
				$bActive = true;
				break;
