		...
		if ($arUser)
		{
			...
			if ($bSuccess)
			{
				...
				$DB->Query(
					...
					"	AUTHOR_NAME = '".CBlogUser::GetUserName($arUser["ALIAS"], $arGloUser["NAME"], $arGloUser["LAST_NAME"], $arGloUser["LOGIN"])."', ".
					"	AUTHOR_ID = null ".
	...
	function AddToUserGroup($ID, $blogID, $arGroups = array(), $joinStatus = "Y", $selectType = BLOG_BY_BLOG_USER_ID, $action = BLOG_CHANGE)
	{
		...
		if ($bSuccess)
		{
			...
			if (count($arGroups) > 0)
			{
				...
				if ($action == BLOG_ADD)
					$arCurrentGroups = CBlogUser::GetUserGroups($ID, $selectType);
