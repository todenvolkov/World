	...
	function CalcActivity($arConfigs)
	{
		...
		if (isset($arConfigs['CONFIG']['ALL_POST_COEF']) && $arConfigs['CONFIG']['ALL_POST_COEF'] != 0) {
			$sqlAllPost = "
				SELECT
					...
					COUNT(*)*".$arConfigs['CONFIG']['ALL_POST_COEF']." as CURRENT_VALUE
				FROM b_blog_post
				GROUP BY AUTHOR_ID
				UNION ALL ";
		}
		...
		if (isset($arConfigs['CONFIG']['ALL_COMMENT_COEF']) && $arConfigs['CONFIG']['ALL_COMMENT_COEF'] != 0) {
			$sqlAllComment = "
				SELECT
					AUTHOR_ID as ENTITY_ID,
					COUNT(*)*".$arConfigs['CONFIG']['ALL_COMMENT_COEF']." as CURRENT_VALUE
				FROM b_blog_comment
				GROUP BY AUTHOR_ID
				UNION ALL ";
		}
		$strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
			...
			FROM (
				...
				SELECT
					AUTHOR_ID as ENTITY_ID,
				    SUM(IF(TO_DAYS(DATE_PUBLISH) > TO_DAYS(NOW())-1, 1, 0))*".$arConfigs['CONFIG']['TODAY_POST_COEF']." +
				    SUM(IF(TO_DAYS(DATE_PUBLISH) > TO_DAYS(NOW())-7, 1, 0))*".$arConfigs['CONFIG']['WEEK_POST_COEF']."+
				    COUNT(*)*".$arConfigs['CONFIG']['MONTH_POST_COEF']." as CURRENT_VALUE
				FROM b_blog_post
				...
				SELECT
				   AUTHOR_ID as ENTITY_ID,
			       SUM(IF(TO_DAYS(DATE_CREATE) > TO_DAYS(NOW())-1, 1, 0))*".$arConfigs['CONFIG']['TODAY_COMMENT_COEF']." +
			       SUM(IF(TO_DAYS(DATE_CREATE) > TO_DAYS(NOW())-7, 1, 0))*".$arConfigs['CONFIG']['WEEK_COMMENT_COEF']." +
			       COUNT(*)*".$arConfigs['CONFIG']['MONTH_COMMENT_COEF']." as CURRENT_VALUE
				FROM b_blog_comment
