...
{
	function Search($phrase = "", $nTopCount = 5, $arParams = array(), $bNotFilter = false)
	{
		...
		if(!empty($this->_arPhrase))
		{
			...
			$strSql = "
				...
				GROUP BY
					ID, MODULE_ID, ITEM_ID, LID, TITLE, PARAM1, PARAM2, DIR, SERVER_NAME, URL, SITE_URL, SITE_ID
				".(count($sqlHaving) > 1? "HAVING ".implode(" AND ", $sqlHaving): "")."
				ORDER BY RANK1 DESC, RANK2 DESC, RANK3 ASC, TITLE
				LIMIT 0, ".($nTopCount+1)."
			";
