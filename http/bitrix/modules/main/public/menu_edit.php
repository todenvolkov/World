...
{
	$success = false;
	if(file_exists($abs_path) && $USER->CanDoFileOperation('fm_delete_file', $arPath_m))
	{
		$arUndoParams = array(
			'module' => 'fileman',
		...
		if (COption::GetOptionInt("main", "disk_space") > 0)
		{
			...
			$quota->UpdateDiskQuota("file", filesize($abs_path), "delete");
		}
	
		@chmod($abs_path, BX_FILE_PERMISSIONS);
		$success = @unlink($abs_path);
...
?>
	...
	<table border="0" cellpadding="0" cellspacing="0" class="bx-width100 internal" class="menu-table">
	<thead>
		<tr class="heading">
			<td width="0"></td>
