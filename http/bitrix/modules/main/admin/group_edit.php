	...
	function gpSync()
	{
		...
		for(var key in arGroupPolicy['parent'])
		{
			...
			if(!el1.checked)
			{
				...
				if(clevel == 'low')
					level = gpSetLevel(clevel);
				else if(clevel == 'middle' && level == 'high')
					level = gpSetLevel(clevel);
			}
		}
	}
	...
	<tr valign="top" id="__subordinate_groups_tr" <?echo $show_subord ? '' : 'style="display:none"';?>>
		...
		<td width="60%">
			...
			while($arRes = $rsData->Fetch())
			{
				...
				?><option value="<?=$arRes['ID']?>"<?echo ($bSel? ' selected' : '')?>><? echo '['.$arRes['ID'].'] '.$arRes['NAME']?></option><?
			}
			?>
			</select>
			<script>
