	...
	function GetEditFormHTML($arUserField, $arHtmlControl)
	{
		...
		if($arUserField["SETTINGS"]["DISPLAY"]=="CHECKBOX")
		{
			...
			if($arUserField["MANDATORY"]!="Y")
				$result .= '<label><input type="radio" value="" name="'.$arHtmlControl["NAME"].'"'.(!$bWasSelect? ' checked': '').($arUserField["EDIT_IN_LIST"]!="Y"? ' disabled="disabled" ': '').'>'.GetMessage("MAIN_NO").'</label><br>';
			$result .= $result2;
		}
		else
		{
