...
class CSVUserImport
{
	...
	function CSVUserImport($csvFilePath, $delimiter)
	{
		...
		for ($i = 0; $i < count($this->arHeader); $i++)
			$this->arHeader[$i] = strtoupper($this->arHeader[$i]);
	...
	function IgnoreDuplicate($ignore = true)
	{
	...
	function ImportUser()
	{
		...
		if($userID <= 0)
		{
			if(($e = $GLOBALS['APPLICATION']->GetException()))
				$this->errorMessage = $arFields["NAME"]." ".$arFields["LAST_NAME"].": ".$e->GetString();
		}
