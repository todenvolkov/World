...
class CComponentUtil
{
	function __IncludeLang($filePath, $fileName, $lang = False)
	{
		...
		if ($lang != "en" && $lang != "ru")
			if (file_exists($_SERVER["DOCUMENT_ROOT"].$filePath."/lang/en/".$fileName))
				__IncludeLang($_SERVER["DOCUMENT_ROOT"].$filePath."/lang/en/".$fileName);

		if (file_exists($_SERVER["DOCUMENT_ROOT"].$filePath."/lang/".$lang."/".$fileName))
			__IncludeLang($_SERVER["DOCUMENT_ROOT"].$filePath."/lang/".$lang."/".$fileName);
	}

	function Dump2File($text)
	{
		$fff = fopen($_SERVER["DOCUMENT_ROOT"]."/__test__.txt", "w");
		fwrite($fff, $text."\n");
		fclose($fff);
	}
	...
	function GetComponentProps($componentName, $arCurrentValues = array())
	{
		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"].$componentPath."/.parameters.php"))
		{
			...
			{
				//Create directories
				$help_path = $_SERVER["DOCUMENT_ROOT"].$componentPath."/help";
				if(!file_exists($help_path)) mkdir($help_path);
				$help_lang_path = $_SERVER["DOCUMENT_ROOT"].$componentPath."/lang";
				if(!file_exists($help_lang_path)) mkdir($help_lang_path);
				$help_lang_path .= "/ru";if(!file_exists($help_lang_path)) mkdir($help_lang_path);
				$help_lang_path .= "/help";if(!file_exists($help_lang_path)) mkdir($help_lang_path);
				if(is_dir($help_path) && is_dir($help_lang_path))
				{
					//Create files if none exists
					$filename = $help_path."/.tooltips.php";
					if(!file_exists($filename))
					{
						$handle=fopen($filename, "w");fwrite($handle, "<?\n\$arTooltips = array(\n);\n?>");fclose($handle);
					}
					...
					{
						$handle=fopen($lang_filename, "w");fwrite($handle, "<?\n?>");fclose($handle);
					}
					$handle=fopen($lang_filename, "r");$lang_contents = fread($handle, filesize($lang_filename));fclose($handle);
					...
					{
						if(strpos($contents, "\"${strName}\" => ")===false)
						{
							$contents = str_replace(");\n", "\t\"${strName}\" => GetMessage(\"${strName}_TIP\"),\n);\n", $contents);
							$file_modified = true;
						}
