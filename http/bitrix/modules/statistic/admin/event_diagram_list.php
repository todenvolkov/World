...
?>
<div class="graph">
<table cellspacing="0" cellpadding="0" class="graph" border="0"><tr>
<td valign="top" class="graph"><?
	...
	?><img class="graph" src="event_diagram.php?rand=<?=rand()?>&lang=<?=LANG?><?=$s?>&find_date1=<?echo $arFilter["DATE1_PERIOD"]?>&find_date2=<?=$arFilter["DATE2_PERIOD"]?>" width="<?=$diameter?>" height="<?=$diameter?>">
</td>
<td valign="top">
	<table cellpadding="2" cellspacing="0" border="0" class="legend">
		...
		?>
			...
			<td align="right"><?echo sprintf("%01.2f", $procent)."%"?></td>
			<td align="right">(<a title="<?echo GetMessage("STAT_VIEW_EVENT_LIST")?>" class="stat_link" href="event_list.php?lang=<?=LANG?>&amp;find_event_id=<?=$id?>&amp;find_date1=<?echo $arFilter["DATE1_PERIOD"]?>&amp;find_date2=<?echo $arFilter["DATE2_PERIOD"]?>&amp;set_filter=Y"><?=$counter?></a>)</td>
			<td>[<a class="stat_link" href="event_type_list.php?lang=<?=LANG?>&amp;find_id=<?=$id?>&amp;set_filter=Y"><?=$id?></a>] <a title="<?echo GetMessage("STAT_VIEW_GRAPH")?>" class="stat_link" href="event_graph_list.php?lang=<?=LANG?>&amp;find_events[]=<?=$id?>&amp;set_filter=Y"><?echo htmlspecialchars($name)?></a></font></td>
			</tr>
		<?endforeach;?>
	</table>
</td>
...
function applyFilter()
{
	...
	<?=$sFilterID?>.OnSet('<?=AddSlashes($sTableID)?>', '<?=AddSlashes($APPLICATION->GetCurPage()."?lang=".LANG."&")?>');
	return false;
}
function deleteFilter()
{
	...
	<?=$sFilterID?>.OnClear('<?=AddSlashes($sTableID)?>', '<?=AddSlashes($APPLICATION->GetCurPage()."?lang=".LANG."&")?>');
	return false;
}
</script>
...
<?$oFilter->Buttons()?>
<input type="submit" name="set_filter" value="<?=GetMessage("STAT_F_FIND")?>" title="<?=GetMessage("STAT_F_FIND_TITLE")?>" onClick="return applyFilter();">
<input type="submit" name="del_filter" value="<?=GetMessage("STAT_F_CLEAR")?>" title="<?=GetMessage("STAT_F_CLEAR_TITLE")?>" onClick="return deleteFilter();">
<?
$oFilter->End();
?>
</form>
