...
if (strlen($strError)<=0 && count($arrLegend)>0) :
	...
	?>
<div class="graph"><table cellpadding="0" cellspacing="0" border="0" class="graph"><tr><td>
	...
	<table border="0" cellspacing="0" cellpadding="0" class="legend">
		...
		<tr>
			<td valign="center">
				<div style="background-color: <?="#".$color?>"><img src="/bitrix/images/1.gif" width="45" height="2" border=0></div>
			</td>
			<td valign="center">
			&nbsp;<?
			if ($arrL["COUNTER_TYPE"]=="DETAIL") :
				?>[<a class="stat_link" href="/bitrix/admin/event_type_list.php?lang=<?=LANG?>&amp;find_id=<?=$keyL?>&amp;set_filter=Y"><?=$keyL?></a>]&nbsp;<a class="stat_link" title="<?echo GetMessage("STAT_EVENT_DYNAMIC")?>" href="/bitrix/admin/event_dynamic_list.php?lang=<?=LANG?>&amp;find_event_id=<?echo $keyL?>&amp;find_date1=<?echo $arFilter["DATE1"]?>&amp;find_date2=<?echo $arFilter["DATE2"]?>&amp;set_filter=Y"><?=$arrL["NAME"]?></a><?
			else :
...
<?$oFilter->Buttons()?>
<input type="submit" name="set_filter" value="<?=GetMessage("STAT_F_FIND")?>" title="<?=GetMessage("STAT_F_FIND_TITLE")?>" onClick="jsSelectUtils.selectAllOptions('find_events[]');<?=$oFilter->id?>.OnSet('<?=AddSlashes($sTableID)?>', '<?=AddSlashes($APPLICATION->GetCurPage()."?lang=".LANG."&")?>'); return false;">
<input type="submit" name="del_filter" value="<?=GetMessage("STAT_F_CLEAR")?>" title="<?=GetMessage("STAT_F_CLEAR_TITLE")?>" onClick="jsSelectUtils.selectAllOptions('find_events[]');jsSelectUtils.deleteSelectedOptions('find_events[]');<?=$oFilter->id?>.OnClear('<?=AddSlashes($sTableID)?>', '<?=AddSlashes($APPLICATION->GetCurPage()."?lang=".LANG."&")?>'); return false;">
<?
$oFilter->End();
?>
</form>
