...
{
	global $strError, $str_REFERER1, $str_REFERER2, $err_mess, $ID;
	...
	elseif (intval($ID)<=0)
	{
		$strSql = "
			...
			FROM
				b_stat_adv
			WHERE
				REFERER1".((strlen($str_REFERER1)>0) ? "='".$DB->ForSql($str_REFERER1,255)."'" : " is null")."
			and REFERER2".((strlen($str_REFERER2)>0) ? "='".$DB->ForSql($str_REFERER2,255)."'" : " is null")."
			";
...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_bitrix_sessid())
{
	...
	if (CheckFields())
	{
		...
		if($ID > 0)
			$DB->Update("b_stat_adv",$arFields,"WHERE ID = ".$ID, $err_mess.__LINE__);
		else
			$ID = $DB->Insert("b_stat_adv", $arFields, $err_mess.__LINE__);

		if (strlen($strError)<=0)
		{
			$DB->Query("DELETE FROM b_stat_adv_searcher WHERE ADV_ID = ".$ID, false, $err_mess.__LINE__);
			if (is_array($arSEARCHERS))
			{
				foreach ($arSEARCHERS as $searcher_id)
				{
					...
					$DB->Insert("b_stat_adv_searcher",$arFields, $err_mess.__LINE__);
				}
			}

			$DB->Query("DELETE FROM b_stat_adv_page WHERE ADV_ID = ".$ID, false, $err_mess.__LINE__);
			...
			if (is_array($arPAGES_TO))
			{
				...
				if (count($arPAGES_TO)>0)
				{
					foreach ($arPAGES_TO as $page_to)
					{
						$arFields = array(
							"ADV_ID" => $ID,
							"PAGE" => "'".$DB->ForSql($page_to, 2000)."'",
							"C_TYPE" => "'TO'",
							);
						$DB->Insert("b_stat_adv_page",$arFields, $err_mess.__LINE__);
					}
				}
			}
...
else
{
	...
	while ($zr=$z->Fetch()) $arSEARCHERS[] = $zr["SEARCHER_ID"];

	$arPAGES_TO = array();
	$strSql = "SELECT PAGE FROM b_stat_adv_page WHERE ADV_ID = ".$ID." and C_TYPE='TO'";
	$z = $DB->Query($strSql, false, $err_mess.__LINE__);
	while($zr = $z->Fetch())
		$arPAGES_TO[] = htmlspecialchars($zr["PAGE"]);
	$PAGES_TO = implode("\r\n", $arPAGES_TO);
	...
	$z = $DB->Query($strSql, false, $err_mess.__LINE__);
	while($zr = $z->Fetch())
		$arPAGES_FROM[] = htmlspecialchars($zr["PAGE"]);
	$PAGES_FROM = implode("\r\n", $arPAGES_FROM);
}
if (strlen($strError)>0)
{
	$DB->InitTableVarsForEdit("b_stat_adv", "", "str_");
}
else
{
...
if(intval($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("STAT_RESET_ADV_CONFIRM")."'))window.location='adv_list.php?clear_id=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
	);
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("STAT_DELETE_ADV_CONFIRM")."'))window.location='adv_list.php?del_id=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
	);
}
...
?>
	...
	<tr>
		<td><span class=required>*</span> referer1:</td>
	...
	<tr>
		...
		<td><textarea name="PAGES_FROM" cols="50" rows="8"><?echo $PAGES_FROM?></textarea></td>
	</tr>
	<tr valign="top">
	...
	<tr>
		...
		<td><textarea name="PAGES_TO" cols="50" rows="8"><?echo $PAGES_TO?></textarea></td>
</tr>
<?
//********************
// 2nd Tab
