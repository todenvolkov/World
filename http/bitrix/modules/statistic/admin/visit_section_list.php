...
<table cellpadding="0" cellspacing="0" border="0" class="graph">
	<tr>
		...
		<img class="graph" src="<?echo htmlspecialchars("visit_section_diagram.php?lang=".LANG."
&find_section=".urlencode($find_section)."&find_date1=".urlencode($find_date1)."&find_date2=".urlencode($find_date2)."&find_show=".urlencode($find_show)."&find_site_id=".urlencode($find_site_id)."&find_page_404=".urlencode($find_page_404)."&find_adv_data_type=".urlencode($find_adv_data_type)."&find_section_exact_match=".urlencode($find_section_exact_match).$s.GetFilterParams("find_")."&by=".urlencode($by)."&order=".urlencode($order))?>" width="<?=$diameter?>" height="<?=$diameter?>">
		...
		<table border="0" cellspacing="2" cellpadding="0" class="legend">
			...
			<tr>
					...
					<td align="right"><?=$q?>%</td>
					<td><?=$str?></td>
					<td align="right"><?echo "<a href=\"hit_list.php?lang=".LANG."&find_url=".urlencode($arVal["URL"]."%")."&find_url_exact_match=Y&set_filter=Y\">".$arVal["COUNTER"]."</a>";?></td>
			</tr>
			<?$i++;endforeach;?>
			<?if ($total==11):?>
			<tr>
					<!-- <td><?=$i?></td> -->
					<td style="background-color: <?="#".GetNextRGB($color, $total);?>"><img src="/bitrix/images/1.gif" width="12" height="12" border=0></td>
					<td align="right"><?=(number_format((($sum_counter-$top_sum)*100)/$sum_counter, 2, '.', ''))?>%</td>
					<td><?=GetMessage("STAT_OTHER")?></td>
					<td align="right"><?=($sum_counter-$top_sum)?></td>
			</tr>
			<?endif?>
		</table>
		</td>
...
<tr valign="center">
	...
	?>&nbsp;<input type="text" name="find_section" size="37" value="<?echo htmlspecialchars($find_section)?>"><?=ShowExactMatchCheckbox("find_section")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr valign="center">
	<td width="0%" nowrap><?
	...
	<td width="100%" nowrap><?
		...
		echo SelectBoxFromArray("find_show", $arr, htmlspecialchars($find_show), GetMessage("MAIN_ALL"));
		?>
		</td>
</tr>
<?
