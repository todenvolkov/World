...
?>
<h2><?echo GetMessage("STAT_DYNAMIC_GRAPH")?></h2>
<?
		...
		<tr>
			<td>
				<table cellpadding="3" cellspacing="0" border="0">
					<tr>
						<td valign="center" nowrap><img src="/bitrix/admin/country_graph.php?find_data_type=<?=$find_data_type?><?=GetFilterParams($FilterArr)?>&width=<?=$width?>&height=<?=$height?>&lang=<? echo LANG?>" width="<?=$width?>" height="<?=$height?>"></td>
					</tr>
				</table>
			</td>
		</tr>
...
<table cellspacing=0 cellpadding=10 class="graph">
	<tr>
		<td valign="top"><img src="/bitrix/admin/country_diagram.php?<?=GetFilterParams($FilterArr)?>&lang=<?=LANG?>&find_data_type=<?=$find_data_type?>" width="<?=$diameter?>" height="<?=$diameter?>"></td>
		<td valign="top">
				...
				<tr>
					...
					<td valign="center">
						<table cellspacing=0 cellpadding=0>
							<tr>
								<td style="background-color: <?="#".$color?>"><?//echo $color?><img src="/bitrix/images/1.gif" width="12" height="12" border=0></td>
							</tr>
						</table>
					</td>
					...
					?></td>
					<td nowrap><a href="city_list.php?lang=<?=LANGUAGE_ID?>&amp;find_country_id=<?echo urlencode($id)?>&amp;find_date1=<?echo urlencode($arFilter["DATE1"])?>&amp;find_date2=<?echo urlencode($arFilter["DATE2"])?>&amp;find_data_type=<?=$find_data_type?>&amp;set_filter=Y">[<?=htmlspecialchars($id)?>] <?=htmlspecialchars($name)?></a></td>
				</tr>
				<?endwhile;?>
			</table>
		</td>
...
<h2><?echo GetMessage("STAT_STATIC_GRAPH")?></h2>
...
<table cellspacing=0 cellpadding=10 class="graph">
	<tr>
		<td valign="top"><img src="/bitrix/admin/country_diagram.php?<?=GetFilterParams($FilterArr)?>&lang=<?=LANGUAGE_ID?>&find_data_type=<?=$find_data_type?>&diagram_type=TOTAL" width="<?=$diameter?>" height="<?=$diameter?>"></td>
		<td valign="top">
			<table cellpadding=2 cellspacing=0 border=0 class="legend">
				<?
				...
				<tr>
					...
					?></td>
					<td nowrap>[<?=htmlspecialchars($id)?>] <?=htmlspecialchars($name)?></td>
				</tr>
				<?endwhile;?>
			</table>
		</td>
	...
	<td width="0%" nowrap><?
		$arr = array(
			...
			"reference_id"=>array(
				...
				"HITS",
				"C_EVENTS"));
		echo SelectBoxFromArray("find_data_type", $arr, htmlspecialchars($find_data_type));
		?></td>
</tr> -->
<tr valign="top">
...
if (is_array($arrCOUNTRY)):
	...
	{
		global $find_country_id, $arrCOUNTRY;
		$ret=0;
		$ka = false;
		$kb = false;
		...
		if ($ret==0)
		{
			if (strtolower($arrCOUNTRY[$a])>strtolower($arrCOUNTRY[$b])) $ret=1;
			if (strtolower($arrCOUNTRY[$a])<strtolower($arrCOUNTRY[$b])) $ret=-1;
		}
		return $ret;
	}
