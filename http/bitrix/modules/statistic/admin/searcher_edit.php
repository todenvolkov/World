...
function Set_Params()
{
	...
	foreach ($PARAM as $pid)
	{
		...
		$arFields = array(
			"DOMAIN"	=> "'".$DB->ForSql($$var_DOMAIN)."'",
			"VARIABLE"	=> "'".$DB->ForSql($$var_VARIABLE)."'",
			"CHAR_SET"	=> "'".$DB->ForSql($$var_CHAR_SET)."'",
			);
		$strSql = "SELECT 'x' FROM b_stat_searcher_params WHERE ID='".intval($$var_PARAM_ID)."'";
		$b = $DB->Query($strSql, false, $err_mess.__LINE__);
		if ($br=$b->Fetch())
		{
			if (strlen(trim($$var_DOMAIN))<=0)
				$DB->Query("DELETE FROM b_stat_searcher_params WHERE ID='".intval($$var_PARAM_ID)."'", false, $err_mess.__LINE__);
			else
				$DB->Update("b_stat_searcher_params",$arFields,"WHERE ID='".intval($$var_PARAM_ID)."'",$err_mess.__LINE__);
		}
		elseif (strlen(trim($$var_DOMAIN))>0)
		{
				$arFields["SEARCHER_ID"] = intval($ID);
				$DB->Insert("b_stat_searcher_params",$arFields, $err_mess.__LINE__);
		}
	}
}
...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_bitrix_sessid())
{
	...
	$rsSearcher = $DB->Query($strSql, false, $err_mess.__LINE__);
	$arSearcher = $rsSearcher->Fetch();

	$DB->PrepareFields("b_stat_searcher");
	...
	if (CheckFields())
	{
		...
		if ($ID>0)
		{
			$DB->Update("b_stat_searcher",$arFields,"WHERE ID='".$ID."'",$err_mess.__LINE__);
			Set_Params();
			if (intval($HIT_KEEP_DAYS)!=$arSearcher["HIT_KEEP_DAYS"])
			{
				$arFields = array("HIT_KEEP_DAYS" => $sql_HIT_KEEP_DAYS);
				$DB->Update("b_stat_searcher_hit",$arFields,"WHERE SEARCHER_ID=$ID",$err_mess.__LINE__);
			}
		}
		else
		{
			$ID = $DB->Insert("b_stat_searcher",$arFields, $err_mess.__LINE__);
...
if (strlen($strError)>0) $DB->InitTableVarsForEdit("b_stat_searcher", "", "str_");
...
if(intval($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("STAT_DELETE_SEARCHER_CONFIRM")."'))window.location='searcher_list.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
	);
}
...
?>
	...
	<tr>
		<td><span class="required">*</span><? echo GetMessage("STAT_NAME")?></td>
		<td><input type="text" name="NAME" size="40" maxlength="50" value="<?echo $str_NAME?>"></td>
	</tr>
	<tr>
		<td><? echo GetMessage("STAT_USER_AGENT")?></td>
		<td><input type="text" name="USER_AGENT" size="40" maxlength="50" value="<?echo $str_USER_AGENT?>"></td>
	</tr>
	<tr>
		...
		<td>
			<table border="0" cellspacing="1" cellpadding="0">
				...
				?>
				<tr>
					<td><input type="hidden" name="PARAM[]" value="<?echo $i?>"><input type="hidden" name="PARAM_ID_<?echo $i?>" value="0"><input type="text" name="DOMAIN_<?echo $i?>" size="30" value="<?echo htmlspecialchars(${"DOMAIN_".$i})?>"></td>
					<td><input type="text" name="VARIABLE_<?echo $i?>" size="10" value="<?echo htmlspecialchars(${"VARIABLE_".$i})?>"></td>
					<td><input type="text" name="CHAR_SET_<?echo $i?>" size="20" value="<?echo htmlspecialchars(${"CHAR_SET_".$i})?>"></td>
				</tr>
				<?
				$i++;
				endwhile;
