...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	if($f_COUNTER > 0)
	{
		$href = htmlspecialchars("event_list.php?lang=".LANGUAGE_ID."&find_event_id=".urlencode($find_event_id)."&find_event_id_exact_match=Y&find_date1=".urlencode($f_DATE_STAT)."&find_date2=".urlencode($f_DATE_STAT)."&set_filter=Y");
...
if($dynamic_days>=2 && function_exists("ImageCreate")):
	...
	$aContext = array(
		array(
			...
			"ICON"=>"btn_stat_graph",
			"LINK"=>htmlspecialchars("event_graph_list.php?lang=".LANGUAGE_ID."&find_events[]=".$find_event_id."&find_date1=".$arFilter["DATE1"]."&find_date2=".$arFilter["DATE2"]."&set_filter=Y"),
...
<tr>
	...
	<td>
	<input name="find_event_id" id="find_event_id" value="<?echo htmlspecialchars($find_event_id)?>">
