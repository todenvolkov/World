...
if ($message)
	$DB->InitTableVarsForEdit("b_stop_list", "", "str_");
...
if(intval($ID)>0)
{
	...
	if ($STAT_RIGHT>="W")
	{
		$aMenu[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("STAT_DELETE_STOPLIST_CONFIRM")."'))window.location='stoplist_list.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
		);
	}
}
...
?>
	...
	<tr valign="top">
		<td align="right"><?echo GetMessage("STAT_START_DATE")." (".CLang::GetDateFormat()."):"?></td>
		<td><?echo CalendarDate("DATE_START", $str_DATE_START, "form1", "19")?></td>
	</tr>
	<tr valign="top">
		<td align="right"><?echo GetMessage("STAT_END_DATE")." (".CLang::GetDateFormat()."):"?></td>
