...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if ($grby=="S")
		$row->AddViewField("URL_FROM",'<a title="'.GetMessage("STAT_GO_LINK").'" href="http://'.$f_URL_FROM.'">&raquo;</a>&nbsp;<a href="'.htmlspecialchars('referer_list.php?find_from_domain='.urlencode(htmlspecialcharsback($f_URL_FROM)).'&group_by=U&set_filter=Y').'">'.$txt.'</a>');
	elseif ($grby=="U")
		$row->AddViewField("URL_FROM",'<a title="'.GetMessage("STAT_GO_LINK").'" href="'.$f_URL_FROM.'">&raquo;</a>&nbsp;<a href="'.htmlspecialchars('referer_list.php?find_from='.urlencode(htmlspecialcharsback($f_URL_FROM)).'&group_by=none&set_filter=Y').'">'.$txt.'</a>');
	else
...
<tr>
	...
	<td><input type="text" name="find_from" size="47" value="<?echo htmlspecialchars($find_from)?>"><?=ShowExactMatchCheckbox("find_from")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr>
	<td><?=GetMessage("STAT_F_PROTOCOL")?>:</td>
	<td><input type="text" name="find_from_protocol" size="47" value="<?echo htmlspecialchars($find_from_protocol)?>"></td>
</tr>
<tr>
	<td><?=GetMessage("STAT_F_DOMAIN")?>:</td>
	<td><input type="text" name="find_from_domain" size="47" value="<?echo htmlspecialchars($find_from_domain)?>"></td>
</tr>
<tr>
	<td><?=GetMessage("STAT_F_PAGE")?>:</td>
	<td><input type="text" name="find_from_url" size="47" value="<?echo htmlspecialchars($find_from_url)?>"><?=ShowExactMatchCheckbox("find_from_detail")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr>
	...
	?>&nbsp;<?
		echo SelectBoxFromArray("find_to_404", array("reference"=>array(GetMessage("STAT_YES"), GetMessage("STAT_NO")), "reference_id"=>array("Y","N")), htmlspecialchars($find_to_404), GetMessage("STAT_404"));
	?>&nbsp;<input type="text" name="find_to" size="34" value="<?echo htmlspecialchars($find_to)?>"><?=ShowExactMatchCheckbox("find_to")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr>
