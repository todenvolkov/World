...
?>
<table border="0" cellspacing="1" cellpadding="1" class="list">
	...
	<tr>
		<td valign="top" nowrap>
			<?echo GetMessage("STAT_SESSION_ID")?></td>
		<td>
			<a target="_blank" href="session_list.php?lang=<?=LANGUAGE_ID?>&amp;find_id=<?echo $f_SESSION_ID?>&amp;find_id_exact_match=Y&amp;set_filter=Y"><?echo $f_SESSION_ID?></a></td>
	</tr>
	<tr>
		<td valign="top" nowrap>
			<?echo GetMessage("STAT_TIME")?></td>
		<td>
			<?echo $f_DATE_HIT?></td>
	</tr>
	<tr>
		<td valign="top" nowrap>
			<?echo GetMessage("STAT_REFERER")?></td>
	...
	<tr>
		<td valign="top" nowrap>
			<?echo GetMessage("STAT_IP")?></td>
		<td>
			<? $arr = explode(".",$f_IP) ?><?=GetWhoisLink($f_IP)?>&nbsp;[<a target="_blank" title="<?echo GetMessage("STAT_ADD_TO_STOPLIST_TITLE")?>" href="stoplist_edit.php?lang=<?=LANGUAGE_ID?>&amp;net1=<?echo $arr[0]?>&amp;net2=<?echo $arr[1]?>&amp;net3=<?echo $arr[2]?>&amp;net4=<?echo $arr[3]?>"><?echo GetMessage("STAT_STOP")?></a>]</td>
	</tr>
	<tr>
	...
	<tr>
		<td valign="top" nowrap>
			<?echo GetMessage("STAT_USER_AGENT")?></td>
		<td>
			<?echo $f_USER_AGENT?></td>
	</tr>
	<tr>
		<td valign="top" nowrap>
			<?echo GetMessage("STAT_METHOD")?></td>
		<td>
			<?echo $f_METHOD?></td>
	</tr>
	<?if($USER->IsAdmin()):?>
	<tr>
		<td valign="top" nowrap>
			<?echo GetMessage("STAT_COOKIES")?></td>
		<td>
			<?if($f_COOKIES):?>
				<div style="overflow:auto;"><?echo str_replace("\n", "<br>", urldecode($f_COOKIES))?></div>
			<?else:?>
				&nbsp;
			<?endif?>
		</td>
