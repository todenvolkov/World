...
$aTabs = array(
	array("DIV" => "load_csv_tab", "TAB" => GetMessage("STAT_FROM_FILE"), "ICON"=>"main_user_edit", "TITLE"=>GetMessage('STAT_LOADING_FROM_CSV')),
	array("DIV" => "load_manual_tab", "TAB" => GetMessage("STAT_FROM_FORM"), "ICON"=>"main_user_edit", "TITLE"=>GetMessage('STAT_LOADING_FROM_TABLE')),
);
...
if (CModule::IncludeModule("fileman")) $fileman_module = "Y";
...
if (strlen($success)<=0 || $success=="Y") :
	if (strlen($base_currency)>0) : ?>
	<p><?=GetMessage("STAT_BASE_CURRENCY")." ".$base_currency?></p>
	<?elseif (CModule::IncludeModule("currency")):?>
	<p><span class="required"><?=GetMessage("STAT_BASE_CURRENCY_NOT_INSTALLED")?></span>&nbsp;[&nbsp;<a href="/bitrix/admin/settings.php?lang=<?=LANG?>&amp;mid=statistic"><?=GetMessage("STAT_CHOOSE_CURRENCY")?></a>&nbsp;]</p>
	<?endif;
endif;?>

<?if($success=="N"):?>
<?CAdminMessage::ShowMessage(array("MESSAGE"=>GetMessage("STAT_EVENTS_LOADING"), "TYPE"=>"OK"));?>
...
<p><a href="<?=htmlspecialchars($link)?>"><?echo GetMessage("STAT_NEXT_STEP")?></a></p>
...
function addNewRow(currentId)
{
	...
	var oCell = oRow.insertCell(i++);
	oCell.noWrap = true;
	oCell.innerHTML = '<input type="hidden" name="ARR[]" value="'+id+'"><input type="text" size="14" name="EVENT_ID_'+id+'" value="" OnFocus="addNewRow('+id+')"><a href="javascript:void(0)" onClick="SelectEvent(\'form1\',\'EVENT_ID_'+id+'\')"><img src="/bitrix/images/statistic/event.gif" width="15" height="15" border="0" alt="<?echo GetMessage("STAT_SELECT_EVENT")?>"></a>';
	...
	strSelectBox = '<select name="CURRENCY_'+id+'" size="1">';
	...
	var oCell = oRow.insertCell(i++);
	oCell.innerHTML = '<a href="javascript: Copy('+id+')"><img src="/bitrix/images/statistic/copy.gif" width="15" height="15" border=0 class="tb2" hspace="2" alt="<?echo GetMessage("STAT_COPY")?>" onMouseOut="this.className=\'tb2\';" onMouseOver="this.className=\'btn\';" onMouseDown="this.className=\'btnDown\';"  onMouseUp="this.className=\'btn\';"></a>';
...
function ClickPreview()
{
	...
	document.getElementsByName("check_unique")[0].disabled = v.checked;
...
?>
	...
	<tr>
		...
		<td width="40%"><?echo InputType("checkbox", "check_unique", "Y", $check_unique, false); ?></td>
	</tr>
	<tr>
...
<?$tabControl->BeginNextTab();?>
	<tr>
		...
		<tr>
			...
			<input type="text" size="14" name="EVENT_ID_<?echo $i?>" value="<?=htmlspecialchars($event_id)?>" OnFocus="addNewRow(<?echo $i?>)">
			<a href="javascript:void(0)" onClick="SelectEvent('form1','EVENT_ID_<?echo $i?>')">
			<img src="/bitrix/images/statistic/event.gif" width="15" height="15" border=0 alt="<?echo GetMessage("STAT_SELECT_EVENT")?>"></a>
			</td>
			<td><input type="text" size="20" name="EVENT3_<?echo $i?>" value="<?=htmlspecialchars($event3)?>" OnFocus="addNewRow(<?echo $i?>)"></td>
			<td nowrap><?echo CalendarDate("DATE_ENTER_".$i, htmlspecialchars($date_enter), "form1", "18")?></td>
			<td><input type="text" style="width:100%" name="PARAM_<?echo $i?>" value="<?=htmlspecialchars($param)?>" OnFocus="addNewRow(<?echo $i?>)"></td>
			<td><input type="text" size="5" name="MONEY_<?echo $i?>" value="<?=htmlspecialchars($money)?>" OnFocus="addNewRow(<?echo $i?>)"></td>
			<?if ($currency_module=="Y") :?>
			...
			<td><a href="javascript: Copy(<?=$i?>)"><img src="/bitrix/images/statistic/copy.gif" width="15" height="15" border=0 class="tb2" hspace="2" alt="<?echo GetMessage("STAT_COPY")?>" onMouseOut="this.className='tb2';" onMouseOver="this.className='btn';" onMouseDown="this.className='btnDown';"  onMouseUp="this.className='btn';"></a></td>
		</tr>
		<?
		$i++;
		endwhile;
...
<?if($STAT_RIGHT=="W"):?>
	...
	<input type="submit" name="Load" value="<?echo GetMessage("STAT_LOAD")?>">
