...
function CheckFilter()
{
	...
	foreach($arr as $ar)
	{
		...
		$DB->CompareDates($ar["date1"], $ar["date2"])==1) $str.= $ar["mess3"]."<br>";
	}
...
if (strlen($arFilter["DATE1_PERIOD"])>0)
	$headers[]=array("id"=>"STAT_PERIOD_HITS", "content"=>GetMessage("STAT_PERIOD_HITS")." $arFilter[DATE1_PERIOD] $arFilter[DATE2_PERIOD]", "sort"=>"s_period_hits", "default"=>true, "align"=>"right");
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if ($f_PERIOD_HITS>0)
	{
		$txt="<a title=\"".GetMessage("STAT_HITS_SHOW")."\" href=\"hit_searcher_list.php?lang=".LANGUAGE_ID."&find_searcher_id=$f_ID&find_searcher_id_exact_match=Y&find_date1=$arFilter[DATE1_PERIOD]&find_date2=$arFilter[DATE2_PERIOD]&set_filter=Y\">$f_PERIOD_HITS</a>";
	...
	if ($STAT_RIGHT>="W")
		$arActions[] = array("ICON"=>"delete", "TITLE"=>GetMessage("STAT_DELETE_SEARCHER"), "ACTION"=>"javascript:if(confirm('".GetMessage("STAT_CONFIRM")."')) window.location='?lang=".LANGUAGE_ID."&action=delete&ID=$f_ID&".bitrix_sessid_get()."'", "TEXT"=>GetMessage("STAT_DELETE"));
...
<tr>
	...
	<td width="0%" nowrap><input type="text" name="find_name" value="<?echo htmlspecialchars($find_name)?>" size="47"><?=ShowExactMatchCheckbox("find_name")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr>
...
<tr>
	...
	<td width="0%" nowrap><input type="text" maxlength="10" name="find_hits1" value="<?echo htmlspecialchars($find_hits1)?>" size="9"><?echo "&nbsp;".GetMessage("STAT_TILL")."&nbsp;"?><input type="text" maxlength="10" name="find_hits2" value="<?echo htmlspecialchars($find_hits2)?>" size="9"></td>
</tr>
<tr>
	<td width="0%" nowrap><?echo GetMessage("STAT_F_LAST_HIT_DATE")." (".CLang::GetDateFormat("SHORT")."):"?></td>
