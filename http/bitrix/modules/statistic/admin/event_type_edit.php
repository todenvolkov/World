...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $STAT_RIGHT=="W" && check_bitrix_sessid())
{
	...
	if($cEventType->CheckFields($arFields, $ID))
	{
		...
		if ($ID>0)
		{
			$DB->Update("b_stat_event",$arFields,"WHERE ID='".$ID."'",$err_mess.__LINE__);
			if (intval($KEEP_DAYS)!=$arEvent["KEEP_DAYS"])
			{
				$arFields = array("KEEP_DAYS" => $sql_KEEP_DAYS);
				$DB->Update("b_stat_event_list",$arFields,"WHERE EVENT_ID=$ID",$err_mess.__LINE__);
			}
		}
		else
		{
...
if($bVarsFromForm)
	$DB->InitTableVarsForEdit("b_stat_event", "", "str_");
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("STAT_RESET_EVENT_TYPE_CONFIRM")."'))window.location='event_type_list.php?ID=".$ID."&action=clear&lang=".LANG."&".bitrix_sessid_get()."';",
	);
	$aMenu[] = array(
		"TEXT"=>GetMessage("STAT_DELETE"),
		"TITLE"=>GetMessage("STAT_DELETE_EVENT_TYPE"),
		"LINK"	=> "javascript:if(confirm('".GetMessage("STAT_DELETE_EVENT_TYPE_CONFIRM")."'))window.location='event_type_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
