						...
						<td nowrap><a title="<?=GetMessage("STAT_GO")?>" href="<?=$page?>">&raquo;</a>&nbsp;<?
							...
							if($path_id!=$parent_id) :
								$prev_parent_path = $path_id;
                 							?><a href="javascript:<?=$table_id==""?$sTableID:$table_id?>.GetAdminList('<?=$APPLICATION->GetCurPage()?>?lang=<?=LANG?>&find_diagram_type=COUNTER&parent_id=<?=$path_id?>&context=<?=$context?>');"><?
								if($err_404=="Y"):
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	{
		$str .= "";
		$str .= '<a title="'.GetMessage("STAT_GO").'" href="'.$f_LAST_PAGE.'">&raquo;</a>&nbsp;<a title="'.GetMessage("STAT_NEXT_STEP").'" href="javascript:'.($table_id==""?$sTableID:$table_id).'.GetAdminList(\''.$APPLICATION->GetCurPage().'?lang='.LANG.'&find_diagram_type=COUNTER&parent_id='.$f_PATH_ID.'&context='.$context.'\')">';
	...
	else
	{
		...
		foreach($arrPAGES as $page)
		{
			if(strlen($page)>0)
			{
				...
				$str .= '<a title="'.GetMessage("STAT_GO").'" href="'.$page.'">&raquo;</a>&nbsp;<a title="'.GetMessage("STAT_NEXT_PAGES").'" href="javascript:'.($table_id==""?$sTableID:$table_id).'.GetAdminList(\''.$APPLICATION->GetCurPage().'?lang='.LANG.'&find_diagram_type=COUNTER&parent_id='.$path_id.'&context='.$context.'\');">';
				if($err_404=="Y")
...
<tr>
	...
	<td><input type="text" name="find_steps1" size="10" value="<?echo htmlspecialchars($find_steps1)?>"><?echo "&nbsp;".GetMessage("STAT_TILL")."&nbsp;"?><input type="text" name="find_steps2" size="10" value="<?echo htmlspecialchars($find_steps2)?>"></td>
</tr>
<tr valign="center">
	...
	?>&nbsp;<?
		echo SelectBoxFromArray("find_first_page_404", array("reference"=>array(GetMessage("STAT_YES"), GetMessage("STAT_NO")), "reference_id"=>array("Y","N")), htmlspecialchars($find_first_page_404), GetMessage("STAT_404"));
	?>&nbsp;<input type="text" name="find_first_page" size="37" value="<?echo htmlspecialchars($find_first_page)?>"><?=ShowExactMatchCheckbox("find_first_page")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr valign="center">
	...
	?>&nbsp;<?
		echo SelectBoxFromArray("find_page_404", array("reference"=>array(GetMessage("STAT_YES"), GetMessage("STAT_NO")), "reference_id"=>array("Y","N")), htmlspecialchars($find_page_404), GetMessage("STAT_404"));
	?>&nbsp;<input type="text" name="find_page" size="37" value="<?echo htmlspecialchars($find_page)?>">&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr valign="center">
	...
	?>&nbsp;<?
		echo SelectBoxFromArray("find_last_page_404", array("reference"=>array(GetMessage("STAT_YES"), GetMessage("STAT_NO")), "reference_id"=>array("Y","N")), htmlspecialchars($find_last_page_404), GetMessage("STAT_404"));
	?>&nbsp;<input type="text" name="find_last_page" size="37" value="<?echo htmlspecialchars($find_last_page)?>"><?=ShowExactMatchCheckbox("find_last_page")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr valign="top">
...
<tr valign="top">
	...
	<td width="0%" nowrap><?
		...
		echo SelectBoxFromArray("find_adv_data_type", $arr, htmlspecialchars($find_adv_data_type));
		?></td>
</tr>
