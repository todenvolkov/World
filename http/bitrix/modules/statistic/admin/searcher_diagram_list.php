...
<table cellpadding="0" cellspacing="0" border="0" class="graph">
	<tr>
		...
		?><img class="graph" src="<?echo htmlspecialchars($url)?>" width="<?=$diameter?>" height="<?=$diameter?>">
		...
		<table border="0" cellspacing="2" cellpadding="0" class="legend">
			...
			while ($wr=$w->Fetch())
			{
				...
				$sum += $count;
				if ($count>0)
					$arr[] = array("ID"=>$wr["ID"], "NAME"=>$wr["NAME"], "COUNTER"=>$count);
			}
			reset($arr);
			$i = 0;
			...
			<tr>
				...
				<td nowrap><?echo sprintf("%01.2f", $procent)."%"?></td>
				<td nowrap>(<a title="<?echo GetMessage("STAT_VIEW_SEARCHER_HITS")?>" href="/bitrix/admin/hit_searcher_list.php?lang=<?=LANGUAGE_ID?>&find_searcher_id=<?=$id?>&find_date1=<?echo $arFilter["DATE1_PERIOD"]?>&find_date2=<?echo $arFilter["DATE2_PERIOD"]?>&set_filter=Y"><?=$counter?></a>)</td>
				<td nowrap>[<a title="<?=GetMessage("STAT_SEARCHER_LIST_OPEN")?>" href="searcher_list.php?lang=<?=LANGUAGE_ID?>&find_id=<?=$id?>&set_filter=Y"><?=$id?></a>] <a title="<?echo GetMessage("STAT_VIEW_SEARCHER_GRAPH")?>" href="searcher_graph_list.php?lang=<?=LANGUAGE_ID?>&find_searchers[]=<?=$id?>&set_filter=Y"><?=htmlspecialchars($name)?></a></td>
			</tr>
			<?
			endwhile;
			?>
...
if (is_array($arrSEARCHERS)):
	...
	$arrSEARCHERS_lower = array();
	foreach($arrSEARCHERS as $k => $v)
		$arrSEARCHERS_lower[$k] = strtolower($v);
