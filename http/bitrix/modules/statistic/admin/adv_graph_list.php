...
if ($dynamic_days<2) :
	...
	if (!$message) :
		...
		$s = "";
		if ($find_sessions=="Y")		$s .= "&find_sessions=Y";
		if ($find_sessions_back=="Y")	$s .= "&find_sessions_back=Y";
		if ($find_guests=="Y")			$s .= "&find_guests=Y";
		if ($find_guests_back=="Y")		$s .= "&find_guests_back=Y";
		if ($find_new_guests=="Y")		$s .= "&find_new_guests=Y";
		if ($find_hosts=="Y")			$s .= "&find_hosts=Y";
		if ($find_hosts_back=="Y")		$s .= "&find_hosts_back=Y";
		if ($find_hits=="Y")			$s .= "&find_hits=Y";
		if ($find_hits_back=="Y")		$s .= "&find_hits_back=Y";
	...
	<?endif;
		...
		if (sizeof($find_events)>0) :
			...
			$s = "";
			foreach ($find_events as $eid) $s .= "&find_events[]=".$eid;
			$str .= $s;
	?>
	<h2><?=GetMessage("STAT_GRAPH_2")?></h2>
	<div class="graph">
	<table cellspacing="0" cellpadding="0" class="graph" border="0"><tr>
	<td valign="top" class="graph">
	...
	<table border="0" cellspacing="0" cellpadding="0" class="graph">
		<tr>
					...
					<tr>
						<td>
							<table cellpadding="3" cellspacing="1" border="0" class="legend">
								...
								<tr>
									<td><img src="/bitrix/admin/graph_legend.php?color=<?=$arrColor["HITS"]?>" width="45" height="2"></td>
								...
								<tr>
									<td><img src="/bitrix/admin/graph_legend.php?color=<?=$arrColor["GUESTS"]?>" width="45" height="2"></td>
								...
								<tr>
									<td><img src="/bitrix/admin/graph_legend.php?color=<?=$arrColor["HOSTS"]?>" width="45" height="2"></td>
								...
								<tr>
									<td><img src="/bitrix/admin/graph_legend.php?color=<?echo $arrColor["NEW_GUESTS"]?>" width="45" height="2"></td>
									<td></td>
			...
			<?if ($graph_2=="Y") :?>
				<?if ($graph_1=="Y") :?>
				<td><img src="/bitrix/images/1.gif" width="20" height="1"></td>
				<?endif;?>
			<td valign="top">
				<table border="0" cellspacing="0" cellpadding="0" >
					<tr>
...
<input type="hidden" name="find_date1" value="<?=htmlspecialchars($find_date1)?>">
<input type="hidden" name="find_date2" value="<?=htmlspecialchars($find_date2)?>">
<input type="hidden" name="ADV_ID" value="<?=$ADV_ID?>">
<input type="hidden" name="set_show" value="Y">
<?foreach($find_events as $val):?>
<input type="hidden" name="find_events[]" value="<?=htmlspecialchars($val)?>">
<?endforeach?>
<div class="graph">
