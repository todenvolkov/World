...
class CKeepStatistics
{
	...
	function ReallyKeep()
	{
		global $USER, $APPLICATION, $SESS_AUTH, $FAVORITES, $HIT_ID, $STOP_SAVE_STATISTIC, $STOP_MESSAGE, $STOP_REDIRECT_URL, $STOP, $STOP_LIST_ID, $STOP_MESSAGE_LID, $event1, $event2, $event3, $goto, $money, $currency, $last_referer1, $last_referer2, $referer1, $referer2, $referer3, $site_id;
		...
		{
			if ($_SESSION["SESS_ADD_TO_FAVORITES"]=="Y")
			{
				$FAVORITES="Y";
				$_SESSION["SESS_ADD_TO_FAVORITES"]="";
			}
			$FAVORITES = ($FAVORITES=="Y") ? "Y" : "N";
					...
					************************************************/
					...
					if($_SESSION["SESS_SESSION_ID"] <= 0)
					{
						...
						elseif (intval($_SESSION["SESS_LAST_ADV_ID"])>0) // comeback?
						{
							...
							$arFields["ADV_BACK"] = "'Y'";
							$arFields["REFERER1"] = "'".$DB->ForSql($last_referer1,255)."'";
							$arFields["REFERER2"] = "'".$DB->ForSql($last_referer2,255)."'";
						}
						...
						elseif (intval($_SESSION["SESS_LAST_ADV_ID"])>0) // иначе если это возврат то
						{
							...
							$arFields["LAST_REFERER1"] = "'".$DB->ForSql($last_referer1,255)."'";
							$arFields["LAST_REFERER2"] = "'".$DB->ForSql($last_referer2,255)."'";
						}
					...
					************************************************/
					...
					if($_SESSION["SESS_SESSION_ID"] > 0)
					{
						if ($SAVE_HITS!="N")
						{
							...
							$HIT_ID = $DB->Insert("b_stat_hit", $arFields, "File: ".__FILE__."<br>Line: ".__LINE__);
							if($ERROR_404=="N")
							{
								CStatistics::Set404("b_stat_hit", "ID = ".intval($HIT_ID), array("URL_404" => "Y"));
							}
						}
						...
						************************************************/
						...
						if ($SESSION_NEW=="Y") $arFields["FIRST_HIT_ID"] = $HIT_ID;
						...
						if (is_array($_SESSION["SESS_FROM_SEARCHERS"]) && count($_SESSION["SESS_FROM_SEARCHERS"])>0)
						{
						...
						if (intval($_SESSION["SESS_REFERER_ID"])>0)
						{
...
echo '<html>
	<head>
		...
		'.(strlen($STOP_REDIRECT_URL) > 0? '<meta http-equiv="Refresh" content="3;URL='.htmlspecialchars($STOP_REDIRECT_URL).'">': '').'
	</head>
	<body>
