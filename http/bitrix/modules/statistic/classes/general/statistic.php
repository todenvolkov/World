...
class CAllStatistics extends CKeepStatistics
{
	...
	{
		$err_mess = "File: ".__FILE__."<br>Line: ";
		global $INC_GUESTS_BACK_COUNTER;
		$DB = CDatabase::GetModuleConnection('statistic');
	...
	function Set_Guest()
	{
		...
		global $USER, $APPLICATION, $SESS_AUTH, $INC_GUESTS_BACK_COUNTER, $last_referer1, $last_referer2;
		$DB = CDatabase::GetModuleConnection('statistic');
			...
			if ($REPAIR_COOKIE_GUEST=="Y")
			{
				...
				if ($COOKIE_ADV>0)
				{
					...
					if ($wr = $w->Fetch())
					{
						...
						if (strpos($COOKIE_ADV, "Y")===false)
						{
							// не нужно увеличивать счетчик посетителей на возврате по рекламной кампании
							$INC_GUESTS_BACK_COUNTER = "N";
						}
	...
	function BlockVisitorActivity()
	{
		...
		if(COption::GetOptionString("statistic", "DEFENCE_ON")=="Y")
		{
			...
			if (intval($_SESSION["SESS_SEARCHER_ID"])<=0 || (intval($_SESSION["SESS_SEARCHER_ID"])>0 && $_SESSION["SESS_SEARCHER_CHECK_ACTIVITY"]=="Y"))
			{
