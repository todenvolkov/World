...
class CStatEvent extends CAllStatEvent
{
	function GetListByGuest($GUEST_ID, $EVENT_ID=false, $EVENT3=false, $SEC=false)
	{
		...
		$strSqlSearch = "";
		if ($EVENT_ID!==false) $strSqlSearch .= " and E.EVENT_ID='".intval($EVENT_ID)."' ";
		if ($EVENT3!==false) $strSqlSearch .= " and E.EVENT3='".$DB->ForSql($EVENT3,255)."' ";
		if ($SEC!==false) $strSqlSearch .= " and E.DATE_ENTER>DATE_ADD(now(),INTERVAL- $SEC SECOND) ";
