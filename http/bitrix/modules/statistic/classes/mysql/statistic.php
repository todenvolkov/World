...
class CStatistics extends CAllStatistics
{
	...
	function CleanUpReferer()
	{
		...
		if ($DAYS>=0)
		{
			...
			$strSql = "
				DELETE FROM b_stat_referer WHERE
					(
						to_days(now())-to_days(DATE_LAST)>=$DAYS or
						DATE_LAST is null or
						length(DATE_LAST)<=0
					)
					and ID not in ($str)
	...
	function CleanUpSearcherHits()
	{
		...
		$strSql = "
			DELETE FROM b_stat_searcher_hit WHERE
				to_days(now())-to_days(DATE_HIT)>=ifnull(HIT_KEEP_DAYS,$DAYS) or
				DATE_HIT is null or
				length(DATE_HIT)<=0
			";
		$DB->Query($strSql, false, $err_mess.__LINE__);
	...
	function CleanUpEvents()
	{
		...
		$strSql = "
			DELETE FROM b_stat_event_list WHERE
				to_days(now())-to_days(DATE_ENTER)>=ifnull(KEEP_DAYS,$DAYS) or
				DATE_ENTER is null or
				length(DATE_ENTER)<=0
			";
		$DB->Query($strSql, false, $err_mess.__LINE__);
