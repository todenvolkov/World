...
class CCityLookup_geoip_extension extends CCityLookup
{
	...
	{
		return $this->CCityLookup_geoip_extension($arDBRecord);
	}

	function CCityLookup_geoip_extension($arDBRecord = false)
	{
		parent::CCityLookup($arDBRecord);
		if(!$arDBRecord)
		{
	...
	function GetFullInfo()
	{
		...
		$ar["POSTAL_CODE"] = array(
			...
			"VALUE" => htmlspecialchars($this->postal_code),
		);
		$ar["LONGITUDE"] = array(
			"TITLE" => GetMessage("STAT_CITY_GEOIP_EXT_LONGITUDE"),
			"VALUE~" => $this->longitude,
			"VALUE" => htmlspecialchars($this->longitude),
		);
		$ar["LATITUDE"] = array(
			"TITLE" => GetMessage("STAT_CITY_GEOIP_EXT_LATITUDE"),
			"VALUE~" => $this->latitude,
			"VALUE" => htmlspecialchars($this->latitude),
		);
		return $ar;
	}
