...
E1 - error open idx
E2 - error open db
E3 - idx[0] > idx[1] (there was no interval found)
E4 - strlen(record)!=record_size
E5 - there is no IP in index
...
function i2c_get_country($ip=false)
{
	...
	{
		if (strlen($I2C_ERROR)<=0) $I2C_ERROR = "E5";
		$country = 'N0';
	}
	return $country;
}
...
function i2c_search_in_index($ip, $idx_name=IP_IDX_FILENAME)
{
	...
	while (!feof($dbidx))
	{
		...
		if (is_array($data) && count($data)>0)
		{
			...
			{
				$rnum = (intval($prev_recnum)>0) ? intval($prev_recnum) : intval($recnum);
				return array($rnum, (int) $data[1]);
			}
			$prev_recnum = $recnum;
...
function i2c_search_in_db($ip, $idx, $db_name=IP_DB_FILENAME)
{
	...
	if (!$ipdb)
	{
		if (strlen($I2C_ERROR)<=0) $I2C_ERROR = "E2";
		return $country;
	}
	$i1_size = 10;
	$i2_size = 10;
	...
	while (!feof($ipdb) && !($range_start <= $ip && $range_end >= $ip))
	{
		...
		{
			if (strlen($I2C_ERROR)<=0) $I2C_ERROR = "E3";
			$country = "N0";
			break;
		}
		...
		{
			if (strlen($I2C_ERROR)<=0) $I2C_ERROR =	"E4";
			$country = "N0";
			break;
		}
