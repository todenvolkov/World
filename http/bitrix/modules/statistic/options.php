...
if ($STAT_RIGHT>="R"):
	...
	$tabControl2 = new CAdminTabControl("tabControl2", $aTabs);
	...
	if($REQUEST_METHOD=="POST" && strlen($Update.$Apply)>0 && $STAT_RIGHT>="W" && check_bitrix_sessid())
	{
		if (CheckFDate($next_exec, GetMessage("STAT_OPT_WRONG_NEXT_EXEC")))
		{
			...
			$arr = array();
			$arr = preg_split("/[\n\r]+/", $BROWSERS);
			$DB->Query("DELETE FROM b_stat_browser", false, $err_mess.__LINE__);
			foreach ($arr as $u)
			{
				if (strlen($u)>0)
				{
					$arFields = Array("USER_AGENT" => "'".$DB->ForSql($u,255)."'");
	...
	$BROWSERS = "";
	$rows = $DB->Query("SELECT USER_AGENT FROM b_stat_browser ORDER BY ID", false, $err_mess.__LINE__);
	...
	if(strlen($runsql)>0 && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_bitrix_sessid())
	{
		...
		if(count($ar = CStatistics::GetDDL())>0)
		{
			foreach($ar as $arDDL)
			{
				...
				{
					$strError.=$arDDL["SQL_TEXT"].":(".$DB->db_Error.")<br>";
					$bDone=false;
				}
			}
		}
	...
	$tabControl->Begin();
	?>
	<form name="form_settings" method="POST" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
	<?$tabControl->BeginNextTab();?>
	<tr>
		<td valign="top" width="50%">
			<?echo GetMessage("STAT_OPT_DAILY_REPORT_TIME", array("#DATE_FORMAT#" => CLang::GetDateFormat("FULL")))?>
		</td>
		<td valign="middle" nowrap width="50%"><?
			...
			echo CalendarDate("next_exec", htmlspecialchars($next_exec), "form_settings", "19");
		?></td>
	</tr>
	<?
	foreach($arOPTIONS["TAB1"] as $key => $Option):
		if (!is_array($Option)):
			if($key == "USE_AUTO_OPTIMIZE" && strtolower($DB->type) == "mysql"):?>
				<tr>
					<td valign="top" width="50%"><label for="<?=$key?>"><?echo GetMessage("STAT_OPT_USE_AUTO_OPTIMIZE")?></label></td>
					<td valign="top" nowrap width="50%"><input type="checkbox" name="<?=$key?>" id="<?=$key?>" value="Y" <?if(${$key}=="Y") echo "checked";?>></td>
				</tr>
			<?elseif($key == "BASE_CURRENCY" && CModule::IncludeModule("currency")):?>
				<tr>
					<td valign="top" width="50%"><?echo GetMessage("STAT_OPT_BASE_CURRENCY")?></td>
					<td valign="middle" width="50%"><?echo CCurrency::SelectBox("BASE_CURRENCY", $BASE_CURRENCY, " ", True, "") ?><br>
	...
	<tr>
		...
		<td><input size="3" type="text" name="DEFENCE_DELAY" id="DEFENCE_DELAY" value="<?=htmlspecialchars($DEFENCE_DELAY)?>">&nbsp;<?echo GetMessage("STAT_OPT_DEFENCE_DELAY_MEASURE_SEC")?></td>
	</tr>
	<tr>
		<td nowrap><?echo GetMessage("STAT_OPT_DEFENCE_STACK_TIME")?></td>
		<td><input size="3" type="text" name="DEFENCE_STACK_TIME" id="DEFENCE_STACK_TIME" value="<?=htmlspecialchars($DEFENCE_STACK_TIME)?>">&nbsp;<?echo GetMessage("STAT_OPT_DEFENCE_STACK_TIME_MEASURE_SEC")?></td>
	</tr>
	<tr>
		<td nowrap><?echo GetMessage("STAT_OPT_DEFENCE_MAX_STACK_HITS")?></td>
		<td><input size="3" type="text" name="DEFENCE_MAX_STACK_HITS" id="DEFENCE_MAX_STACK_HITS" value="<?=htmlspecialchars($DEFENCE_MAX_STACK_HITS)?>">&nbsp;<?echo GetMessage("STAT_OPT_DEFENCE_MAX_STACK_HITS_MEASURE")?></td>
	</tr>
	<tr>
	...
	foreach($arOPTIONS["TAB5"] as $key => $Option):
		if(!is_array($Option)):
			if($key == "ADV_EVENTS_DEFAULT"):?>
				<tr>
					<td valign="top" width="50%"><?echo GetMessage("STAT_OPT_ADV_EVENTS_DEFAULT")?></td>
					<td valign="middle" nowrap width="50%"><?
						$arr = array(
							"reference" => array(
						...
						echo SelectBoxFromArray("ADV_EVENTS_DEFAULT", $arr, htmlspecialchars($ADV_EVENTS_DEFAULT));
					?></td>
				</tr>
			<?else:?>
				<tr class="heading">
			...
			<table class="internal">
				...
				while($arEvent = $rsEvents->Fetch()):
					...
					<tr>
						...
						<td align="center"><?echo $arDescr["IS_INSTALLED"]? GetMessage("MAIN_YES"): GetMessage("MAIN_NO")?></td>
						<td align="center"><?echo $arDescr["IS_INSTALLED"]? ($arDescr["CAN_LOOKUP_COUNTRY"]? GetMessage("MAIN_YES"): GetMessage("MAIN_NO")): "-"?></td>
						<td align="center"><?echo $arDescr["IS_INSTALLED"]? ($arDescr["CAN_LOOKUP_CITY"]? GetMessage("MAIN_YES"): GetMessage("MAIN_NO")): "-"?></td>
						<td align="center"><input type="radio" name="IP_LOOKUP_CLASS" value="<?echo $arDescr["CLASS"]?>" <?echo ($arDescr["CLASS"] == $selected? "checked": "")?>></td>
					</tr>
				<?endwhile?>
			</table>
	...
	foreach($arOPTIONS["TAB3"] as $key => $Option):
		if(!is_array($Option)):
			if ($key == "BROWSERS"):?>
				<tr>
					<td valign="top" width="50%"><?=GetMessage("STAT_OPT_STORAGE_BROWSERS")?></td>
					<td valign="middle" nowrap width="50%"><textarea name="BROWSERS" rows="5" cols="30"><?=htmlspecialchars($BROWSERS)?></textarea></td>
				</tr>
			<?else:?>
				<tr class="heading">
					<td valign="top" colspan="2" align="center"><b><?=$Option?></b></td>
				...
				<td valign="top" nowrap width="50%">
					<?if($type[0]=="text"):
						...
						if(strlen($Option[3]) > 0):?>
							&nbsp;<label for="<?echo htmlspecialchars($Option[0])?>_clear"><?echo GetMessage("STAT_OPT_TIME_CLEAR")?>:</label>&nbsp;<input type="checkbox" name="<?echo htmlspecialchars($Option[0])?>_clear" id="<?echo htmlspecialchars($Option[0])?>_clear" value="Y">
						<?endif;?>
						<?if(strlen($Option[4]) > 0):?>
	...
	foreach($arOPTIONS["TAB4"] as $key => $Option):
		if(!is_array($Option)):
			if ($key=="SKIP_STATISTIC_WHAT"):?>
				<tr>
					<td valign="top" width="50%"><?echo GetMessage("STAT_OPT_SKIP_RULES")?>:</td>
			...
			<?elseif($key == "SKIP_STATISTIC_GROUPS"):
				...
				?>
				<tr>
					<td valign="top" width="50%"><?=GetMessage("STAT_OPT_SKIP_GROUPS_LABEL")?>:<br><img src="/bitrix/images/statistic/mouse.gif" width="44" height="21" border=0 alt=""></td>
					<td valign="middle" nowrap width="50%"><?=SelectBoxMFromArray("arSKIP_STATISTIC_GROUPS[]", array("REFERENCE" => $ug, "REFERENCE_ID" => $ug_id), $arSKIP_STATISTIC_GROUPS, "", false, 10);?></td>
				</tr>
			<?elseif($key == "SKIP_STATISTIC_IP_RANGES"):?>
				<tr>
					<td valign="top" width="50%"><?=GetMessage("STAT_OPT_SKIP_RANGES_LABEL")?>:</td>
					<td valign="middle" nowrap width="50%"><textarea name="SKIP_STATISTIC_IP_RANGES" rows="5" cols="30"><?=htmlspecialchars($SKIP_STATISTIC_IP_RANGES)?></textarea><br>
	...
	<?require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/admin/group_rights.php");?>
	<?$tabControl->Buttons();?>
	<input <?if ($STAT_RIGHT<"W") echo "disabled" ?> type="submit" name="Update" value="<?=GetMessage("MAIN_SAVE")?>" title="<?=GetMessage("MAIN_OPT_SAVE_TITLE")?>">
	...
	<?if(strlen($_REQUEST["back_url_settings"])>0):?>
		<input <?if ($STAT_RIGHT<"W") echo "disabled" ?> type="button" name="Cancel" value="<?=GetMessage("MAIN_OPT_CANCEL")?>" title="<?=GetMessage("MAIN_OPT_CANCEL_TITLE")?>" onclick="window.location='<?echo htmlspecialchars(CUtil::addslashes($_REQUEST["back_url_settings"]))?>'">
		<input type="hidden" name="back_url_settings" value="<?=htmlspecialchars($_REQUEST["back_url_settings"])?>">
	<?endif?>
	<input <?if ($STAT_RIGHT<"W") echo "disabled" ?> type="submit" name="RestoreDefaults" title="<?echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS")?>" OnClick="return confirm('<?echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>')" value="<?echo GetMessage("MAIN_RESTORE_DEFAULTS")?>">
	<?=bitrix_sessid_post();?>
	<?$tabControl->End();?>
	<?if(strlen($_REQUEST["back_url_settings"])>0):?>
		<input type="hidden" name="back_url_settings" value="<?=htmlspecialchars($_REQUEST["back_url_settings"])?>">
	...
	<form name="cleanupform" method="POST" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
	<?$tabControl2->BeginNextTab();?>
		<tr>
			<td width="10%" nowrap><?echo GetMessage("STAT_OPT_CLEANUP_DATE", array("#DATE_FORMAT#"=>CLang::GetDateFormat("SHORT")))?></td>
			<td valign="top" width="90%"><?echo CalendarDate("cleanup_date", htmlspecialchars($cleanup_date), "cleanupform", "10")?></td>
		</tr>
		<tr>
	...
	<script language="JavaScript">
		...
		{
			if(confirm('<?=GetMessage("STAT_OPT_CLEANUP_CONFIRMATION")?>'))
	...
	<?if (strtolower($DB->type)=="mysql"):?>
		<form name="optimizeform" method="POST" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
		<?$tabControl2->BeginNextTab();?>
		<tr>
			<td align="left" colspan="2">
				...
				<SCRIPT LANGUAGE="JavaScript">
					...
					{
						if(confirm('<?=GetMessage("STAT_OPT_OPTIMIZE_CONFIRMATION")?>'))	document.optimizeform.submit();
					}
				</SCRIPT>
			</td>
		</tr>
		...
		<tr>
			<td align="left" colspan="2">
				...
				foreach($arDDL as $DDL):
					echo htmlspecialchars($DDL["SQL_TEXT"])."<br>";
				endforeach;?>
				<br>
