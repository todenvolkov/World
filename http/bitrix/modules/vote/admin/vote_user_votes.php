...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if ($VOTE_RIGHT=="W")
        	$arActions[] = array("ICON"=>"delete", "TEXT"=>GetMessage("VOTE_DELETE_U"), "ACTION"=>$lAdmin->ActionRedirect("javascript:if(confirm('".GetMessage("VOTE_DELETE_CONFIRMATION")."')) window.location='?lang=".LANGUAGE_ID."&action=delete&ID=$f_ID&".bitrix_sessid_get()."'"));
...
<tr> 
	...
	<td><input type="text" name="find_vote_user" size="47" value="<?echo htmlspecialchars($find_vote_user)?>"><?=InputType("checkbox", "find_vote_user_exact_match", "Y", $find_vote_user_exact_match, false, "", "title='".GetMessage("VOTE_EXACT_MATCH")."'")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr> 
	<td><?echo GetMessage("VOTE_F_SESSION")?></td>
	<td><input type="text" name="find_session" size="47" value="<?echo htmlspecialchars($find_session)?>"><?=InputType("checkbox", "find_session_exact_match", "Y", $find_session_exact_match, false, "", "title='".GetMessage("VOTE_EXACT_MATCH")."'")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
</tr>
<tr> 
	<td>IP</td>
...
<tr>
	<td nowrap><?echo GetMessage("VOTE_F_DATE")." (".CLang::GetDateFormat("SHORT")."):"?></td>
...
<tr>
	...
	<td nowrap><input type="text" name="find_vote" size="47" value="<?echo htmlspecialchars($find_vote)?>"><?=InputType("checkbox", "find_vote_exact_match", "Y", $find_vote_exact_match, false, "", "title='".GetMessage("VOTE_EXACT_MATCH")."'")?>&nbsp;<?=ShowFilterLogicHelp()?><br><?
		echo SelectBox("find_vote_id", CVote::GetDropDownList(), GetMessage("VOTE_ALL"), htmlspecialchars($find_vote_id));
		?></td>
</tr>
<tr valign="top">
