...
class CAllVoteQuestion
{
	...
	function CheckFields($ACTION, &$arFields, $ID = 0)
	{
		...
		if (is_set($arFields, "QUESTION") || $ACTION == "ADD")
		{
			...
			$arFields["QUESTION"] = substr($arFields["QUESTION"], 0, 2000);
		}
...
/***************** Cleaning cache/**********************************/
		...
		if (!empty($strUpdate)):
			$strSql = "UPDATE b_vote_question SET ".$strUpdate." WHERE ID=".$ID;
...
/***************** /Event ******************************************/
	...
	function GetList($VOTE_ID, &$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		global $DB;
		...
		if (in_array($by1, array("ID", "TIMESTAMP", "ACTIVE", "DIAGRAM", "C_SORT", "QUESTION", "REQUIRED"))):
	...
	function GetListEx($arOrder = array("ID" => "ASC"), $arFilter=array())
	{
		...
		foreach ($arOrder as $by => $order)
		{
			...
			if ($by == "ID") $arSqlOrder[] = " VQ.ID ".$order." ";
			elseif ($by == "ACTIVE") $arSqlOrder[] = " VQ.ACTIVE ".$order." ";
			else
			{
...
/***************** /Event ******************************************/
	...
	function Reset($ID, $VOTE_ID = false)
	{
		global $DB;
