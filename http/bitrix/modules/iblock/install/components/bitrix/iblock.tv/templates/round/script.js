...
jsPublicTV.prototype.GeneratePlayer = function()
{
	...
	if(/*this.PlayerConfig.file && */this.PlayerConfig.width && this.PlayerConfig.height && this.PlayerConfig.block_id[this['Sections'][this.CurrentItem.Section]['Items'][this.CurrentItem.Item].Type])
	{
		if(this['Sections'][this.CurrentItem.Section]['Items'][this.CurrentItem.Item].Type == 'wmv') //WMV
		{
			...
			this.Player.wmv = new jeroenwijering.Player(BX(this.PlayerConfig.block_id.wmv), '/bitrix/components/bitrix/player/wmvplayer/wmvplayer.xaml',  this.PlayerConfig);
			this.PlayerConfig.height +=4; //height fix
		...
		else if(this['Sections'][this.CurrentItem.Section]['Items'][this.CurrentItem.Item].Type == 'flv') //FLW
		{
			if (window.jwplayer)
			{
				this.Player.oJw = jwplayer(this.PlayerConfig.block_id.flv);
...
jsPublicTV.prototype.PlayFile = function(i, j, autoplay, handle)
{
	...
	if(params.type == 'wmv')
	{
		...
		if(this.Player.wmv)
		{
			...
			if(autoplay == true)
				this.RunDelayFunction(function(){_player.sendEvent('LOAD', params.file); _this.RunDelayFunction(function(){_player.sendEvent('PLAY');}, 50, 0, 50);}, 50, 0, BX.browser.IsSafari() ? 100 : 0); //Safari +100
			else
...
jsPublicTV.prototype.EffectCreateOverlay = function()
{
	...
	{
		var _this = this;
		this.RunDelayFunction(function(){_this.Player.flv.sendEvent("LOAD", _this['Sections'][_this.CurrentItem.Section]['Items'][_this.CurrentItem.Item].File);}, 50, 0, 50 + (BX.browser.IsSafari() ? 100 : 0)); //FF +50, Safari +150
	}
