	...
	<div id="bx_tv_block_<?=$arResult['PREFIX']?>" style="width: <?=$arParams['WIDTH']?>px;">
		...
		<?$APPLICATION->IncludeComponent(
			...
			Array(
				...
				"PLAYER_ID" => "bitrix_tv_".$arResult["PREFIX"],
			),
			$component,
			Array("HIDE_ICONS" => "Y")
		);?>
			<div id="myIDwmv_<?= $arResult['PREFIX']?>" style="height: <?=$arParams['HEIGHT']?>px; padding-top:<?= ( $arResult['CORRECTION']['FLV'] - $arResult['CORRECTION']['WMV'])?>px; display: none;"></div>
		</div>
	...
	if(jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>])
	{
		jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].Init(
			...
			'tv_description_<?=$arResult['PREFIX']?>',
			{
				block_id:
				{
					wmv: 'myIDwmv_<?=$arResult['PREFIX']?>',
					flv: 'bitrix_tv_<?=$arResult['PREFIX']?>_div'
				},
