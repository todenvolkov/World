<?
$MESS ['IBLOCK_TYPE_TIP'] = "Select here one of the existing information block types. Click <b><i>OK</i></b> to load information blocks of the selected type.";
$MESS ['IBLOCK_ID_TIP'] = "Select here one of the existing information blocks. If you select <b><i>(other)</i></b>, you will have to specify the information block ID in the field beside. ";
$MESS ['SECTION_ID_TIP'] = "This field contains an expression that evaluates to the section ID. By default, the expression is <b>={\$_REQUEST[\"SECTION_ID\"]}</b>.";
$MESS ['ELEMENT_ID_TIP'] = "The expression that evaluates to the section ID. The default value is <b>={\$_REQUEST[\"ELEMENT_ID\"]}</b>.";
$MESS ['SECTION_CODE_TIP'] = "Specify here the section mnemonic code. You can leave the field empty if <b>section ID</i> is specified.";
$MESS ['ELEMENT_CODE_TIP'] = "Specify here the element mnemonic code. You can leave the field empty if <b>element ID</i> is specified.";
$MESS ['ELEMENT_SORT_FIELD_TIP'] = "Specifies the field by which the photos will be sorted. Used for navigation via the \"Next\" and \"Prev\" links.";
$MESS ['ELEMENT_SORT_ORDER_TIP'] = "The sorting order.";
$MESS ['FIELD_CODE_TIP'] = "Select here the fields that you want to show in the list of section elements. If you select <i><b>none</b></i> and do not define field expressions below, the default fields will be shown.";
$MESS ['PROPERTY_CODE_TIP'] = "Select information block properties that you want to display in an element list. Selecting <i>none</i> and not specifying property codes below will cause properties to be not displayed.";
$MESS ['SECTION_URL_TIP'] = "Specify here the URL of a page displaying the section contents.";
$MESS ['DETAIL_URL_TIP'] = "Specify here the path to an information block element details page.";
$MESS ['META_KEYWORDS_TIP'] = "This drop-down list shows properties available for this information block. Choose the one that contains keywords.";
$MESS ['META_DESCRIPTION_TIP'] = "This drop-down list shows properties available for this information block. Choose the one that contains the description.";
$MESS ['DISPLAY_PANEL_TIP'] = "If checked, the tool buttons will be shown on the administration toolbar and the component edit area toolbar in the site edit mode.";
$MESS ['SET_TITLE_TIP'] = "Checking this option will set the page title to the photo title.";
$MESS ['CACHE_TYPE_TIP'] = "<i>Auto</i>: the cache is valid during the time predefined in the cache settings;<br /><i>Cache</i>: always cache for the period specified in the next field;<br /><i>Do not cahce</i>: no caching is performed.";
$MESS ['CACHE_TIME_TIP'] = "Specify here the period of time during which the cache is valid.";
$MESS ['AJAX_MODE_TIP'] = "Enables AJAX in the component.";
$MESS ['AJAX_OPTION_SHADOW_TIP'] = "Specifies to shade modifiable area on AJAX transition.";
$MESS ['AJAX_OPTION_JUMP_TIP'] = "Specifies to scroll to component when AJAX transition completes.";
$MESS ['AJAX_OPTION_STYLE_TIP'] = "Specifies to download and process CSS styles of component on AJAX transition.";
$MESS ['AJAX_OPTION_HISTORY_TIP'] = "Allows \"Back\" and \"Forward\" browser buttons for AJAX transitions.";
?>