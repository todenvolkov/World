...
class CAllIBlockSection
{
	...
	function Delete($ID, $bCheckPermissions = true)
	{
		...
		if($s = $s->Fetch())
		{
			...
			$iblocksections = CIBlockSection::GetList(Array(), Array("SECTION_ID"=>$ID), false, array("ID"));
		...
		if(strlen($this->LAST_ERROR)<=0)
		{
			...
			if(IntVal($arFields["IBLOCK_SECTION_ID"])>0)
			{
				$r = CIBlockSection::GetList(
					Array(),
					Array(
						"ID" => IntVal($arFields["IBLOCK_SECTION_ID"]),
						"CHECK_PERMISSIONS" => $arFields["CHECK_PERMISSIONS"],
					),
