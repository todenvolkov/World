...
//							$arUserGroups[] = SONET_RELATIONS_TYPE_FRIENDS2;
	...
	function Delete($ID)
	{
		...
		for($i=0; $i<count($arSql); $i++)
		{
			...
			while ($zr = $z->Fetch())
			{
				...
				$DB->Query("DELETE FROM b_iblock_element_lock WHERE IBLOCK_ELEMENT_ID=".IntVal($zr["ID"]), false, $err_mess.__LINE__);
	...
	function CheckFields(&$arFields, $ID=false)
	{
		...
		if($IBLOCK_ID <= 0)
		{
			...
			if($ar = $res->Fetch())
				$IBLOCK_ID = intval($ar["IBLOCK_ID"]);
		}

		//Read iblock metadata
		static $IBLOCK_CACHE = array();
		if(!array_key_exists($IBLOCK_ID, $IBLOCK_CACHE))
		{
			if($IBLOCK_ID > 0)
	...
	function __GetFileContent($FILE_ID)
	{
		...
		if($arFile)
		{
			...
			if($max_file_size > 0)
			{
				if(filesize($_SERVER["DOCUMENT_ROOT"]."/".$arFile["SRC"]) > $max_file_size)
					return "";
			}
