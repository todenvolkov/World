...
class CIBlockPropertyElementAutoComplete
{
	public function GetUserTypeDescription() 
	{
		return array( 
			...
			//"CheckFields" => array('CIBlockPropertyElementAutoComplete','CheckFields'),
			//"GetLength" => array('CIBlockPropertyElementAutoComplete','GetLength'),
			//"ConvertToDB" => array('CIBlockPropertyElementAutoComplete','ConvertToDB'),
			//"ConvertFromDB" => array('CIBlockPropertyElementAutoComplete','ConvertFromDB'),
	...
	protected function GetLinkElement($intElementID,$intIBlockID)
	{
		...
		if (0 >= $intIBlockID) $intIBlockID = 0;
		$intElementID = intval($intElementID);
		if (0 >= $intElementID) return false;
		if (false == array_key_exists($intElementID,$cache))
		{
			$arFilter = array();
			...
			if ($arElement = $rsElements->GetNext(true,false))
			{
				$arResult = array(
					'ID' => $arElement['ID'],
					'NAME' => $arElement['NAME'],
	...
	protected function GetPropertyValue($arProperty,$arValue)
	{
		...
		if (0 < intval($arValue['VALUE']))
		{
			...
			if (true == is_array($mxResult))
			{
				...
				if (true == isset($arProperty['PROPERTY_VALUE_ID']))
				{
	...
	protected function GetValueForAutoComplete($arProperty,$arValue,$arBanSym,$arRepSym)
	{
		...
		if (true == is_array($mxResult))
		{
			$strResult = str_replace($arBanSym,$arRepSym,$mxResult['NAME']).' ['.$mxResult['ID'].']';
		}
		return $strResult;
	}
	...
	protected function GetValueForAutoCompleteMulti($arProperty,$arValues,$arBanSym,$arRepSym)
	{
		...
		if (true == is_array($arValues))
		{
			foreach ($arValues as $intPropertyValueID => $arOneValue)
			{
				...
				if (true == is_array($mxResult))
				{
					$arResult[$intPropertyValueID] = str_replace($arBanSym,$arRepSym,$mxResult['NAME']).' ['.$mxResult['ID'].']';
				}
			}
		}
		return $arResult;
	...
	public function CheckFields($arProperty,$arValue)
	{

	}
	
	public function GetLength($arProperty,$arValue)
	{
	...
	public function GetPropertyFieldHtml($arProperty, $arValue, $strHTMLControlName)
	{
		...
		{
		ob_start();
		?><?php $control_id = $APPLICATION->IncludeComponent(
	...
	public function GetPropertyFieldHtmlMulty($arProperty, $arValues, $strHTMLControlName)
	{
		...
		?><?php $control_id = $APPLICATION->IncludeComponent(
			...
			array(
				"INPUT_NAME" => $strHTMLControlName['VALUE'].'[]',
				"INPUT_NAME_STRING" => "inp_".$strHTMLControlName['VALUE'],
				"INPUT_VALUE_STRING" => htmlspecialcharsback(implode("\n",self::GetValueForAutoCompleteMulti($arProperty,$arValues,$arSymbols['BAN_SYM'],$arSymbols['REP_SYM']))),
				"START_TEXT" => GetMessage('BT_UT_EAUTOCOMPLETE_MESS_INVITE'),
				"MULTIPLE" => $arProperty["MULTIPLE"],
				"MAX_WIDTH" => $arSettings['MAX_WIDTH'],
...
?><?php $APPLICATION->IncludeComponent(
	...
	public function GetAdminListViewHTML($arProperty, $arValue, $strHTMLControlName)
	{
		$strResult = '';
		$mxResult = self::GetPropertyValue($arProperty,$arValue);
		if (true == is_array($mxResult)) $strResult = $mxResult['NAME'].' [<a href="iblock_element_edit.php'.
						'?WF=Y'.
	...
	public function GetPublicViewHTML($arProperty, $arValue, $strHTMLControlName) 
	{
		...
		if (0 < $arValue['VALUE'])
		{
			if (false == array_key_exists($arValue['VALUE'],$cache))
			{
				$arFilter = array();
	...
	public function GetPublicEditHTML($arProperty, $arValue, $strHTMLControlName)
	{
		
	}
	
	public function GetSearchContent($arProperty, $arValue, $strHTMLControlName)
	{
	...
	public function PrepareSettings($arFields)
	{
		...
		$strView = '';
		$strView = ((true == isset($arFields['USER_TYPE_SETTINGS']['VIEW'])) && (true == in_array($arFields['USER_TYPE_SETTINGS']['VIEW'],$arViewsList)) ? $arFields['USER_TYPE_SETTINGS']['VIEW'] : current($arViewsList));
		
		$strShowAdd = (true == isset($arFields['USER_TYPE_SETTINGS']['SHOW_ADD']) ? $arFields['USER_TYPE_SETTINGS']['SHOW_ADD'] : '');
		$strShowAdd = ('Y' == $strShowAdd ? 'Y' : 'N');

		$intMaxWidth = intval(true == isset($arFields['USER_TYPE_SETTINGS']['MAX_WIDTH']) ? $arFields['USER_TYPE_SETTINGS']['MAX_WIDTH'] : 0);
		if (0 >= $intMaxWidth) $intMaxWidth = 0;
		
		$intMinHeight = intval(true == isset($arFields['USER_TYPE_SETTINGS']['MIN_HEIGHT']) ? $arFields['USER_TYPE_SETTINGS']['MIN_HEIGHT'] : 0);
		if (0 >= $intMinHeight) $intMinHeight = 24;
		
		$intMaxHeight = intval(true == isset($arFields['USER_TYPE_SETTINGS']['MAX_HEIGHT']) ? $arFields['USER_TYPE_SETTINGS']['MAX_HEIGHT'] : 0);
		if (0 >= $intMaxHeight) $intMaxHeight = 1000;
		
		$strBannedSymbols = trim(true == isset($arFields['USER_TYPE_SETTINGS']['BAN_SYM']) ? $arFields['USER_TYPE_SETTINGS']['BAN_SYM'] : ',');
		$strBannedSymbols = str_replace(' ','',$strBannedSymbols);
		if ('' == $strBannedSymbols) $strBannedSymbols = ',';
		
		$strOtherReplaceSymbol = '';
		$strReplaceSymbol = (true == isset($arFields['USER_TYPE_SETTINGS']['REP_SYM']) ? $arFields['USER_TYPE_SETTINGS']['REP_SYM'] : ' ');
		if (BT_UT_AUTOCOMPLETE_REP_SYM_OTHER == $strReplaceSymbol)
		{
			$strOtherReplaceSymbol = (true == isset($arFields['USER_TYPE_SETTINGS']['OTHER_REP_SYM']) ? substr($arFields['USER_TYPE_SETTINGS']['OTHER_REP_SYM'],0,1) : '');
		...
		$strIBlockMess = (true == isset($arFields['USER_TYPE_SETTINGS']['IBLOCK_MESS']) ? $arFields['USER_TYPE_SETTINGS']['IBLOCK_MESS'] : '');
	...
	public function GetAdminFilterHTML($arProperty, $strHTMLControlName)
	{
		...
		if ((true == isset($_REQUEST[$strHTMLControlName["VALUE"]])) && (true == is_array($_REQUEST[$strHTMLControlName["VALUE"]])))
		{
			$strValue = implode("\n",self::GetValueForAutoCompleteMulti($arProperty,$_REQUEST[$strHTMLControlName["VALUE"]],$arSymbols['BAN_SYM'],$arSymbols['REP_SYM']));
		}
		elseif ((true == isset($GLOBALS[$strHTMLControlName["VALUE"]])) && (true == is_array($GLOBALS[$strHTMLControlName["VALUE"]])))
		{
			$strValue = implode("\n",self::GetValueForAutoCompleteMulti($arProperty,$GLOBALS[$strHTMLControlName["VALUE"]],$arSymbols['BAN_SYM'],$arSymbols['REP_SYM']));
		}
	...
	public function AddFilterFields($arProperty, $strHTMLControlName, &$arFilter, &$filtered)
	{
		...
		if ((true == isset($_REQUEST[$strHTMLControlName["VALUE"]])) && (true == is_array($_REQUEST[$strHTMLControlName["VALUE"]])))
		{
			$arFilterValues = $_REQUEST[$strHTMLControlName["VALUE"]];
		}
		elseif ((true == isset($GLOBALS[$strHTMLControlName["VALUE"]])) && (true == is_array($GLOBALS[$strHTMLControlName["VALUE"]])))
		{
			$arFilterValues = $GLOBALS[$strHTMLControlName["VALUE"]];
		}
		
		if (false == empty($arFilterValues))
		{
