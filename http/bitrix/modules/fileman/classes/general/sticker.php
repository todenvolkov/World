...
class CSticker
{
	...
	function GetPagesList()
	{
		...
		switch(strtoupper($DB->type))
		{
			case "MYSQL":
				$strSql = "
				SELECT
					DISTINCT  ST.PAGE_URL, ST.PAGE_TITLE, ST.DATE_UPDATE
				FROM
					b_sticker ST
				WHERE
					ST.DELETED='N'
					AND ((ST.PERSONAL='Y' AND ST.CREATED_BY=".intVal($userId).") OR ST.PERSONAL='N')
					ORDER BY ST.DATE_UPDATE asc
					LIMIT ".$topCount;
			break;
			case "MSSQL":
				$strSql = "
				SELECT
					DISTINCT TOP ".$topCount." ST.PAGE_URL, ST.PAGE_TITLE, ST.DATE_UPDATE
				FROM
					b_sticker ST
				WHERE
					ST.DELETED='N'
			...
			case "ORACLE":
				...
				WHERE
					...
					AND ((ST.PERSONAL='Y' AND ST.CREATED_BY=".intVal($userId).") OR ST.PERSONAL='N')
					AND ROWNUM <= ".$topCount."
					ORDER BY ST.DATE_UPDATE asc NULLS FIRST";
			break;
		}
	...
	function GetCount($Params)
	{
		...
		$strSqlSearch .= "\n AND ST.CLOSED='N' AND ST.DELETED='N'";
		...
		return 'var BXST_MESS =
{
	...
	function BBParseToHTML($str, $bForList = false)
	{
		if (!$oParser)
