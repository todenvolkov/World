...
?>
	...
	for(var i=0;i<map_point_data.length;i++){
		var obPlacemark = new map.bx_context.YMaps.Placemark(new map.bx_context.YMaps.GeoPoint(map_point_data[i].LON, map_point_data[i].LAT), {'style': map_styles[map_point_data[i]['CAT2']] }); //,'draggable': true
...
function admin_mode_save(){
	...
	for (var i=0; i<map_point_all.length; i++){
		...
		val+=map_point_all[i]['ID']+':'+koord[1]+','+koord[0]+';';
	}
...
&nbsp;&nbsp;<input type="submit" name="admin_submit" value="Сохранить" onClick="javascript:admin_mode_save();">
</form>
</div>
<? endif; ?>
<?
