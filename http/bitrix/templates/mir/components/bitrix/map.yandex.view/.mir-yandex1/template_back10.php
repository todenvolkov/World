...
if ($_POST['admin_submit']!='' && $USER->isAdmin()){
	...
	for ($i=0;$i<sizeof($pdata);$i++) {
		if ($pdata[$i]!=""){
			CIBlockElement::SetPropertyValues($pdata[$i], 6, $pdata[$i+1], "KOORD1");
		}
	}
	*/
...
var Dialog_screen = new BX.CDialog({'title': 'Заказ видеорекламы','head': '','content':'','width': '540','height': '300','buttons': [ '<input type="submit" value="В корзину" id="to_basket" onClick="javascript:Dialog_screen.Close();">', '<input type="submit" value="Закрыть" onClick="javascript:Dialog_screen.Close();">' ]});


function make_num(num){
	return num.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');
}
function screen_recount(){	// перерасчет экранов
	var days = Math.floor( (new Date(document.getElementById('screendate2').value.replace(/(\d+)\.(\d+)\.(\d+).*/, '$2/$1/$3')) - new Date(document.getElementById('screendate1').value.replace(/(\d+)\.(\d+)\.(\d+).*/, '$2/$1/$3'))) / (1000*24*60*60) ) + 1;
	if (days <= 0 ){
		document.getElementById('screendays').innerHTML = 'ошибка';
	}else{
		document.getElementById('screendays').innerHTML = 'дней: '+days;
		var cost = days*0.5;
		document.getElementById('screencost').innerHTML = make_num(cost.toFixed(2));
	}
        return false;
}
...
function show_window_screen(i){
	Dialog_screen.SetContent('<div id="bbb"></div>');
