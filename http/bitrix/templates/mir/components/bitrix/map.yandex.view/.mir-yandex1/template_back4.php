<?
//$zzz = stat("/home/temp/public_html/csv/status.csv");
//echo $zzz["mtime"]."<br>".$zzz["ctime"];
if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true) die();

$status_file = "/home/temp/public_html/csv/status.csv";
...
<table cellspacing="0" cellpadding="0" border="0" align="center" width="100%"> 
	<tbody> 
		<tr>
  			...
  			<td height="50" valign="center" align="left"><table width="90%" border="0" cellpadding="0" cellspacing="0"><tr><?=implode("",$rz_html);?><td width=70>Мой заказ<br><a href="#" onClick="javascript:return show_hide_overlay(-1);"><img src="/bitrix/templates/mir/images/OrderItemsShow.png" width="47" height="41" border="0" title="Мой заказ" id="imgc_-1" style="opacity: 0.2"></a></td></tr></table></td>
		</tr>
	</tbody>
</table>
<?
...
map_showhide[-1] = 0;
for (var i=0; i < <?=sizeof($rz_index);?>; i++){
	map_showhide[i] = 1;
}
...
function show_hide_overlay(type){
	...
	if (type==-1){
		if (map_showhide[type]==0){ // показываются все невидимые включая невидымые корзины
			document.getElementById('imgc_'+type).style.opacity = 0.2;
			for(var i=0;i<map_point_data.length;i++){
				if (map_point_data[i]['CAT']>-1 && map_showhide[ map_point_data[i]['CAT'] ]!=1) map_main.addOverlay(arObjects.PLACEMARKS[arObjects_skip+i]);
			}
			for(var i=0;i<map_showhide.length;i++){
				document.getElementById('imgc_'+i).style.opacity = 1;
				map_showhide[i]	= 1;
			}
		}else if (map_showhide[type]==1){ // гаснут все видимые, кроме корзины, невидимые корзины - появляются
			document.getElementById('imgc_'+type).style.opacity = 1;
			for(var i=0;i<map_point_data.length;i++){
				if (map_point_data[i]['CAT']>-1 && map_showhide[ map_point_data[i]['CAT'] ]==1){ 
					map_main.removeOverlay(arObjects.PLACEMARKS[arObjects_skip+i]);
				}else if (map_point_data[i]['CAT']==-1 && map_showhide[ map_point_data[i]['CAT_ORIG'] ]==0){ 
					map_main.addOverlay(arObjects.PLACEMARKS[arObjects_skip+i]);
				}
			}
			for(var i=0;i<map_showhide.length;i++){
				document.getElementById('imgc_'+i).style.opacity = 0.2;
				map_showhide[i]	= 0;
			}
		}
	}else{
		if (map_showhide[type]==0){
			document.getElementById('imgc_'+type).style.opacity = 0.2;
			for(var i=0;i<map_point_data.length;i++){
				if (map_point_data[i]['CAT']==type || (map_point_data[i]['CAT']==-1 && map_point_data[i]['CAT_ORIG']==type && map_showhide[-1]==0)) map_main.removeOverlay(arObjects.PLACEMARKS[arObjects_skip+i]);
			}
		}else if (map_showhide[type]==1){
			document.getElementById('imgc_'+type).style.opacity = 1;
			for(var i=0;i<map_point_data.length;i++){
				if (map_point_data[i]['CAT']==type || (map_point_data[i]['CAT']==-1 && map_point_data[i]['CAT_ORIG']==type && map_showhide[-1]==0)) map_main.addOverlay(arObjects.PLACEMARKS[arObjects_skip+i]);
			}		
		}
	}
	return false;
