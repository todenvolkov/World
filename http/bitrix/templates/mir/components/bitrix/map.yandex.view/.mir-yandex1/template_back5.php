...
<table cellspacing="0" cellpadding="0" border="0" align="center" width="100%"> 
	<tbody> 
		<tr>
  			...
  			<td height="50" valign="center" align="left" id="outdoor_header"></td>
		</tr>
	</tbody>
</table>
<?
...
<link rel="stylesheet" href="<?=$templateFolder?>/calendar.css">
<style type="text/css">
...
<script src="<?=$templateFolder?>/calendar.js"></script>
...
document.getElementById("outdoor_header").innerHTML = '<table width="90%" border="0" cellpadding="0" cellspacing="0"><tr>'+map_header+'<td width=70>Мой заказ<br><a href="#" onClick="javascript:return show_hide_overlay(-1);"><img src="/bitrix/templates/mir/images/OrderItemsShow.png" width="47" height="41" border="0" title="Мой заказ" id="imgc_-1" class="opacity1"></a></td></tr><tr><td><input type=checkbox name="check_status" id="check_status" onChange="javascript:return show_hide_status();" value="1"></td></tr></table>';
...
function show_hide_point2(i,show){
	var zz = (document.getElementById('check_status').checked==true)?1:0;
	if (arObjects.PLACEMARKS[i].metaDataProperty.show!=show){
		if (show==0){ // нас хотят погасить
			// если мы не в включенной корзине,
			if (!(map_showhide[-1]==1 && map_point_data[i-arObjects_skip]['BASKET']>0)){
				// если иконка сверху погасла или неудовлетворяем флагу                 
				//aa=1 zz=0 = true
				//aa=1 zz=1 = false
				//aa=0 zz=0 = true
				//aa=0 zz=1 = true
				if (map_showhide[ arObjects.PLACEMARKS[i].metaDataProperty.show ]==0 || (map_point_data[i-arObjects_skip]['STATUS']==1 && zz=1)){
					map_main.removeOverlay(arObjects.PLACEMARKS[i]);
					arObjects.PLACEMARKS[i].metaDataProperty.show = show;
  				}
			}
		}else{ // если нас хотят показать
			// если иконка сверху горит и удовлетворяем флагу
			if (map_showhide[ arObjects.PLACEMARKS[i].metaDataProperty.show ]==1 && !(map_point_data[i-arObjects_skip]['STATUS']==1 && zz=1)){
				map_main.addOverlay(arObjects.PLACEMARKS[i]);
				arObjects.PLACEMARKS[i].metaDataProperty.show = show;
  			}	
		}
	}
...
function show_hide_status(){
	var zz = (document.getElementById('check_status').checked==true)?0:1;
	for (var i=arObjects_skip; i<arObjects.PLACEMARKS.length; i++){
		if (map_point_data[i-arObjects_skip]['STATUS']==1){ // если занят итем, группа включена и он не в открытой корзине то гасим/показываем
			if (map_showhide[arObjects.PLACEMARKS[i].metaDataProperty.cat]==1 && !(map_point_data[i-arObjects_skip]['BASKET']>0 && map_showhide[-1]==1)){
				show_hide_point(i,zz);
			}
		}
	}
	return false;	
...
function show_hide_overlay(type){
	...
	var zz = (document.getElementById('check_status').checked==true)?0:1;
	if (type==-1){
		if (map_showhide[type]==0){ // показываются все
		...
		}else if (map_showhide[type]==1){ // гаснут все кроме корзины
			...
			for (var i=arObjects_skip; i<arObjects.PLACEMARKS.length; i++){
				if (map_point_data[i-arObjects_skip]['BASKET']>0){
					show_hide_point(i,1);
				}else{
					show_hide_point(i,0);
				}
		...
		if (map_showhide[type]==0){ // надо скрыть слой, если только мы не в корзине и корзина открыта
			document.getElementById('imgc_'+type).className = "opacity1";
			for (var i=arObjects_skip; i<arObjects.PLACEMARKS.length; i++){
				if (arObjects.PLACEMARKS[i].metaDataProperty.cat==type){
					if (!(map_showhide[-1]==1 && map_point_data[i-arObjects_skip]['BASKET']>0)){
						show_hide_point(i,0);
					}
				}
			}
		}else if (map_showhide[type]==1){ // открыть слой
			document.getElementById('imgc_'+type).className = "opacity2";
			for (var i=arObjects_skip; i<arObjects.PLACEMARKS.length; i++){
				if (arObjects.PLACEMARKS[i].metaDataProperty.cat==type){
					show_hide_point(i,1);
