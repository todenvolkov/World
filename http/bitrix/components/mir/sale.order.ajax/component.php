...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y")
{
	if(IntVal($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		$dbBasketItems = CSaleBasket::GetList(
				...
				false,
				false,
				array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY", "CAN_BUY", "PRICE", "WEIGHT", "NAME", "CURRENCY")
			);
		while ($arBasketItems = $dbBasketItems->GetNext())
		{
			if (strlen($arBasketItems["CALLBACK_FUNC"])>0)
			{
				...
				$dbBasketItemsTmp = CSaleBasket::GetList(Array(), Array("ID" => $arBasketItems["ID"]), false, false, array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY", "CAN_BUY", "PRICE", "WEIGHT", "NAME", "CURRENCY"));
		...
		if($USER->IsAuthorized() && empty($arResult["ERROR"]))
		{
			...
			if (empty($arResult["ERROR"]))
			{
				...
				if ($arResult["PAY_FROM_ACCOUNT"] == "Y" && $arUserResult["PAY_CURRENT_ACCOUNT"] == "Y"
					&& (($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && DoubleVal($arResult["USER_ACCOUNT"]["CURRENT_BUDGET"]) >= DoubleVal($arResult["ORDER_PRICE"])) || $arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] != "Y"))
				{
			...
			if (empty($arResult["ERROR"]))
			{
				...
				while ($arBasketItems = $dbBasketItems->Fetch())
				{
					...
					while ($ar_res = $db_res->Fetch()) {
						if (strpos($ar_res["NAME"],"XML")===false){
							$strOrderList.=" ".$ar_res["NAME"]." - ".$ar_res["VALUE"]."\n";
						}
					}
					//******************************************
					//******************************************
				...
				while ($arVals = $db_vals->Fetch()){
					$personal_profile .= " ".$arVals["NAME"]." - ".$arVals["VALUE"]."\n";
				} 

				$arFields = Array(
						"PPROFILE" => $personal_profile,
						"ORDER_ID" => $arResult["ORDER_ID"],
