...
<a name="compare_table"></a>
	...
	<?if(count($arResult["DELETED_PROPERTIES"])>0):?>
		...
		<?foreach($arResult["DELETED_PROPERTIES"] as $arProperty):?>
			<a href="<?=htmlspecialchars($APPLICATION->GetCurPageParam("action=ADD_FEATURE&pr_code=".$arProperty["CODE"],array("pr_code","action")))?>" rel="nofollow"><?=$arProperty["NAME"]?></a>
		<?endforeach?>
		</p></noindex>
	<?endif?>
					...
					<?switch($code):
						case "NAME":
							...
							if($arElement["CAN_BUY"]):
								?><br /><a href="<?=$arElement["DETAIL_PAGE_URL"]?>#buy"><?=GetMessage("CATALOG_COMPARE_BUY"); ?></a><?
				...
				<?foreach($arResult["ITEMS"] as $arElement):?>
					<td valign="top">
						<?if($arElement["PRICES"][$code]["CAN_ACCESS"]):?>
							<b><?=$arElement["PRICES"][$code]["PRINT_VALUE"]?></b>
						<?endif;?>
					</td>
				<?endforeach?>
			</tr>
