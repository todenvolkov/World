...
if (array_key_exists("ecb", $_REQUEST))
	$entity_cb = trim($_REQUEST["ecb"]);

if (array_key_exists("evid", $_REQUEST))
	...
	elseif ($action == "get_data")
	{
		...
		foreach ($arSocNetLogEvents as $event_id_tmp => $arEventTmp)
		{
			...
			if (
				...
				&& !CSocNetEventUserView::CheckPermissionsByEvent($entity_type, $entity_id, $event_id_tmp, $GLOBALS["USER"]->GetID())
			)
				continue;				
		...
		{
			if (!in_array($entity_type, $arFeature["allowed"]))
				continue;

			if (!array_key_exists("subscribe_events", $arFeature) || !$arFeature["subscribe_events"])
				continue;
