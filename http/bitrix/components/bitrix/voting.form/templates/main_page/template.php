...
<div class="voting-form-box">
	...
	<?foreach ($arResult["QUESTIONS"] as $arQuestion):?>
		...
		<?foreach ($arQuestion["ANSWERS"] as $arAnswer):?>
			...
			<?switch ($arAnswer["FIELD_TYPE"]):
				case 0://radio?>
                    <label><input <?=$value?> type="radio" name="vote_radio_<?=$arAnswer["QUESTION_ID"]?>" value="<?=$arAnswer["ID"]?>" <?=$arAnswer["FIELD_PARAM"]?> <?=$checked?> />&nbsp;<?=$arAnswer["MESSAGE"]?></label>
					<br />
				<?break?>

				<?case 1://checkbox?>
                    <label><input <?=$value?> type="checkbox" name="vote_checkbox_<?=$arAnswer["QUESTION_ID"]?>[]" value="<?=$arAnswer["ID"]?>" <?=$arAnswer["FIELD_PARAM"]?> <?=$checked?> />&nbsp;<?=$arAnswer["MESSAGE"]?></label>
					<br />
				<?break?>
