...
if (!array_key_exists("SKIP_BP_TYPE_SELECT", $arParams) || $arParams["SKIP_BP_TYPE_SELECT"] != "Y")
{
	...
	$arSubMenu[] = array(
		...
		"ACTION"=> "if(confirm('".GetMessage("BIZPROC_WFEDIT_MENU_ADD_WARN")."'))window.location='".str_replace("#ID#", "0", $arResult["EDIT_PAGE_TEMPLATE"]).(strpos($arResult["EDIT_PAGE_TEMPLATE"], "?")?"&":"?")."init=statemachine';"
	);

	$arSubMenu[] = array(
		"TEXT"	=> GetMessage("BIZPROC_WFEDIT_MENU_ADD_SEQ"),
		"ACTION" => "if(confirm('".GetMessage("BIZPROC_WFEDIT_MENU_ADD_WARN")."'))window.location='".str_replace("#ID#", "0", $arResult["EDIT_PAGE_TEMPLATE"]).(strpos($arResult["EDIT_PAGE_TEMPLATE"], "?")?"&":"?")."';"
	);
...
?>
<style>
table.contextmenu td {font-size:100%;}
...
<input type="button" onclick="window.location='<?=htmlspecialchars(CUtil::JSEscape($arResult['LIST_PAGE_URL']))?>';" value="<?echo GetMessage("BIZPROC_WFEDIT_CANCEL_BUTTON")?>">
</div>
