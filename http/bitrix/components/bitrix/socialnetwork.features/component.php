	...
	if (StrLen($arResult["FatalError"]) <= 0)
	{
		...
		if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["save"]) > 0 && check_bitrix_sessid())
		{
			...
			foreach ($arResult["Features"] as $feature => $arFeature)
			{
				...
				if ($idTmp && (!array_key_exists("hide_operations_settings", $GLOBALS["arSocNetFeaturesSettings"][$feature]) || !$GLOBALS["arSocNetFeaturesSettings"][$feature]["hide_operations_settings"]))
				{
