...
?>
<style>
ul.photo-album-thumbs-list div.photo-item-cover-block-container, 
ul.photo-album-thumbs-list div.photo-item-cover-block-outer, 
ul.photo-album-thumbs-list div.photo-item-cover-block-inner{
...
?>
	<li class="photo-item photo-album-item photo-album-<?=($res["ACTIVE"] != "Y" ? "nonactive" : "active")?> <?=(
		...
		?> title="<?=trim("&laquo;".$res["~NAME"]."&raquo;  ".(!empty($res["DATE"]) ? " ".$res["DATE"]." " : "")).
			(!empty($sTitle) ? " (".$sTitle.")" : "")?>" <?
				...
				<div class="photo-item-cover-block-outer">
					<div class="photo-item-cover-block-inner">
						<div class="photo-item-cover-block-inside"><?
							?><div class="photo-item-cover photo-album-thumbs-avatar <?=(empty($res["DETAIL_PICTURE"]["SRC"])? "photo-album-avatar-empty" : "")?>" <?
								...
								?><div class="photo-album-menu" onmouseout="this.style.display='none';" <?
									...
									?><div class="photo-album-menu-controls"><?
										?><a rel="nofollow" href="<?=$res["EDIT_LINK"]?>" <?
											...
											?> onclick="BX.PreventDefault(event); EditAlbum('<?=CUtil::JSEscape(htmlspecialchars($res["EDIT_LINK"]))?>'); return false;" <?
										...
										?><a rel="nofollow" href="<?=$res["EDIT_ICON_LINK"]?>" <?
											...
											?> onclick="BX.PreventDefault(event); EditAlbum('<?=CUtil::JSEscape(htmlspecialchars($res["EDIT_ICON_LINK"]))?>'); return false;" <?
										...
										?><a rel="nofollow" href="<?= $res["DROP_LINK"]?>" <?
											...
											?> onclick="BX.PreventDefault(event); if(confirm('<?=GetMessage('P_SECTION_DELETE_ASK')?>')) {DropAlbum(this.href);} return false;" <?
