<?
$MESS ['COURSE_ID_TIP'] = "Select here one of the existing courses. If you select <b><i>(other)</i></b>, you will have to specify the course ID in the field beside.";
$MESS ['CHAPTER_ID_TIP'] = "The expression that evaluates to the chapter ID.";
$MESS ['CHAPTER_DETAIL_TEMPLATE_TIP'] = "Specifies the path to a course chapter page.";
$MESS ['LESSON_DETAIL_TEMPLATE_TIP'] = "Specifies the path to a course lesson page.";
$MESS ['CHECK_PERMISSIONS_TIP'] = "Select here \"Yes\" if you want to check access permissions.";
$MESS ['SET_TITLE_TIP'] = "Checking this option will set the page title to the chapter name.";
$MESS ['CACHE_TYPE_TIP'] = "<i>Auto</i>: the cache is valid during the time predefined in the cache settings;<br /><i>Cache</i>: always cache for the period specified in the next field;<br /><i>Do not cahce</i>: no caching is performed.";
$MESS ['CACHE_TIME_TIP'] = "Specify here the period of time during which the cache is valid.";
?>