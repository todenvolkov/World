...
if ($arParams["THUMBS_SIZE"] > 0)
	$arParams["PICTURES"]["standart"] = array("size" => $arParams["THUMBS_SIZE"]);
//PROPERTIES
	...
	if ($rsElements)
	{
		...
		while ($obElement = $rsElements->GetNextElement())
		{
			...
			if (true):
				if (strToUpper($arParams["PICTURES_SIGHT"]) == "REAL" || empty($arElement["PROPERTIES"]["REAL_PICTURE"]["VALUE"])):
					$arElement["REAL_PICTURE"] = $arElement["PICTURE"];
				else:
		...
		*/
		...
		while ($obElement = $rsElements->GetNextElement())
		{
			...
			for ($i = 0; $i < $l; $i++)
			{
				if (isset($props[$arUsableProps[$i]]))
					$arElement["PROPERTIES"][$arUsableProps[$i]] = array("VALUE" => $props[$arUsableProps[$i]]["VALUE"]);
			}
		...
		if (strLen($strFileId) > 0)
		{
			...
			while ($obFile = $rsFile->Fetch())
			{
				...
				// Copypaste from CFile::GetFileArray
				$src = "/".$upload."/".$obFile["SUBDIR"]."/".$obFile["FILE_NAME"];
				$src = str_replace("//", "/", $src);
...
if (in_array($this->__templateName, array("table", "ascetic")) &&
	(!($this->__parent && is_dir($_SERVER['DOCUMENT_ROOT'].$this->__parent->__template->__folder."/bitrix/photogallery.detail.list/".$this->__templateName))))
{
	$arParams["TEMPLATE"] = $this->__templateName;
