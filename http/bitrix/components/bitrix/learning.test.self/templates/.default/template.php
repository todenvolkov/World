...
<div id="learn_question_<?=$arResult["LESSON"]["ID"]?>_<?=($index+1)?>" style="display:none;">
	...
	<form name="form_self_<?=$arResult["LESSON"]["ID"]?>_<?=($index+1)?>" onSubmit="return false;" action="">
		<input type="hidden" name="QUESTION_TYPE" id="question_type" value="<?php echo $arQuestion["QUESTION_TYPE"]?>" />
		<?php if ($arQuestion["QUESTION_TYPE"] == "R"):?>
			<?php for ($i = 0; $i < sizeof($arQuestion["ANSWERS"]); $i++):?>
				...
				<div class="sorting">
				<?php echo $i+1?>.
				<select name="answer[<?php echo $arQuestion["ANSWERS_ORIGINAL"][$i]["ID"]?>]" onChange="LearnTab_<?=$arResult["LESSON"]["ID"]?>.OnChangeAnswer(true);">
