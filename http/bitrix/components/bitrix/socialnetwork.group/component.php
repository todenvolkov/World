...
if (strlen($arParams["PATH_TO_USER_REQUEST_GROUP"]) <= 0)
	$arParams["PATH_TO_USER_REQUEST_GROUP"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_request_group&".$arParams["USER_VAR"]."=#user_id#&".$arParams["GROUP_VAR"]."=#group_id#");
...
if (strlen($arParams["PATH_TO_GROUP_REQUESTS_OUT"]) <= 0)
	$arParams["PATH_TO_GROUP_REQUESTS_OUT"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=group_requests_out&".$arParams["GROUP_VAR"]."=#group_id#");
...
if (strlen($arParams["PATH_TO_USER_LEAVE_GROUP"]) <= 0)
	$arParams["PATH_TO_USER_LEAVE_GROUP"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_leave_group&".$arParams["GROUP_VAR"]."=#group_id#");

$arParams["PATH_TO_GROUP_FEATURES"] = trim($arParams["PATH_TO_GROUP_FEATURES"]);
if (strlen($arParams["PATH_TO_GROUP_FEATURES"]) <= 0)
	$arParams["PATH_TO_GROUP_FEATURES"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=group_features&".$arParams["GROUP_VAR"]."=#group_id#");
...
if (strlen($arParams["PATH_TO_GROUP_DELETE"]) <= 0)
	$arParams["PATH_TO_GROUP_DELETE"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=group_delete&".$arParams["GROUP_VAR"]."=#group_id#");
...
if (strlen($arParams["PATH_TO_MESSAGE_TO_GROUP"]) <= 0)
	$arParams["PATH_TO_MESSAGE_TO_GROUP"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=message_to_group&".$arParams["GROUP_VAR"]."=#group_id#");
...
if (strlen($arParams["PATH_TO_GROUP_LOG"]) <= 0)
	$arParams["PATH_TO_GROUP_LOG"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=group-log&".$arParams["GROUP_VAR"]."=#group_id#");
...
else
{
	...
	else
	{
		...
		if ($arParams["SET_TITLE"]=="Y")
			$APPLICATION->SetTitle($arResult["Group"]["NAME"].": ".GetMessage("SONET_C5_TITLE"));
		...
		if (!$arParams["SHORT_FORM"])
		{
			...
			$dbModerators = CSocNetUserToGroup::GetList(
				...
				array("ID", "USER_ID", "ROLE", "USER_NAME", "USER_LAST_NAME", "USER_SECOND_NAME", "USER_LOGIN", "USER_PERSONAL_PHOTO", "USER_PERSONAL_GENDER")
			);
			if ($dbModerators)
			{
