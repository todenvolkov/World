...
function __forum_default_template_show_message($arResult, $message, $arRes, $arAddParams, $arParams)
{
	$iCount = 0;
...
<?
					...
					if ($res["PANELS"]["MODERATE"] == "Y"):
						if ($res["APPROVED"] == "Y"):
?>
								<span class="forum-action-hide"><a href="<?=$res["URL"]["MESSAGE_SHOW"]?>"><?=GetMessage("F_HIDE")?></a></span>&nbsp;&nbsp;
<?
						else:
?>
								<span class="forum-action-show"><a href="<?=$res["URL"]["MESSAGE_SHOW"]?>"><?=GetMessage("F_SHOW")?></a></span>&nbsp;&nbsp;
<?
						endif;
					endif;
					if ($res["PANELS"]["DELETE"] == "Y"):
?>
								<span class="forum-action-delete"><a href="<?=$res["URL"]["MESSAGE_DELETE"]?>" <?
