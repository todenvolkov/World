...
if (strlen($arParams["REDIRECT_PAGE"]) <= 0) $arParams["REDIRECT_PAGE"] = "index.php";
...
if (CModule::IncludeModule("sale"))
{
	...
	if (!$GLOBALS["USER"]->IsAuthorized())
	{
		...
		elseif ($_REQUEST["do_register"] == "Y")
		{
			...
			$arResult["NEW_LOGIN"] = htmlspecialchars($NEW_LOGIN);
			$arResult["NEW_NAME"] = htmlspecialchars($NEW_NAME);
			$arResult["NEW_LAST_NAME"] = htmlspecialchars($NEW_LAST_NAME);
			$arResult["NEW_PASSWORD"] = htmlspecialchars($NEW_PASSWORD);
			$arResult["NEW_PASSWORD_CONFIRM"] = htmlspecialchars($NEW_PASSWORD_CONFIRM);
			$arResult["NEW_EMAIL"] = htmlspecialchars($NEW_EMAIL);
		}
	}
	...
	$arResult["REDIRECT_PAGE"] = htmlspecialchars($arParams["REDIRECT_PAGE"]);
	$arResult["DEFAULT_USER_LOGIN"] = (strlen($_REQUEST["USER_LOGIN"]) > 0) ? htmlspecialchars($_REQUEST["USER_LOGIN"]) : htmlspecialchars($arResult["DEFAULT_USER_LOGIN"]);
