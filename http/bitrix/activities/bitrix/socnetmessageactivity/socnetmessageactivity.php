	...
	extends CBPActivity
{
	...
	public function Execute()
	{
		...
		$arMessageUserFrom = array();
		$arMessageUserFromTmp = $this->MessageUserFrom;
		$arMessageUserFromTmp = (is_array($arMessageUserFromTmp) ? $arMessageUserFromTmp : array($arMessageUserFromTmp));
		$l = strlen("user_");
		foreach ($arMessageUserFromTmp as $user)
		{
			if (substr($user, 0, $l) == "user_")
			{
				$user = intval(substr($user, $l));
				if ($user > 0)
					$arMessageUserFrom[] = $user;
			}
			else
			{
		...
		$arMessageUserTo = array();
		$arMessageUserToTmp = $this->MessageUserTo;
		$arMessageUserToTmp = (is_array($arMessageUserToTmp) ? $arMessageUserToTmp : array($arMessageUserToTmp));
		$l = strlen("user_");
		foreach ($arMessageUserToTmp as $user)
		{
			if (substr($user, 0, $l) == "user_")
			{
				$user = intval(substr($user, $l));
				if ($user > 0)
					$arMessageUserTo[] = $user;
			}
			else
			{
				$arDSUsers = $documentService->GetUsersFromUserGroup($user, $documentId);
		...
		$arMessageFields = array(
			...
			"FROM_USER_ID" => $arMessageUserFrom[0],
