...
else
{
	...
	switch($STEP)
	{
		...
		case 1:
			...
			if (strlen($data) > 0)
			{
				$fp = fopen($_SERVER['DOCUMENT_ROOT'].'/bitrix/wizards/bitrix/sale.locations/upload/'.$CSVFILE, 'w');
		...
		case 2:
			...
			if (strlen($data) > 0)
			{
				$fp = fopen($_SERVER['DOCUMENT_ROOT'].'/bitrix/wizards/bitrix/sale.locations/upload/'.DLZIPFILE, 'w');
				fwrite($fp, $APPLICATION->ConvertCharset($data, 'windows-1251', LANG_CHARSET));
				fclose($fp);
