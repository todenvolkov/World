...
class Post extends CActiveRecord
{
	...
	public static function getImages($text,$index="all",$default="/site/themes/mir/css/images/post-li-bg.png"){
		$have_images = preg_match_all('/<img.+\"(.+)\.([\w]+)+".+>/i', $text, $matches, PREG_SET_ORDER);
		if($have_images){
			if($index=="all"){
				$out = array();
				foreach ($matches as $val){
					array_push($out,$val[1].".".$val[2]);
				}
			}else{
				$out = array($matches[(int)$index][1].".".$matches[(int)$index][2]);
			}
			return $out;
